...
func NewCmdBuild() *cobra.Command {
	return NewCmd("build").
		...
		WithExample("Build the artifacts and then deploy them", "build -q | devloop deploy --build-artifacts -").
...
func doBuild(ctx context.Context, out io.Writer) error {
	...
	return withRunner(ctx, out, func(r runner.Runner, configs []util.VersionedConfig) error {
		...
		if quietFlag || buildOutputFlag != "" {
			...
			if buildOutputFlag != "" {
				if err := os.WriteFile(buildOutputFlag, buildOutput.Bytes(), 0o644); err != nil {
...
func targetArtifacts(opts config.DevloopOptions, configs []util.VersionedConfig) []*latest.Artifact {
	var targetArtifacts []*latest.Artifact
	for _, cfg := range configs {
		for _, artifact := range cfg.(*latest.DevloopConfig).Build.Artifacts {
