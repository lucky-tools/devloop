...
func newMockCreateRunner(artifacts []*latest.Artifact) func(context.Context, io.Writer, config.DevloopOptions) (runner.Runner, []util.VersionedConfig, *runcontext.RunContext, error) {
	return func(context.Context, io.Writer, config.DevloopOptions) (runner.Runner, []util.VersionedConfig, *runcontext.RunContext, error) {
		return &mockRunner{}, []util.VersionedConfig{&latest.DevloopConfig{
...
func TestTagFlag(t *testing.T) {
	mockCreateRunner := newMockCreateRunner([]*latest.Artifact{{ImageName: "gcr.io/devloop/example"}})
	...
	testutil.Run(t, "override tag with argument", func(t *testutil.T) {
		...
		t.CheckNoError(err)
		t.CheckDeepEqual(string([]byte(`{"builds":[{"imageName":"gcr.io/devloop/example","tag":"test"}]}`)), output.String())
	})
}
...
func TestQuietFlag(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedOutput: []byte(`{"builds":[{"imageName":"gcr.io/devloop/example","tag":"test"}]}`),
			shouldErr:      false,
		},
		{
			description:    "quiet flag print build images applies pattern specified in template ",
			template:       "{{range .Builds}}{{.ImageName}} -> {{.Tag}}\n{{end}}",
			expectedOutput: []byte("gcr.io/devloop/example -> test\n"),
			shouldErr:      false,
		},
		{
...
func TestFileOutputFlag(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedFileContent: []byte(`{"builds":[{"imageName":"gcr.io/devloop/example","tag":"test"}]}`),
		},
		{
		...
		},
		{
			...
			template:            "{{range .Builds}}{{.ImageName}} -> {{.Tag}}\n{{end}}",
			expectedOutput:      []byte("gcr.io/devloop/example -> test\n"),
			expectedFileContent: []byte("gcr.io/devloop/example -> test\n"),
		},
	}
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
...
func TestRunBuild(t *testing.T) {
	errRunner := func(context.Context, io.Writer, config.DevloopOptions) (runner.Runner, []util.VersionedConfig, *runcontext.RunContext, error) {
	...
	tests := []struct {
		description string
		mock        func(context.Context, io.Writer, config.DevloopOptions) (runner.Runner, []util.VersionedConfig, *runcontext.RunContext, error)
		shouldErr   bool
	}{
		{
...
func TestRuntimeType(t *testing.T) {
	mockCreateRunner := newMockCreateRunner([]*latest.Artifact{{
		ImageName:   "gcr.io/devloop/example",
		RuntimeType: "go",
	}})
	...
	testutil.Run(t, "set runtime type on artifact", func(t *testutil.T) {
		...
		t.CheckNoError(err)
		t.CheckDeepEqual(string([]byte(`{"builds":[{"imageName":"gcr.io/devloop/example","tag":"test","runtimeType":"go"}]}`)), output.String())
	})
}
