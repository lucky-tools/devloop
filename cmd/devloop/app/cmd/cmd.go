...
import (
	...
	"github.com/lucky-tools/devloop/pkg/devloop/instrumentation/prompt"
	...
	"github.com/lucky-tools/devloop/pkg/devloop/server"
...
const (
	HouseKeepingMessagesAllowedAnnotation = "devloop_annotation_housekeeping_allowed"
)

func NewDevloopCommand(out, errOut io.Writer) *cobra.Command {
  ...
  Find more information at: https://devloop.dev/docs/getting-started/`,
		SilenceErrors: true,
		...
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			...
			log.Entry(context.TODO()).Infof("Devloop %+v", versionInfo)
	...
	rootCmd.PersistentFlags().BoolVar(&update.EnableCheck, "update-check", true, "Check for a more recent version of Devloop")
...
// setEnvVariablesFromFile will read the `devloop.env` file and load them into ENV for this process.
func setEnvVariablesFromFile() {
	if _, err := os.Stat(constants.DevloopEnvFile); os.IsNotExist(err) {
		log.Entry(context.TODO()).Debugf("Skipped loading environment variables from file %q: %s", constants.DevloopEnvFile, err)
		return
	}
	err := godotenv.Load(constants.DevloopEnvFile)
	if err != nil {
		log.Entry(context.TODO()).Warnf("Failed to load environment variables from file %q: %s", constants.DevloopEnvFile, err)
	}
}
...
func FlagToEnvVarName(f *pflag.Flag) string {
	return fmt.Sprintf("DEVLOOP_%s", strings.ReplaceAll(strings.ToUpper(f.Name), "-", "_"))
}
