...
const (
	...
	OS X:
		...
		$ devloop completion bash > ~/.devloop-completion  # for bash users
		$ devloop completion fish > ~/.devloop-completion  # for fish users
		$ devloop completion zsh > ~/.devloop-completion   # for zsh users
		$ source ~/.devloop-completion
	Ubuntu:
		$ apt-get install bash-completion
		$ source /etc/bash-completion
		$ source <(devloop completion bash) # for bash users
		$ devloop completion fish | source  # for fish users
		$ source <(devloop completion zsh)  # for zsh users

	Additionally, you may want to output the completion to a file and source in your .bashrc
`

	zshCompdef = "\ncompdef _devloop devloop\n"
)
