...
import (
	...
	"github.com/lucky-tools/devloop/cmd/devloop/app/cmd/config"
)

func NewCmdConfig() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "config",
		Short: "Interact with the global Devloop config file (defaults to `$HOME/.devloop/config`)",
	}
...
func NewCmdSet() *cobra.Command {
	return NewCmd("set").
		WithDescription("Set a value in the global Devloop config").
...
func NewCmdUnset() *cobra.Command {
	return NewCmd("unset").
		WithDescription("Unset a value in the global Devloop config").
		WithFlagAdder(func(f *pflag.FlagSet) {
			config.AddCommonFlags(f)
			config.AddSetUnsetFlags(f)
		}).
...
func NewCmdList() *cobra.Command {
	return NewCmd("list").
		WithDescription("List all values set in the global Devloop config").
		WithFlagAdder(func(f *pflag.FlagSet) {
			config.AddCommonFlags(f)
