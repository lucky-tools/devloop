...
func NewCmdExec() *cobra.Command {
	return NewCmd("exec").
		...
		WithExample("Execute a defined action that uses an image built from Devloop. First, build the images", "build --file-output=build.json").
...
func getActionImgs(action string, configs []util.VersionedConfig) map[string]bool {
	...
	for _, cfg := range configs {
		allActions = append(allActions, cfg.(*latest.DevloopConfig).CustomActions...)
	}
