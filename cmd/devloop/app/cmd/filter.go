...
import (
	...
	"github.com/lucky-tools/devloop/pkg/devloop/render/applysetters"
	rUtil "github.com/lucky-tools/devloop/pkg/devloop/render/renderer/util"
	...
	pkgutil "github.com/lucky-tools/devloop/pkg/devloop/util"
)
...
func NewCmdFilter() *cobra.Command {
	...
	return NewCmd("filter").
		...
		WithCommonFlags().
		WithFlags([]*Flag{
			{Value: &renderFromBuildOutputFile, Name: "build-artifacts", Shorthand: "a", Usage: "File containing build result from a previous 'devloop build --file-output'"},
			{Value: &debuggingFilters, Name: "debugging", DefValue: false, Usage: `Apply debug transforms similar to "devloop debug"`, IsEnum: true},
...
// Unlike `devloop debug`, this filtering affects all images and not just the built artifacts.
...
func getTransformList(configs []util.VersionedConfig) (map[apim.GroupKind]latest.ResourceFilter, map[apim.GroupKind]latest.ResourceFilter) {
	...
	for _, cfg := range configs {
		for _, rf := range cfg.(*latest.DevloopConfig).ResourceSelector.Allow {
			groupKind := apim.ParseGroupKind(rf.GroupKind)
			allow[groupKind] = rUtil.ConvertJSONPathIndex(rf)
		}
		for _, rf := range cfg.(*latest.DevloopConfig).ResourceSelector.Deny {
			groupKind := apim.ParseGroupKind(rf.GroupKind)
			deny[groupKind] = rUtil.ConvertJSONPathIndex(rf)
		}
	}
...
func getInsecureRegistries(opts config.DevloopOptions, configs []util.VersionedConfig) (map[string]bool, error) {
	...
	for _, cfg := range configs {
		regList = append(regList, cfg.(*latest.DevloopConfig).Build.InsecureRegistries...)
	}
