...
// NewCmdFindConfigs list the devloop config files in the specified directory.
func NewCmdFindConfigs() *cobra.Command {
	return NewCmd("find-configs").
		WithDescription("Find in a given directory all devloop yamls files that are parseable or upgradeable with their versions.").
		WithFlags([]*Flag{
...
func findConfigs(ctx context.Context, directory string) (map[string]string, error) {
	...
	// Find files ending in ".yaml" and parseable to devloop config in the specified root directory recursively.
