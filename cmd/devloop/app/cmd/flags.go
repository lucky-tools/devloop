...
import (
	...
	"github.com/lucky-tools/devloop/cmd/devloop/app/flags"
...
// Flag defines a Devloop CLI flag which contains a list of
...
// flagRegistry is a list of all Devloop CLI flags.
...
var flagRegistry = []Flag{
	{
		Name:          "filename",
		Shorthand:     "f",
		Usage:         "Path or URL to the Devloop config file",
		Value:         &opts.ConfigurationFile,
		DefValue:      "devloop.yaml",
	...
	},
	{
		Name:          "module",
		Shorthand:     "m",
		Usage:         "Filter Devloop configs to only the provided named modules",
	...
	{
		Name:          "cache-file",
		Usage:         "Specify the location of the cache file (default $HOME/.devloop/cache)",
	...
	},
	{
		...
		Usage:         "Specify the location of the remote cache (default $HOME/.devloop/remote-cache)",
	...
	},
	{
		...
		Usage:    "Controls how Devloop manages the remote config cache (see `remote-cache-dir`). One of `always` (default), `missing`, or `never`. `always` syncs remote repositories to latest on access. `missing` only clones remote repositories if they do not exist locally. `never` means the user takes responsibility for updating remote repositories.",
	...
	},
	{
		...
		Usage:         "Blocks ending execution of devloop until the /v2/events gRPC/HTTP endpoint is hit",
	...
	{
		Name:          "event-log-file",
		Usage:         "Save Devloop events to the provided file after devloop has finished executing, requires --rpc-port or --rpc-http-port",
		Hidden:        true,
	...
	{
		Name:          "last-log-file",
		Usage:         "Save Devloop output to the provided file after devloop has finished executing, requires --rpc-port or --rpc-http-port (defaults to $HOME/.devloop/repos)",
		Hidden:        true,
	...
	{
		Name:          "rpc-port",
		Usage:         "tcp port to expose the Devloop API over gRPC",
		Value:         &opts.RPCPort,
		DefValue:      nil,
		FlagAddMethod: "Var",
	...
	{
		Name:          "rpc-http-port",
		Usage:         "tcp port to expose the Devloop API over HTTP REST",
	...
	{
		Name:          "no-prune",
		Usage:         "Skip removing images and containers built by Devloop during cleanup after dev or debug mode",
		Value:         &opts.NoPrune,
		DefValue:      false,
		FlagAddMethod: "BoolVar",
		DefinedOn:     []string{"dev", "run", "debug"},
	...
	},
	{
		...
		Usage:         "Skip removing layers reused by Devloop",
	...
	},
	{
		...
		Usage:         "Configures `status-check` to tolerate failures until Devloop's statusCheckDeadline duration or the deployments progressDeadlineSeconds  Otherwise deployment failures devloop encounters will immediately fail the deployment.  Defaults to 'false'",
	...
	},
	{
		Name:          "config",
		Shorthand:     "c",
		Usage:         "File for global configurations (defaults to $HOME/.devloop/config)",
	...
	},
	{
		...
		Usage:         "forces devloop use the given minikube-profile and forces building against the docker daemon inside that minikube profile",
		...
		// https://github.com/lucky-tools/devloop/issues/3668
		Hidden: true,
	},
	{
	...
	{
		Name:          "auto",
		Usage:         "Run with an auto-generated devloop configuration. This will create a temporary `devloop.yaml` file and kubernetes manifests necessary to run the application",
		Value:         &opts.AutoInit,
		DefValue:      false,
		FlagAddMethod: "BoolVar",
	...
	},
	{
		...
		Usage:         "Setting '--propagate-profiles=false' disables propagating profiles set by the '--profile' flag across config dependencies. This mean that only profiles defined directly in the target 'devloop.yaml' file are activated.",
...
The build result from a previous 'devloop build --file-output' run can be used here`,
...

	{
		...
		Usage:         "If true, devloop will try to create a config for the user's run if it doesn't find one",
	...
	{
		Name:          "assume-yes",
		Usage:         "If true, devloop will skip yes/no confirmation from the user and default to yes",
	...
	{
		Name:          "load-images",
		Usage:         "If true, devloop will force load the container images into the local cluster.",
	...
	},
	{
		...
		Usage:         "Path to JSON file specifying the deny list of yaml objects for devloop to NOT transform with 'image' and 'label' field replacements.  NOTE: this list is additive to devloop's default denylist and denylist has priority over allowlist",
	...
	},
	{
		Name:          "docker-network",
		Shorthand:     "",
		Usage:         "Name of an existing docker network to use when running the verify tests. If not specified, Devloop will create a new network to use of the form 'devloop-network-<uuid>'",
	...
	},
	{
		...
		Usage:    "If true, when deploying to a mixed node cluster, devloop will add platform (os/arch) node affinity definition to rendered manifests based on the image platforms",
