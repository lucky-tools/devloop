...
// NewCmdInit describes the CLI command to generate a Devloop configuration.
func NewCmdInit() *cobra.Command {
	return NewCmd("init").
		WithDescription("Generate configuration for deploying an application").
		WithCommonFlags().
		WithFlags([]*Flag{
			{Value: &skipBuild, Name: "skip-build", DefValue: false, Usage: "Skip generating build artifacts in Devloop config", IsEnum: true},
			{Value: &skipDeploy, Name: "skip-deploy", DefValue: false, Usage: "Skip generating deploy stanza in Devloop config", Hidden: true, IsEnum: true},
			{Value: &force, Name: "force", DefValue: false, Usage: "Force the generation of the Devloop config", IsEnum: true},
			...
			{Value: &enableManifestGeneration, Name: "generate-manifests", DefValue: false, Usage: "Allows devloop to try and generate basic kubernetes resources to get your project started", IsEnum: true},
