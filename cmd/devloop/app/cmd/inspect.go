...
func NewCmdInspect() *cobra.Command {
	return NewCmd("inspect").
		...
		WithDescription("Helper commands for Cloud Code IDEs to interact with and modify devloop configuration files.").
...
func cmdInspectFlags(f *pflag.FlagSet) {
	f.StringVarP(&inspectFlags.filename, "filename", "f", "devloop.yaml", "Path to the local Devloop config file. Defaults to `devloop.yaml`")
	f.StringVarP(&inspectFlags.outFormat, "format", "o", "json", "Output format. One of: json(default)")
	f.StringVar(&inspectFlags.remoteCacheDir, "remote-cache-dir", "", "Specify the location of the remote cache (defaults to $HOME/.devloop/remote-cache)")
}
