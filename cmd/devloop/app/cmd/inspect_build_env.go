...
import (
	...
	buildEnv "github.com/lucky-tools/devloop/pkg/devloop/inspect/buildEnv"
)
...
func cmdBuildEnv() *cobra.Command {
	return NewCmd("build-env").
		WithDescription("Interact with devloop build environment definitions.").
...
func cmdBuildEnvAdd() *cobra.Command {
	return NewCmd("add").
		...
		WithCommands(cmdBuildEnvAddLocal(), cmdBuildEnvAddCluster())
}
...
In these respective scenarios, it will fail if the build env definition for the default pipeline or the named profile already exists. To override an existing 'local' build env definition use 'devloop inspect build-env modify' command instead.
Use the '--module' filter to specify the individual module to target. Otherwise, it'll be applied to all modules defined in the target file. Also, with the '--profile' flag if the target config imports other configs as dependencies, then the new profile will be recursively created in all the imported configs also.`).
		WithExample("Add a new profile named 'local' targeting the local build environment with option to push images", "inspect build-env add local --profile local --push=true -f devloop.yaml").
...
Without the '--profile' flag the new environment definition is added to the default pipeline. With the '--profile' flag it will create a new profile with this build env definition.
In these respective scenarios, it will fail if the build env definition for the default pipeline or the named profile already exists. To override an existing 'cluster' build env definition use 'devloop inspect build-env modify' command instead.
Use the '--module' filter to specify the individual module to target. Otherwise, it'll be applied to all modules defined in the target file. Also, with the '--profile' flag if the target config imports other configs as dependencies, then the new profile will be recursively created in all the imported configs also.`).
		WithExample("Add a new profile named 'cluster' targeting the builder 'kaniko' using the Kubernetes secret 'my-secret'", "inspect build-env add cluster --profile cluster --pullSecretName my-secret -f devloop.yaml").
...
func cmdBuildEnvListFlags(f *pflag.FlagSet) {
	...
	f.BoolVar(&inspectFlags.propagateProfiles, "propagate-profiles", true, `Setting '--propagate-profiles=false' disables propagating profiles set by the '--profile' flag across config dependencies. This mean that only profiles defined directly in the target 'devloop.yaml' file are activated.`)
}
