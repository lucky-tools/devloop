...
import (
	...
	configDependencies "github.com/lucky-tools/devloop/pkg/devloop/inspect/configDependencies"
...
func cmdConfigDependencies() *cobra.Command {
	return NewCmd("config-dependencies").
		WithDescription("Interact with devloop config dependency definitions.").
...
func cmdConfigDependenciesAdd() *cobra.Command {
	return NewCmd("add").
		...
		WithExample("Add config dependency defined in `dependency.json`.", "inspect config-dependencies add dependency.json -f devloop.yaml").
