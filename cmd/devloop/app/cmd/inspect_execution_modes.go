...
import (
	...
	executionModes "github.com/lucky-tools/devloop/pkg/devloop/inspect/executionModes"
)

func cmdExecutionModes() *cobra.Command {
	return NewCmd("executionModes").
		WithDescription("View devloop executionMode information defined in the specified devloop configuration").
...
func cmdExecutionModesList() *cobra.Command {
	return NewCmd("list").
		...
		WithDescription("Print the list of executionModes that would be run for a given configuration (default devloop configuration, specific module, specific profile, etc).").
