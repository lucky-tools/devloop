...
import (
	...
	namespaces "github.com/lucky-tools/devloop/pkg/devloop/inspect/namespaces"
	olog "github.com/lucky-tools/devloop/pkg/devloop/output/log"
)

func cmdNamespaces() *cobra.Command {
	return NewCmd("namespaces").
		WithDescription("View devloop namespace information for resources it manages").
...
func cmdNamespacesList() *cobra.Command {
	return NewCmd("list").
		...
		WithDescription("Print the list of namespaces that would be run for a given configuration (default devloop configuration, specific module, specific profile, etc).").
...
//     additionally to a devloop command (eg: devloop apply -n foo)
