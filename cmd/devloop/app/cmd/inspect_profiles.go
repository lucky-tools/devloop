...
import (
	...
	profiles "github.com/lucky-tools/devloop/pkg/devloop/inspect/profiles"
)
...
func cmdProfilesList() *cobra.Command {
	return NewCmd("list").
		...
		WithDescription("Print the list of profile names that can be invoked with the --profile flag in other devloop commands.").
...
func cmdProfilesListFlags(f *pflag.FlagSet) {
	f.StringVar(&inspectFlags.buildEnv, "build-env", "", `If specified as one of "local" or "cluster", then filter the output profiles list to that build environment.`)
}
