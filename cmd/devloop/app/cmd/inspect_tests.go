...
import (
	...
	tests "github.com/lucky-tools/devloop/pkg/devloop/inspect/tests"
)

func cmdTests() *cobra.Command {
	return NewCmd("tests").
		WithDescription("View devloop test information").
...
func cmdTestsList() *cobra.Command {
	return NewCmd("list").
		...
		WithDescription("Print the list of tests that would be run for a given configuration (default devloop configuration, specific module, specific profile, etc).").
