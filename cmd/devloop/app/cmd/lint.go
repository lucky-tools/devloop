...
func NewCmdLint() *cobra.Command {
	return NewCmd("lint").
		WithDescription("Lints devloop app devloop.yaml(s) to improve devloop's - user-experience, performance, etc..").
		WithCommonFlags().
		WithFlags([]*Flag{
			{
...
func doLint(ctx context.Context, out io.Writer) error {
	...
	log.Entry(ctx).Debugf("starting devloop lint with runCtx: %v", runCtx)
	if err != nil {
		return err
	}
