...
import (
	...
	"github.com/lucky-tools/devloop/pkg/devloop/lsp"
...
func NewCmdLSP() *cobra.Command {
	return NewCmd("lsp").
		WithDescription("Starts devloop LSP language server that provides lint suggestions to IDEs").
		WithCommonFlags().
		WithFlags([]*Flag{
			{
				Value: &port, Name: "port", DefValue: 4389,
				Usage: "port number that the devloop lsp will expose for connections, '4389' is the default port value",
			},
		}).
		Hidden().
		NoArgs(doLSP)
...
func doLSP(ctx context.Context, out io.Writer) error {
	...
	fmt.Fprintf(out, "devloop lsp listening for TCP connections on: %v\n", addr)
	connectionCount := 0
	for {
		...
		connectionCount++
		log.Entry(ctx).Infof("devloop lsp received incoming connection #%d\n", connectionCount)
