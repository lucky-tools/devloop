...
import (
	...
	"github.com/lucky-tools/devloop/cmd/devloop/app/cmd/schema"
)

func NewCmdSchema() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "schema",
		Short: "List JSON schemas used to validate devloop.yaml configuration",
	}
...
func NewCmdSchemaList() *cobra.Command {
	return NewCmd("list").
		...
		WithDescription("List devloop.yaml's json schema versions").
...
func NewCmdSchemaGet() *cobra.Command {
	return NewCmd("get").
		WithDescription("Print a given devloop.yaml's json schema").
		WithExample("Print the schema in version `devloop/v1`", "schema get devloop/v1").
