...
func NewCmdTest() *cobra.Command {
	return NewCmd("test").
		...
		WithExample("Run test against images previously built by Devloop into a 'tags.json' file", "test --build-artifacts=tags.json").
		WithCommonFlags().
...
func doTest(ctx context.Context, out io.Writer) error {
	return withRunner(ctx, out, func(r runner.Runner, configs []util.VersionedConfig) error {
		...
		for _, c := range configs {
			artifacts = append(artifacts, c.(*latest.DevloopConfig).Build.Artifacts...)
		}
