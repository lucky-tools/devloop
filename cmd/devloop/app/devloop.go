...
func Run(out, stderr io.Writer) error {
	...
	c := cmd.NewDevloopCommand(out, stderr)
	if cmdLine := os.Getenv("DEVLOOP_CMDLINE"); cmdLine != "" && len(os.Args) == 1 {
		parsed, err := shell.Split(cmdLine)
		if err != nil {
			return fmt.Errorf("DEVLOOP_CMDLINE is invalid: %w", err)
		}
		// XXX logged before logrus.SetLevel is called in NewDevloopCommand's PersistentPreRunE
		log.Entry(ctx).Debugf("Retrieving command line from DEVLOOP_CMDLINE: %q", parsed)
		c.SetArgs(parsed)
	}
	...
	if err != nil {
		...
		if errors.Is(err, context.Canceled) {
			...
			// for the entire devloop run here. It's possible SetupColors() was never called, so call it again
