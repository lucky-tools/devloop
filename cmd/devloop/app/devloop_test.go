...
func TestMainHelp(t *testing.T) {
	testutil.Run(t, "", func(t *testutil.T) {
		...
		t.Override(&os.Args, []string{"devloop", "help", "--interactive=false"})
...
func TestMainUnknownCommand(t *testing.T) {
	testutil.Run(t, "", func(t *testutil.T) {
		// --interactive=false removes the update check and survey prompt.
		t.Override(&os.Args, []string{"devloop", "unknown", "--interactive=false"})
...
func TestDevloopCmdline_MainHelp(t *testing.T) {
	testutil.Run(t, "", func(t *testutil.T) {
		var (
			output    bytes.Buffer
			errOutput bytes.Buffer
		...
		t.Setenv("DEVLOOP_CMDLINE", "help")
		t.Override(&os.Args, []string{"devloop"})
...
func TestDevloopCmdline_MainUnknownCommand(t *testing.T) {
	testutil.Run(t, "", func(t *testutil.T) {
		t.Override(&os.Args, []string{"devloop"})
		t.Setenv("DEVLOOP_CMDLINE", "unknown")
...
func TestMain_InvalidUsageExitCode(t *testing.T) {
	...
	testutil.Run(t, "unknown flag", func(t *testutil.T) {
		// --interactive=false removes the update check and survey prompt.
		t.Override(&os.Args, []string{"devloop", "--help2", "--interactive=false"})
		err := Run(io.Discard, io.Discard)
		t.CheckErrorAndExitCode(127, err)
	})
	testutil.Run(t, "exactargs error", func(t *testutil.T) {
		// --interactive=false removes the update check and survey prompt.
		t.Override(&os.Args, []string{"devloop", "config", "set", "a", "b", "c"})
		err := Run(io.Discard, io.Discard)
		t.CheckErrorAndExitCode(127, err)
	})
}
...
func TestMain_SuppressedErrorReporing(t *testing.T) {
	testutil.Run(t, "inspect should suppress error output", func(t *testutil.T) {
		...
		t.Override(&os.Args, []string{"devloop", "inspect", "build-env", "list", "--profile", "non-existent"})
		err := Run(&output, &errOutput)
		t.CheckError(true, err)
	...
	testutil.Run(t, "diagnose should report error output", func(t *testutil.T) {
		...
		// non-existent profile should report an error
		t.Override(&os.Args, []string{"devloop", "diagnose", "--yaml-only", "--profile", "non-existent"})
		err := Run(&output, &errOutput)
		t.CheckError(true, err)
		t.CheckEmpty(output.String())
		// checking quoted filename ensures there are no JSON errors too
		t.CheckContains(`unable to find configuration file "devloop.yaml"`, errOutput.String())
	})
}
