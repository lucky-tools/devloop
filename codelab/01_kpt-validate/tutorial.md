# Validate Your Configurations in devloop Workflow
...
*  You will get an hands-on off-the-shelf experience about the **GitOps** CI/CD workflow in devloop.
...
If you are new to devloop, you can check out [the devloop tutorials](https://devloop.dev/docs/tutorials/) to get a basic idea. 
Or just follow this codelab, we will explain what happens in each step.

#### Install `devloop`

*   Check if `devloop` is installed and the installed version is >= v1.17.0
    ```bash
    devloop version
    ```

*   If you haven't installed `devloop` previously, run
    ```bash
    curl -Lo devloop https://storage.googleapis.com/devloop/builds/latest/devloop-linux-amd64 && \
    sudo install devloop /usr/bin/ | bash
    ```  
*   To upgrade devloop to a newer version, run
    ```bash
    sudo rm -f /usr/bin/devloop && curl -Lo devloop https://storage.googleapis.com/devloop/builds/latest/devloop-linux-amd64 && \
    sudo install devloop /usr/bin/ | bash
    ```
...
*   Now let's use `kpt pkg` to download the application example. 
    ...
    ```bash
    kpt pkg get https://github.com/lucky-tools/devloop.git/codelab/01_kpt-validate/resources/sample-app guestbook-cl && cd guestbook-cl
    ```
...
## Taking a deeper look at the devloop.yaml

Time to complete: **About 3 minutes**


The example has already configured the <walkthrough-editor-open-file filePath="guestbook-cl/devloop.yaml">devloop.yaml</walkthrough-editor-open-file> 
for you. 
...
- <walkthrough-editor-select-line filePath="guestbook-cl/devloop.yaml" startLine="4" endLine="4" startCharacterOffset="0" endCharacterOffset="5">build</walkthrough-editor-select-line> defines the methods to **build** and **upload** the application images (by default, it uses docker)
- <walkthrough-editor-select-line filePath="guestbook-cl/devloop.yaml" startLine="8" endLine="8" startCharacterOffset="0" endCharacterOffset="6">deploy</walkthrough-editor-select-line> defines the methods to **manage** the app configuration and **deploy** the application to the bundled cluster.
        
In the example, the configurations will build an image <walkthrough-editor-select-line filePath="guestbook-cl/devloop.yaml" startLine="6" endLine="6" startCharacterOffset="12" endCharacterOffset="20">frontend</walkthrough-editor-select-line> and use `kpt` to *hydrate[1]* 
and deploy the applications to the cluster. 

The <walkthrough-editor-select-line filePath="guestbook-cl/devloop.yaml" startLine="6" endLine="6" startCharacterOffset="12" endCharacterOffset="20">frontend</walkthrough-editor-select-line> source code is stored in 
...
`devloop dev` is the essential devloop command. It builds the application and then deploys 
...
> `devloop dev` can automatically detect file changes and kick off a re-deploy.
...
*   Download the kpt function resource
    ...
    ```bash
    kpt pkg get https://github.com/lucky-tools/devloop.git/codelab/01_kpt-validate/resources/validation-kubeval validation-kubeval
    ```
    
*   Now let's update the devloop.yaml to use the new validator. Replace the following code with the 
`.deploy` section in the <walkthrough-editor-select-line filePath="guestbook-cl/devloop.yaml" startLine="8" endLine="10" startCharacterOffset="0" endCharacterOffset="100">devloop.yaml</walkthrough-editor-select-line>

    See the full [devloop.yaml](https://github.com/yuwenma/sample-app/blob/kubeval/devloop.yaml#L12) 
    ```yaml
    deploy:
      kpt:
        dir: config
...
-  <walkthrough-editor-select-line filePath="guestbook-cl/devloop.yaml" startLine="12" endLine="12" startCharacterOffset="6" endCharacterOffset="12">.deploy.kpt.fn.fnPath</walkthrough-editor-select-line> refers to the kpt function directory we just downloaded. 
-  <walkthrough-editor-select-line filePath="guestbook-cl/devloop.yaml" startLine="13" endLine="13" startCharacterOffset="6" endCharacterOffset="13">.deploy.kpt.fn.network</walkthrough-editor-select-line> enables the kpt access to the network. This is required to run the function in a docker container.
...
Let's run `devloop dev`. Now it will validate the resource according to the functions in 
<walkthrough-editor-select-line filePath="guestbook-cl/devloop.yaml" startLine="14" endLine="14" startCharacterOffset="7" endCharacterOffset="13">`fnPath`, 
...
Congratulations, you known how to validate configurations in devloop! You can explore the full kpt configuration 
from the devloop.yaml [reference doc](https://devloop.dev/docs/references/yaml/#deploy-kpt). 
