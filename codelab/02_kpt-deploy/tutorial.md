...
This codelab is the second session. Check out the [01_kpt-validate](https://github.com/lucky-tools/devloop/tree/main/codelab/01_kpt-validate) about how to use kpt to validate your configuration before deploying to the cluster.
...
*   If you haven't installed `devloop` previously, run
    ...
    sudo install devloop /usr/local/bin/ | bash
    ```  
*   To upgrade devloop to a newer version, run
    ```bash
    sudo rm -f /usr/local/bin/devloop && curl -Lo devloop https://storage.googleapis.com/devloop/builds/latest/devloop-linux-amd64 && \
    sudo install devloop /usr/local/bin/ | bash
    ```
...
*   Now let's use `kpt pkg` to download the application example. 
    ...
    ```bash
    kpt pkg get https://github.com/lucky-tools/devloop.git/codelab/02_kpt-deploy/resources/sample-app guestbook-cl2 && cd guestbook-cl2
    ```

<walkthrough-pin-section-icon></walkthrough-pin-section-icon>
**Tips**
>  If you haven't used `devloop dev`, you can go through this [codelab](https://github.com/lucky-tools/devloop/tree/main/codelab/01_kpt-validate).

## Deploy
Time to complete: **About 2 minutes**

`Devloop` supports using `kpt` to deploy your configurations to the cluster.
...
*  To enable the `kubectl` pruning, you can configure the <walkthrough-editor-open-file filePath="guestbook-cl2/devloop.yaml">devloop.yaml</walkthrough-editor-open-file>, and replace the following code in the <walkthrough-editor-select-line filePath="guestbook-cl2/devloop.yaml" startLine="8" endLine="10" startCharacterOffset="0" endCharacterOffset="100">deploy</walkthrough-editor-select-line> section.
...
*   Run `devloop dev` to deploy the resources.
    
    ```bash
    devloop dev 
    ```

*   Now, let's open a new <walkthrough-editor-spotlight spotlightId="menu-terminal-new-terminal">terminal</walkthrough-editor-spotlight> and run the following command in the terminal. This will move the <walkthrough-editor-open-file filePath="guestbook-cl2/config/frontend/deployment.yaml">deployment.yaml</walkthrough-editor-open-file> out of the <walkthrough-editor-select-line filePath="guestbook-cl2/devloop.yaml" startLine="16" endLine="16" startCharacterOffset="5" endCharacterOffset="28">config/frontend</walkthrough-editor-select-line>, meaning the  `Deployment` resource *should be pruned*.
...
**Note**
> Do not exit the `devloop dev` in the first cloud 
...
*   First, you can exit `devloop dev` with *Ctrl+ C*
...
*   In <walkthrough-editor-open-file filePath="guestbook-cl2/devloop.yaml">devloop.yaml</walkthrough-editor-open-file>, replace the <walkthrough-editor-select-line filePath="guestbook-cl2/devloop.yaml" startLine="8" endLine="16" startCharacterOffset="0" endCharacterOffset="28">deploy</walkthrough-editor-select-line> with the following content. 
...
Congratulations, you know how to use kpt in devloop! You can explore other kpt features 
...
in the devloop soon. Stay tuned!
