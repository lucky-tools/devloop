...
In order to promote good PR quality, the devloop team will push back
against big PRs. See [0](https://github.com/lucky-tools/devloop/pull/2750#pullrequestreview-283621241), [1](https://github.com/lucky-tools/devloop/pull/2917#pullrequestreview-291415741)
...
Adding a new command or devloop config results in large number of changes due to
...
A contributor wanted to support [custom build args to `kustomize` deployer](https://github.com/lucky-tools/devloop/issues/2488).
This required devloop to add the `deploy.kustomize.buildArgs` field and then plumb the arguments to the `kustomize` command.
...
1. Introduce a [place holder for a new config](https://github.com/lucky-tools/devloop/pull/2870).
...
2. [Add a field to devloop config](https://github.com/lucky-tools/devloop/pull/2871) and pass it to the place holder.
...
Below is an example on how you can introduce a new functionality to devloop in smaller incremental PRs.

To add ability to [render templated k8 manifests without deploying](https://github.com/lucky-tools/devloop/issues/1187), required us to make a bunch of changes in devloop
1. Add this feature to `kubectl`, `helm` and `kustomize` deployers supported by devloop.
2. support a new flag for `devloop deploy` and `devloop run` command
...
1. First added an [method to interface `deployer` with empty stub](https://github.com/lucky-tools/devloop/pull/2834).
   
   Note: The code added in this PR does not get exercised other than in tests.
2. Then add render implementation for each [kubectl deployer](https://github.com/lucky-tools/devloop/pull/2943), kustomize and helm.
3. Add a [render command](https://github.com/lucky-tools/devloop/pull/2942)

Remember, its ok to land code which is not exercised. However please mention the follow up work
in **Next PRs** section when creating a Pull Request [e.g. here.](https://github.com/lucky-tools/devloop/pull/2811)
...
See for example [#2917](https://github.com/lucky-tools/devloop/pull/2917).
