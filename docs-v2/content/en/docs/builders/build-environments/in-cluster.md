...
Devloop supports building in cluster via [Kaniko]({{< relref "/docs/builders/builder-types/docker#dockerfile-in-cluster-with-kaniko" >}}) 
or [Custom Build Script]({{<relref "/docs/builders/builder-types/custom#custom-build-script-in-cluster" >}}).

## Configuration

To configure in-cluster Build, add build type `cluster` to the build section of `devloop.yaml`. 
...
Devloop can build multiple artifacts in parallel, by settings a value higher than `1` to `concurrency`.
