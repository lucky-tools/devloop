...
Devloop will use your locally-installed build tools (such as Docker, Maven or Gradle) to execute the build.

## Configuration

To configure the local execution explicitly, add build type `local` to the build section of `devloop.yaml`
...
Devloop defaults `push` to `false` to speed up builds.  The `push`
...
Devloop disables concurrency by default for local builds as several
...
`tryImportMissing: true` causes Devloop to avoid building an image when
