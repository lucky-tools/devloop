...
Devloop supports building with Cloud Native Buildpacks, requiring only
a local Docker daemon. Devloop performs the build inside a container
...
{{<alert title="Note">}}
In Devloop 1.11 (schema `v2beta5`), the singular `buildpack` field was renamed to `buildpacks`.
{{</alert>}}
...
`builder` is *required* and tells Devloop which
...
`dependencies` tells the devloop file watcher which files should be watched to
