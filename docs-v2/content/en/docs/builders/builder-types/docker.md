...
Devloop supports building with Dockerfile
...
installed, Devloop can be configured to build artifacts with the local
Docker daemon.

By default, Devloop connects to the local Docker daemon using
...
of `devloop.yaml`. The following options can optionally be configured:
...
Docker image `gcr.io/k8s-devloop/example` with the local Docker daemon:
...
Devloop can help build artifacts in a Kubernetes cluster using the Kaniko
...
`devloop.yaml`. The following options can optionally be configured:
...
The following `build` section, instructs Devloop to build a
Docker image `gcr.io/k8s-devloop/example` with Kaniko:
