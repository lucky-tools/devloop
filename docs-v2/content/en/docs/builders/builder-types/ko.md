...
Devloop embeds `ko` as a library, so you do not need to download `ko`
...
If you run Devloop in a Google Cloud VPC network with limited or no internet
...
```yaml
    ko:
      labels:
        ...
        org.opencontainers.image.source: https://github.com/lucky-tools/devloop
```
...
The `dependencies` section configures what files Devloop should watch for
...
Any patterns in `ignore` will be ignored by the Devloop file watcher, even if
...
These templates are evaluated by Devloop. Note that the syntax is slightly
different to
...
- Specify your destination image registry using Devloop's
...
- Image naming follows the
  [Devloop image naming strategy]({{< relref "/docs/environment/image-registries" >}}).
  Devloop removes the `ko://` prefix, if present, before determining the image
  name.
...
- The ko builder supports reading
  ...
  artifact in `devloop.yaml`.

- The ko builder supports reading
  [build configs](https://ko.build/configuration/#overriding-go-build-settings)
  from the `.ko.yaml` file if `devloop.yaml` does not specify any of the build
...
- You can generate `devloop.yaml` files by examining an existing code base,
  using the command
  [`devloop init --XXenableKoInit=true`]({{< relref "/docs/init" >}}).
  Select the Ko builder for your images when prompted.

### `ko` commands and workflows in Devloop

Here are some examples of Devloop equivalents of `ko` commands and worflows.
...
To achieve the same using Devloop's ko builder, use the `flags` field in
`devloop.yaml`:
...
If you want Devloop to print out the full image name and digest (and nothing
else) to `stdout`, similar to what `ko build` does, use the
[`--quiet` and `--output` flags]({{< relref "/docs/references/cli#devloop-build" >}}).
These flags enable you to capture the full image references in an environment
variable or redirect to a file, e.g.:

```shell
devloop build --quiet --output='{{range .Builds}}{{.Tag}}{{end}}' > out.txt
```

Note that Devloop produces a JSON file with the image names if you run
`devloop build` with the `--file-output` flag. You can then use this flag as
input to `devloop render` to render Kubernetes manifests. For details on how
to do this, see the next section.

#### Rendering Kubernetes manifests

When you use the Devloop ko builder, Devloop takes care of replacing the
...
Devloop works with existing Kubernetes manifest files that use this image name
placeholder format. Note that Devloop only replaces image references in fields
...
You can instead use Devloop's
[`render` subcommand]({{< relref "/docs/references/cli#devloop-render" >}})
with the `--digest-source local` flag to build and render manifests:

```shell
devloop render --digest-source local --offline --output out.yaml
```
...
```shell
devloop build --file-output artifacts.json --push
devloop render --build-artifacts artifacts.json --digest-source none --offline --output out.yaml
```

Specify the location of your Kubernetes manifests in `devloop.yaml`:
...
To build images in parallel, consider setting the `DEVLOOP_BUILD_CONCURRENCY`
environment variable value to `0`:

```shell
DEVLOOP_BUILD_CONCURRENCY=0 devloop [...]
```

You can also set the concurrency value in your `devloop.yaml`:
...
[`devloop debug`]({{< relref "/docs/references/cli#devloop-debug" >}})
...
Devloop configures `ko` to build with compiler optimizations and inlining
disabled (`-gcflags='all=-N -l'`) when you run `devloop debug` or use
Cloud Code to
...
To learn more about how Devloop debugs Go applications, read the
...
`devloop.yaml` file to specify a base image that contains the `tar` command,
...
`devloop dev`, such as in this example:
...
[example](https://github.com/lucky-tools/devloop/tree/main/examples/custom).
...
[issue](https://github.com/lucky-tools/devloop/issues/new)
...
uploads it to the image registry. The Devloop ko builder does not generate
or upload SBOMs. If you need this feature, please
[raise an issue](https://github.com/lucky-tools/devloop/issues).
