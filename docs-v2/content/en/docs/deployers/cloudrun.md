...
{{< alert title="Note" >}}
This feature is currently experimental and subject to change. Not all Devloop features are supported, for example `debug` is currently not supported in Cloud Run (but is on our roadmap).
{{< /alert >}}

[Cloud Run](https://cloud.google.com/run) is a managed compute platform on Google Cloud that allows you to run containers on Google's infrastructure. With Devloop, now you are able to configure your dev loop to build, test, sync and use Cloud Run as the deployer for your images.


## Deploying applications to Cloud Run
Devloop can deploy [Services](https://cloud.google.com/run/docs/reference/rest/v1/namespaces.services#resource:-service) and [Jobs](https://cloud.google.com/run/docs/reference/rest/v1/namespaces.jobs#resource:-job) to Cloud Run. If this deployer is used, all provided manifests must be valid Cloud Run services, using the `serving.knative.dev/v1` schema, or valid Cloud Run jobs.
...
With Devloop you can deploy Cloud Run [Services](https://cloud.google.com/run/docs/overview/what-is-cloud-run#services) and [Jobs](https://cloud.google.com/run/docs/overview/what-is-cloud-run#jobs) just referencing them from the `devloop.yaml` file. The following example ilustrates a project using the Cloud Run deployer:
...
kind: Service
metadata:
  name: cloud-run-service-name # this service will be created in Cloud Run via Devloop
spec:
  template:
    spec:
      containers:
...
kind: Job
metadata:
  name: cloud-run-job-name # this job will be created in Cloud Run via Devloop
  annotations:
...
`devloop.yaml` content:
{{% readfile file="samples/deployers/cloud-run/simple-service-job-deployment.yaml" %}}

Running `devloop run` will deploy one Cloud Run service, and one Cloud Run job in the `YOUR-GCP-PROJECT` project, inside the given `GCP-REGION`.
...
Devloop will manage automatically the necessary configuration to open the deployed Cloud Run services URLs locally, even if they are private services, using the [Cloud Run proxy](https://cloud.google.com/sdk/gcloud/reference/beta/run/services/proxy) and Devloop's Port Forwarding. To enable this, you will have to either add the `--port-forward` flag running Devloop, or add a `portForward` stanza in your `devloop.yaml` file. From the previous example, running `devloop dev --port-forward` will result in the following output:
...
Using the previous example, changing `devloop.yaml` to:
{{% readfile file="samples/deployers/cloud-run/service-port-forward.yaml" %}}

Running `devloop dev --port-forward`, will result in:
...
When doing [local development]({{< relref "docs/workflows/dev">}}), Devloop will log stream to your console the output from the Cloud Run services deployed. From the previous example, running `devloop dev --port-forward` or `devloop run --tail --port-forward` in your terminal, you will see the following output:
...
Now Devloop is log streaming the output from the service. If you access it through `localhost:9001`, you'll see the logs:
...
To deploy to Cloud Run, use the `cloudrun` type in the `deploy` section, together with `manifests.rawYaml` stanza of `devloop.yaml`.
...
The following `deploy` section instructs Devloop to deploy the artifacts under `manifests.rawYaml` to Cloud Run:

{{% readfile file="samples/deployers/cloud-run/cloud-run.yaml" %}}

## Cloud Run deployer + Devloop Local build

With Devloop you can configure your project to [locally build]({{< relref "docs/builders/build-environments/local" >}}) your images and deploy them to Cloud Run. The following example demonstrates how to set up Devloop for this:
...
spec:
  template:
    spec:
      containers:
      - image: my-img # <- Same image name from devloop.yaml file
```
...
Running a Devloop command like `devloop run --default-repo=gcr.io/your-registry` will build your local images, push them to the specified registry, and deploy them to Cloud Run. Please notice the following from the previous example:

### Build local push option
When you use [Devloop Local build]({{< relref "docs/builders/build-environments/local#avoiding-pushes" >}}), the `push` option is set to `false` by default. However, Cloud Run will need your images published in a registry that it has access to. Therefore, we need to set this to `true`.

### Platform
According to the [Cloud Run runtime contract](https://cloud.google.com/run/docs/container-contract#languages), your images must be compiled for a specific architecture. Devloop can help us with this by using its [Cross/multi-platform build support]({{< relref "docs/builders/cross-platform" >}}).

### Registry
You'll need to specify a registry so Devloop can push your images. For this, we can use the `--default-repo` flag when running a command to include it in all your local images.
