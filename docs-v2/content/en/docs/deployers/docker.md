...
Kubernetes resource types, Devloop can "deploy" these applications
...
in the `deploy` section of your `devloop.yaml`.
...
{{< alert title="Note" >}}
Images listed to be deployed with the `docker` deployer **must also have a corresponding build artifact built by Devloop.**
{{< /alert >}}
