...
Some context specific settings can be configured in a global configuration file, which defaults to `~/.devloop/config`. Options can be configured globally or for specific Kubernetes contexts. Context name matching supports regex, e.g.: `.*-cluster.*-regex.*-test.*`
...
| `update-check` | boolean | Check for a more recent version of Devloop. |
...
```
This will create a global configuration file at `~/.devloop/config` with `local-cluster` set to `true`.
