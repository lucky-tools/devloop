...
Devloop supports fast deployments to supported locally-hosted clusters,
...
Devloop's heuristic to detect local clusters is based on the Kubernetes context name
...
Devloop checks for the following context names:
...
For any other name, Devloop assumes that the cluster is remote and that images
have to be pushed.
...
1. Ensure that Devloop builds the images with the same docker daemon that runs the pods' containers.
1. Tell Devloop to skip pushing images either by configuring
...
1. Set up the docker environment for Devloop with `source <(minikube docker-env -p my-profile)`.
   This should set some environment variables for docker (check with `env | grep DOCKER`).
   **It is important to do this in the same shell where Devloop is executed.**

2. Tell Devloop that the Kubernetes context `my-profile` refers to a local cluster with

    ```bash
    devloop config set --kube-context my-profile local-cluster true
    ```
...
that may be running on the host.  Devloop automatically uses
...
```shell
# build the image `devloop-example`
$ devloop build
...
Starting build...
Found [minikube] context, using local docker daemon.
...
Successfully tagged devloop-example:v1.35.0-37-g7ccebe58e
Build [devloop-example] succeeded

# but the image is not found in the host docker!
$ docker images devloop-example
...
...
$ eval $(minikube docker-env) && docker images devloop-example
REPOSITORY         TAG                                                                IMAGE ID       CREATED       SIZE
devloop-example   160fe3a3c1358ef7b3fbfd1ae19fc8c5ac096635c39171e22ad1e5242b6ad8fd   160fe3a3c135   3 weeks ago   7.43MB
devloop-example   v1.35.0-37-g7ccebe58e                                              160fe3a3c135   3 weeks ago   7.43MB
```
...
Devloop's direct loading of images into a local cluster does mean that resources specifying
