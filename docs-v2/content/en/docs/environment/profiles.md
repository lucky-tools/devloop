...
Devloop profiles allow you to define build, test and deployment
...
You can create profiles in the `profiles` section of `devloop.yaml`.
...
in the main section of `devloop.yaml`. The `build`, `test` and `deploy` configuration in the `profiles`
section use the same syntax as the `build`, `test` and `deploy` sections of
`devloop.yaml`; for more information, see [Builders]({{< relref "/docs/builders" >}}),
[Testers](/docs/testers), [Deployers]({{< relref "/docs/deployers" >}}) and you can always refer to
 [devloop.yaml reference]({{< relref "/docs/references/yaml" >}}) for an overview of the syntax.
 Alternatively, you can override the main configuration with finer grained control using `patches`.


### Activation

You can activate a profile two ways: CLI flag or devloop.yaml activations.

**CLI flag**: You can activate profiles with the `-p` (`--profile`) parameter in the
`devloop dev` and `devloop run` commands.
  ```bash
  devloop run -p [PROFILE]
  ```

**Activations in devloop.yaml**: You can auto-activate a profile based on

* kubecontext (could be either a string or a regexp: prefixing with `!` will negate the match)
* environment variable value
* devloop command (dev/run/build/deploy)
 ...
 * `profile2` is activated if `MAGIC_VAR` is 1337 or we are running `devloop dev` while kubecontext is set to `minikube`.
...
```bash
devloop dev -p hello,world
```

Devloop will activate both profiles, `hello` and `world`. 
...
Devloop will activate the `hello` profile, and deactivate the `world` profile, even if it had otherwise been activated through the configuration.
