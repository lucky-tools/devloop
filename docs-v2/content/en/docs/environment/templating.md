...
Devloop allows for certain fields in the config to be templated via the [Go `text/template` package](https://pkg.go.dev/text/template).
Environment variables and certain special values computed by Devloop (see below) are available in the templating
...
in the standard library are complemented in Devloop by the  [Sprig template function library](http://masterminds.github.io/sprig/).
...
* all environment variables passed to the Devloop process at startup
...
In addition to the functions listed above, Devloop locally provides the following:
