...
******
-->
This is a generated reference for the [Devloop API]({{<relref "/docs/design/api">}}) gRPC layer.

We also generate the [reference doc for the HTTP layer]({{<relref "/docs/references/api/swagger">}}).



<a name="v2/devloop.proto"></a>

## v2/devloop.proto

You can find the source for v2/devloop.proto [on Github](https://github.com/lucky-tools/devloop/blob/main/proto/v2/devloop.proto).



### Services

<a name="proto.v2.DevloopV2Service"></a>
...
| ----------- | ------------ | ------------- | ------------|
| GetState | [.google.protobuf.Empty](#google.protobuf.Empty) | [State](#proto.v2.State) | Returns the state of the current Devloop execution |
| Events | [.google.protobuf.Empty](#google.protobuf.Empty) | [Event](#proto.v2.Event) stream | Returns all the events of the current Devloop execution from the start |
| ApplicationLogs | [.google.protobuf.Empty](#google.protobuf.Empty) | [Event](#proto.v2.Event) stream | Returns all the user application logs of the current Devloop execution |
...
| richFormattedMessage | [string](#string) |  | full message that devloop writes, with format and color |
...
| context | [string](#string) |  | devloop.yaml context field |
| dockerfile | [string](#string) |  | devloop.yaml path to dockerfile. Not guaranteed to be filled |
...
#### BuildState
`BuildState` maps Devloop artifacts to their current build states


| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| artifacts | [BuildState.ArtifactsEntry](#proto.v2.BuildState.ArtifactsEntry) | repeated | A map of `artifact name -> build-state`. Artifact name is defined in the `devloop.yaml`. The `build-state` can be: <br> - `"NotStarted"`: not yet started <br> - `"InProgress"`: build started <br> - `"Complete"`: build succeeded <br> - `"Failed"`: build failed |
...
#### BuildSubtaskEvent
`BuildSubtaskvent` describes the build status per artifact, and will be emitted by Devloop anytime a build starts or finishes, successfully or not.
If the build fails, an error will be attached to the event.


| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| id | [string](#string) |  | id of the subtask which will be used in DevloopLog |
| task_id | [string](#string) |  | id of the task of devloop that this event came from |
...
#### DeploySubtaskEvent
`DeploySubtaskEvent` represents the status of a deployment, and is emitted by Devloop
...
<a name="proto.v2.DevloopLogEvent"></a>
#### DevloopLogEvent
`DevloopLogEvent` represents a piece of output that comes from a devloop run, for example: "Generating tags...", "Step 1/3 : FROM gcr.io/distroless/base"


| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| task_id | [string](#string) |  | id of the task of devloop that this log came from |
| subtask_id | [string](#string) |  | id of the subtask that the log came from |
| level | [proto.enums.LogLevel](#proto.enums.LogLevel) |  | string origin = 3; // REMOVED: which tool the output came from ex: devloop, docker
...
#### Event
`Event` describes an event in the Devloop process.
...
| metaEvent | [MetaEvent](#proto.v2.MetaEvent) |  | contains general information regarding Devloop like version info |
| devloopLogEvent | [DevloopLogEvent](#proto.v2.DevloopLogEvent) |  | describes a log that comes from a devloop phase |
...
| taskEvent | [TaskEvent](#proto.v2.TaskEvent) |  | describes the start, end, or failure of a devloop phase |
...
| terminationEvent | [TerminationEvent](#proto.v2.TerminationEvent) |  | describes a devloop termination event |
...
#### ExecSubtaskEvent
`ExecSubtaskEvent` represents the status of an exec, and is emitted by Devloop
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| build | [bool](#bool) |  | in case devloop dev is ran with autoBuild=false, a build intent enables building once |
| sync | [bool](#bool) |  | in case devloop dev is ran with autoSync=false, a sync intent enables file sync once |
| deploy | [bool](#bool) |  | in case devloop dev is ran with autoDeploy=false, a deploy intent enables deploys once |
| devloop | [bool](#bool) |  | in case devloop dev is ran with autoDeploy=false, autoSync=false and autoBuild=false a devloop intent enables the entire dev loop once |
...
#### MetaEvent
`MetaEvent` provides general information regarding Devloop


| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| entry | [string](#string) |  | entry, for example: `"Starting Devloop: {Version:v0.39.0-16-g5bb7c9e0 ConfigVersion:devloop/v1 GitVersion: GitCommit:5bb7c9e078e4d522a5ffc42a2f1274fd17d75902 GitTreeState:dirty BuildDate01:29Z GoVersion:go1.13rc1 Compiler:gc Platform:linux/amd64}"` |
| metadata | [Metadata](#proto.v2.Metadata) |  | Metadata describing devloop pipeline |
...
#### RenderSubtaskEvent
`RenderSubtaskEvent` represents the status of a render, and is emitted by Devloop
...
#### State
`State` represents the current state of the Devloop components
...
Devloop polls for resource status every 0.5 second. If the resource status changes, an event with `status` “Pending”, “Complete” and “Failed”
...
| id | [string](#string) |  | id of the subtask which will be used in DevloopLog |
| task_id | [string](#string) |  | id of the task of devloop that this event came from |
| resource | [string](#string) |  | id of the subtask which will be used in DevloopLog |
| status | [string](#string) |  | id of the task of devloop that this event came from |
...
#### TaskEvent
`TaskEvent` represent the different larger phases of a devloop session, for example Build, Deploy, etc.
If a phase fails, an actionable error will be attached


| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| id | [string](#string) |  | will be used by DevloopLog to link it to a task. Follows the form "{task_name}-{iteration-number}" |
...
#### TerminationEvent
`TerminationEvent` marks the end of the devloop session
...
#### TestSubtaskEvent
`TestSubtaskEvent` represents the status of a test, and is emitted by Devloop
...
#### VerifySubtaskEvent
`VerifyEvent` represents the status of a render, and is emitted by Devloop
anytime a render starts or completes, successfully or not.
...
### BuildType
Enum indicating build type i.e. local vs cluster
...
| INIT_UNKNOWN | 510 | Initialization of the Devloop session failed due to unknown reason(s) |
...
| DEVINIT_REGISTER_CONFIG_DEP | 704 | Failed to configure watcher for Devloop configuration file. |
...
| STATUSCHECK_USER_CANCELLED | 800 | User cancelled the devloop dev run |
...
| INIT_CREATE_TAGGER_ERROR | 901 | Devloop was unable to create the configured tagger |
| INIT_MINIKUBE_PAUSED_ERROR | 902 | Devloop was unable to start as Minikube appears to be paused |
| INIT_MINIKUBE_NOT_RUNNING_ERROR | 903 | Devloop was unable to start as Minikube appears to be stopped |
| INIT_CREATE_BUILDER_ERROR | 904 | Devloop was unable to create a configured image builder |
| INIT_CREATE_DEPLOYER_ERROR | 905 | Devloop was unable to create a configured deployer |
| INIT_CREATE_TEST_DEP_ERROR | 906 | Devloop was unable to create a configured test |
| INIT_CACHE_ERROR | 907 | Devloop encountered an error validating the artifact cache |
| INIT_CREATE_WATCH_TRIGGER_ERROR | 908 | Devloop encountered an error when configuring file watching |
| INIT_CREATE_ARTIFACT_DEP_ERROR | 909 | Devloop encountered an error when evaluating artifact dependencies |
...
| DEPLOY_TRANSFORM_MANIFEST_ERR | 1018 | Error transforming a manifest during devloop debug |
...
| RENDER_TRANSFORM_MANIFEST_ERR | 1406 | Error transforming a manifest during devloop debug |
...
| CONFIG_UNKNOWN_VALIDATOR | 1214 | The validator is not allowed in devloop-managed mode. |
| CONFIG_UNKNOWN_TRANSFORMER | 1215 | The transformer is not allowed in devloop-managed mode. |
...
| CONFIG_UPGRADE_ERR | 1218 | Devloop config version mismatch |
| INSPECT_UNKNOWN_ERR | 1301 | Catch-all `devloop inspect` command error |
...
| FIX_DEVLOOP_CONFIG_DOCKERFILE | 110 | Fix `dockerfile` config for a given artifact and try again. |
...
| SET_CLEANUP_FLAG | 116 | Set Cleanup flag for devloop command. |
...
| FIX_DEVLOOP_CONFIG_HELM_ARTIFACT_OVERRIDES | 205 | Fix helm `releases.artifactOverrides` config to match with `build.artifacts` (no longer used in Devloop v2) |
| UPGRADE_HELM32 | 206 | Upgrade helm version to v3.2.0 and higher. |
| FIX_DEVLOOP_CONFIG_HELM_CREATE_NAMESPACE | 207 | Set `releases.createNamespace` to false. |
...
| CONFIG_FIX_API_VERSION | 707 | Fix config API version or upgrade the devloop binary |
| CONFIG_ALLOWLIST_VALIDATORS | 708 | Only the allow listed validators are acceptable in devloop-managed mode. |
| CONFIG_ALLOWLIST_transformers | 709 | Only the allow listed transformers are acceptable in devloop-managed mode. |
...
| CONFIG_FIX_DEVLOOP_CONFIG_VERSION | 712 | Upgrade devloop config version to latest |
