...
This document sets out the deprecation policy for Devloop, and outlines how the Devloop project will approach the introduction of breaking changes over time.
...
- **Breaking config change**:  In case of Devloop's pipeline config (devloop.yaml) a breaking change between an old and new version occurs when the devloop binary cannot parse the input yaml with auto-upgrade. This can happen when the new version removes a feature or when the new version introduces a mandatory field with no default value
...
1. We document the deprecation in the following places if applicable
    ...
    1. [Release notes](https://github.com/lucky-tools/devloop/blob/main/CHANGELOG.md)
    1. [Command help]({{< relref "/docs/references/cli" >}})
    1. Log messages
    1. [devloop yaml reference]({{< relref "/docs/references/yaml" >}})
...
# Current maturity of devloop

Devloop and its features are considered GA unless specified (in this document, CLI reference, config YAML reference or in docs in devloop.dev).  
Devloop is constantly evolving with the tools space, we want to be able to experiment and sometimes change things. 
...
## Devloop.yaml (pipeline config)

You can safely depend on the devloop config with the assumption that devloop will auto-upgrade to the latest version:
...
Such situations should be discussed on the given bugs / feature requests and during Devloop Office Hours, always bearing in mind that Devloop is committed to being a stable system that, as much as possible, never breaks users. 
Exceptions will always be announced in all relevant release notes.

## Current deprecation notices

04/07/2021: Release v1.22.0 deprecates the `--render-only` and `--render-output` flags from `devloop run` and `devloop dev`, in favor of simply using the `devloop render` command directly.
05/05/2021: Release v1.23.0 deprecates the `--add-devloop-labels` flag from `devloop render`, and implicitly deprecates the addition of Devloop-internal labels to any Kubernetes resources
hydrated by Devloop through `devloop render`.
