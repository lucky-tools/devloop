...
(async function() {
  ...
  if (index === -1) {
    ...
    version = version.replace('devloop/', '');
  } else {
    version = window.location.href.substr(index + versionParam.length);
    table.attributes['data-version'].value = 'devloop/' + version;
  }
  ...
  if (versionInfoResponse.ok) {
    ...
    render(html`
    <strong>Important:</strong> To use this schema, you need Devloop version ${versionInfoJson.binVersion} or later.
...
function* template(definitions, parentDefinition, ref, ident, parent, parentLinks) {
  ...
  for (let [key, definition] of allProperties) {
    ...
    if ((name === 'Profile') && definitions['DevloopConfig'].properties[key]) {
      value = '{}';
      yield html`
        <tr>
          <td>
