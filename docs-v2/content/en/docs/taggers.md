...
weight: 43
featureId: tagpolicy
aliases: [/docs/how-tos/taggers, /docs/pipeline-stages/taggers]
---

Devloop supports multiple taggers or tag policies for tagging images:
...
The default tagger, if none is specified in the `devloop.yaml`, is the `gitCommit` tagger.
...
of the `devloop.yaml` configuration file.
...
### How tagging works
 ...
 + No matter the tagger, Devloop always uses immutable references in Kubernetes manifests.
   Which reference is used depends on whether the images are pushed to a registry or loaded directly into the cluster (such as via the Docker daemon):
     + **When images are pushed**, their immutable digest is available. Devloop then references
     ...
     + **When images are loaded directly into the cluster**, such as loading into the cluster's Docker daemon, digests are not available. We have the tags and the
       imageIDs. Since imageIDs can't be used in Kubernetes manifests, Devloop creates
 ...
 + Devloop never references images just by their tags because those tags are mutable and
...
`gitCommit` is the default tag policy of Devloop: if you do not specify the
`tagPolicy` field in the `build` section, Devloop will use Git information
to tag artifacts.
...
The following `build` section instructs Devloop to build a
Docker image `gcr.io/k8s-devloop/example` with the `gitCommit` tag policy
...
The following `build` section instructs Devloop to build a
Docker image `gcr.io/k8s-devloop/example` with the `inputDigest` tag policy:
...
The following `build` section, for example, instructs Devloop to build a
Docker image `gcr.io/k8s-devloop/example` with the `envTemplate`
tag policy. The tag template is `{{.FOO}}`; when Devloop
...
will be `gcr.io/k8s-devloop/example:v1`.
...
**required** parameter, `template`, which is the tag template to use. To learn more about templating support in Devloop.yaml see [Templated fields]({{< relref "./environment/templating.md" >}})

## `dateTime`: uses data and time values as tags

`dateTime` uses the time when Devloop starts building artifacts as the
tag. You can choose which format and timezone Devloop should use. By default,
Devloop uses the time format `2006-01-02_15-04-05.999_MST` and the local
timezone.

### Example

The following `build` section, for example, instructs Devloop to build a Docker
image `gcr.io/k8s-devloop/example` with the `dateTime`
tag policy:
...
be `gcr.io/k8s-devloop/example:2006-01-02_15-04-05.999_MST`.
...
The following `build` section, for example, instructs Devloop to build a Docker image
`gcr.io/k8s-devloop/example` with the `customTemplate` tag policy.
The tag template is `{{.FOO}}_{{.BAR}}`. The components are a `dateTime` tagger
named `FOO` and a `gitCommit` tagger named `BAR`. When Devloop finishes building the image,
...
Suppose the current time is `15:04:09.999 January 2nd, 2006` and the abbreviated commit sha is `25c65e0`, the image built will be `gcr.io/k8s-devloop/example:2006-01-02_25c65e0`.
...
**required** parameter, `template`, which is the tag template to use. To learn more about templating support in the devloop.yaml, see [Templated fields]({{< relref "./environment/templating.md" >}})

## `sha256`: uses `latest` to tag images

`sha256` is a misleading name. It is named like that because, in the end, when Devloop
...
The following `build` section instructs Devloop to build a
Docker image `gcr.io/k8s-devloop/example` with the `sha256` tag policy:
