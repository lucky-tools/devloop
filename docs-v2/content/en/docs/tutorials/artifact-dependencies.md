...
This tutorial will be based on the [simple-artifact-dependency](https://github.com/lucky-tools/devloop/tree/main/examples/simple-artifact-dependency) example in our repository.
...
This artifact is used as the base image for the `app` artifact. We express this dependency in the `devloop.yaml` using the `requires` expression.
{{% readfile file="samples/builders/artifact-dependencies/devloop.yaml" %}}
...
In the [simple-artifact-dependency](https://github.com/lucky-tools/devloop/tree/main/examples/simple-artifact-dependency) directory, run:

```text
devloop dev
```

If this is the first time you're running this, then it should build the artifacts, starting with `base` and later `app`. Devloop can handle any arbitrary dependency graph between artifacts and schedule builds in the right order. It'll also report an error if it detects dependency cycles or self-loops.
...
Hitting `Ctrl + C` on a running Devloop process should end it and cleanup its deployments. If there are still persisting objects then you can issue `devloop delete` command to attempt the cleanup again.