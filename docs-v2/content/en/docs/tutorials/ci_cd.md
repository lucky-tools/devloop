---
title: "Using Devloop for CI/CD with GitLab"
linkTitle: "Devloop in CI/CD"
weight: 100
---
Contributed by: [@tvvignesh](https://github.com/tvvignesh)

Devloop is a tool which is non-opinionated about the CI/CD tool you should use and acts as a platform agnostic solution for both development and deploying to production.

To facilitate building and deploying images in CI/CD, devloop offers docker images which you can build on top of. If that doesn’t work for your use-case, you can make your own Dockerfile and pull in devloop and its dependencies using curl.

Let us have a look at how we can use Devloop with GitLab CI.
...
Devloop supports multiple builders like Docker, Kaniko and more and if the builder you are looking for is not supported out of the box, you can [script it out](https://devloop.dev/docs/tutorials/custom-builder/) as well. We will use Docker in this demonstration.
...
Devloop can use your dockerfile to automatically build, tag and push images to the registry when needed.

## Step 2: Choosing the deployment method

Devloop supports deployment through kubectl, helm and a lot of other mechanisms. You can have a look at the complete list of deployers available [here](https://devloop.dev/docs/deployers/).
...
Devloop automatically tags images based on different strategies as documented here: [https://devloop.dev/docs/taggers/](https://devloop.dev/docs/taggers/). The tagging strategy used is configurable, so choose the mechanism which is right for you. By default, devloop uses the git sha tagger.

If you are using helm as your deployer, you might want to use the helm image strategy if you would like to follow helm specific conventions, as devloop will pass in all the details for you.

## Step 4: Plugins (optional)

If you want to do some kind of processing before deployment like decrypting the secrets in the CI/CD pipeline, devloop also supports plugins like [Helm Secrets](https://github.com/zendesk/helm-secrets/). So, if you would like to deploy secrets that have been encrypted using KMS or any other mechanism supported by [SOPS](https://github.com/mozilla/sops) you can actually set **useHelmSecrets** option to true and devloop will handle everything automatically for you.

## Step 5: The `devloop.yaml` file

Your `devloop.yaml` file will change depending on the way you would like to build and deploy your project. It is recommended that you set up devloop and run it manually before you setup your CI pipeline since the CI pipeline can just call devloop to do all the builds and deployments which can be tested out locally via devloop.

A sample devloop file can look something like this:
...
kind: Config
profiles:
  # Specify the profile name which you can run later using devloop run -p <profilename>
  - name: my-profile-name
    build:
      artifacts:
        # Devloop will use this as your image name and push it here after building
    ...
    deploy:
      ...
      helm:
        # Pass the parameters according to https://devloop.dev/docs/references/yaml/
        releases:
          - name: my-release
            namespace: default
...
Rather, if you want to deploy via kubectl, your devloop file can look something like this:
              ...
              - image: asia.gcr.io/my-project/my-image
                ...
                docker:
                    ...
                    buildArgs:
                        ...
                        NPM_TOKEN: '{{.NPM_TOKEN}}'
      deploy:
          # In case we enable status check, this will be the timeout till which devloop will wait
          statusCheckDeadlineSeconds: 600
          # Using kubectl as our deployer
          kubectl:
              # Location to our yaml files
              # Refer https://devloop.dev/docs/references/yaml/ for more options
              manifests:
                  - k8/dev/*.yml
```
...
Before we move on to setting up the CI pipeline, we should test it out locally. Use `devloop run` if you are deploying it to the cluster, `devloop build` to just build and push the artifacts, `devloop dev` if you are developing (this will enable auto reload on changes) or debug using `devloop debug`

You can find docs about all these workflows [here](https://devloop.dev/docs/workflows/)

Also, note that you can also set up file synchronization to enable faster development workflows. You can read more about that [here](https://devloop.dev/docs/filesync/).
...
Now that we have Devloop ready and working locally, we look at the CI pipeline.
...
This pipeline might look complicated at first glance, but most of it related to configuring access for deploying to GCP and configure GitLab's Docker-in-Docker support to cache artifacts between builds. What is important to note is that we use Devloop to do all the builds and deployments.
...
my-job:
    ...
    image:
        name: gcr.io/k8s-devloop/devloop:v1.15.0
    ...
    script:
        ...
        - kubectl config get-contexts
          # Pass in all the environment variables to be passed to devloop during build and deployment with all the args as documented in https://devloop.dev/docs/references/cli/ (in our case our cluster context, env vars, namespace and some labels)
        - DOCKER_HOST="$DIND_DEV" NPM_REGISTRY="$NPM_REGISTRY_DEV" NPM_TOKEN="$NPM_TOKEN_DEV" devloop run --kube-context $CLUSTER_DEV_CONTEXT -n default -l devloop.dev/run-id=deploydep -p dev-svc --status-check
    only:
...
So, ultimately our GitLab CI pipeline does not know anything about the helm charts or build strategy or any deployment methods and relies completely on devloop to do the job for us.
...
**NOTE:** If you are running Devloop using the Kubernetes executor, make sure that you are running the runner with appropriate permissions or pod security policies. If you are running using DIND, it requires access to the Docker socket and being in privileged mode which might actually tend to be insecure. An alternative can be to use [kaniko](https://github.com/GoogleContainerTools/kaniko), [buildkit](https://github.com/moby/buildkit) or other custom builders like [Buildah](https://github.com/containers/buildah) for building the image.
...
Hope this post was informative. Good luck with your journey with Devloop.
