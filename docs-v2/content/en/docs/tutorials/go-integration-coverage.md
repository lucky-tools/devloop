...
This tutorial describes how to use Devloop to collect
...
Devloop orchestrates the steps of:
...
For steps 3-5, this tutorial uses Devloop
...
[`go-integration-coverage`](https://github.com/lucky-tools/devloop/tree/main/examples/go-integration-coverage)
example.
...
The following snippet shows how to configure the image build using the Devloop
...
Use Devloop post-deploy hooks to run the tests automatically after deploying
the application. These hooks can run either on the
[`host`]({{<relref "/docs/lifecycle-hooks#before-deploy-and-after-deploy" >}})
where you run Devloop, or in the deployed
...
files from the pods to the host where you run Devloop:
...
First, the shell script below locates all pods deployed by the Devloop run
using a selector on the
[`devloop.dev/run-id` label]({{<relref "/docs/tutorials/devloop-resource-selector" >}}).
...
the host where you run Devloop.

Finally, the `go tool covdata` command reports the coverage as percentage on
the terminal.

Devloop provides the
[`DEVLOOP_KUBE_CONTEXT` and `DEVLOOP_RUN_ID` environment variables]({{<relref "/docs/lifecycle-hooks#environment-variables" >}})
to the shell script.
...
Devloop [profiles](https://devloop.dev/docs/environment/profiles/) enable
different configurations for different contexts.

The `devloop.yaml` file for this tutorial contains a `coverage` profile that
...
To activate the profile, add the flag `--profile coverage` to Devloop
commands.

## Running the steps

To run the steps, follow the instructions in the
[README.md](https://github.com/lucky-tools/devloop/tree/main/examples/go-integration-coverage).
