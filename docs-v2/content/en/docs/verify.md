...
Devloop `v2.0.0`+ supports running post-deployment verification tests. You define these tests as a list of test containers that are either standalone or built by Devloop. Devloop runs these containers after the [deploy]({{< relref "/docs/deployers/" >}}) stage and monitors them for success or failure.

You can configure and execute post-deployment verification tests using the [`verify` command]({{< relref "/docs/references/cli/#devloop-verify" >}}) and associated [`devloop.yaml` schema configuration]({{< relref "/docs/references/yaml#verify" >}}).
...
When Devloop runs a post-deployment verifications test in the local execution mode, it uses the `docker` CLI to run the test container on the host machine. This is the default execution mode.

### Kubernetes cluster

When Devloop runs a post-deployment verification test in the Kubernetes cluster execution mode, it uses the `kubectl` CLI to run the test container as a Kubernetes Job.

There are two ways to optionally customize the Devloop-generated Kubernetes Job:

* To selectively overwrite the configuration of the Devloop-generated Kubernetes Job with inline JSON, use the `overrides` configuration option. This is similar to the `--overrides` option provided by `kubectl run`.
* To use your own Kubernetes Job manifest and have Devloop replace the containers with those specified in the `containers` stanza of your `verify` configuration, use the `jobManifestPath` configuration option.

## Examples

Below is an example of a `devloop.yaml` file with a `verify` configuration that runs three successful verification tests against deployments:
...
Running `devloop verify` against this `devloop.yaml` (and associated Dockerfiles where relevant) yields:
``` console
$ devloop verify -a build.artifacts 
...
0
```
and `devloop verify` will exit with error code `0`
...
1
```
and `devloop verify` will exit with error code `1`