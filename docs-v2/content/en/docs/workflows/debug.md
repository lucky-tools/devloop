...
Devloop lets you set breakpoints and step through your application, even when deployed to remote Kubernetes clusters, as if the code were running locally on your machine. Devloop detects the language runtimes of your container images, reconfigures the pods for debugging, forwards debugging ports, and then monitors for when debuggable containers come online.  IDE integrations like Cloud Code leverage Devloop's events to automatically set up debugging sessions.
...
Devloop can usually detect the correct language runtime if present. However if you encounter difficulties then checkout the [Supported Language Runtimes]({{< relref "#supported-language-runtimes">}}) section for the exact heuristics that Devloop uses and you can modify your application accordingly, or read about [how you can manually configure your container image]({{< relref "#can-images-be-debugged-without-the-runtime-support-images" >}}).

## (Recommended) Debugging using Cloud Code

The easiest way to debug using Devloop is by using the [Cloud Code IDE extension]({{< relref "../install/#managed-ide">}}) for Visual Studio Code, JetBrains IDEs, and Cloud Shell. Cloud Code will automatically configure container images for debugging so you can debug Kubernetes services just like how you would debug a local service in your IDE.
...
This section describes how Devloop configures the container image for debugging for the supported language runtimes, and how to setup with certain IDEs.
...
##### Devloop debug using the JetBrains GoLand and IntelliJ Ultimate
...
##### Devloop debug using the VS Code Go extension
...
`context` directory in `devloop.yaml`. You can represent this using the
...
are
[often used in multi-stage builds](https://github.com/lucky-tools/devloop/tree/main/examples/getting-started/Dockerfile),
...
<tt>--inspect</tt> arguments.  Devloop introduces a <tt>node</tt> wrapper that
...
{{< alert title="Note" >}}
As many Python web frameworks use launcher scripts, like `gunicorn`, Devloop now uses
...
The language runtime for the artifacts can be specified in the `devloop.yaml` file.
{{% readfile file="samples/debug/runtime-type.yaml" %}}

Otherwise Devloop `debug` recognizes the language runtime used in a
...
environment variables to allow Devloop to detect your container as Go.
...
statement reordering and inlining. Devloop configures Docker builds with a
`DEVLOOP_GO_GCFLAGS` build argument that you should use in your `Dockerfile`:
...
`devloop debug` acts like `devloop dev`, but it configures containers in pods
...
local machine.
One notable difference from `devloop dev` is that `debug` disables image rebuilding and
...
flags, or triggering a [devloop iteration]({{< relref "/docs/design/api#control-api" >}}) using the Devloop API.

Enabling debugging has two phases:

1. **Configuring:** Devloop automatically examines each built container image and
   attempts to recognize the underlying language runtime.  Container images can be
   explicitly configured too.
3. **Monitoring:** Devloop watches the cluster to detect when debuggable containers
   start execution.

### Configuring container images for debugging

`devloop debug` examines the *built artifacts* to determine the underlying language runtime technology.
...
`gcr.io/k8s-devloop/devloop-debug-support`; alternative locations can be
specified in [Devloop's global configuration]({{< relref "/docs/design/global-config.md" >}}).

For images that are successfully recognized, Devloop adds a `debug.cloud.google.com/config`
...
`debug.cloud.google.com/config` annotation.  For each new debuggable pod,  Devloop emits
...
**Was this image built by Devloop?**
`debug` only works for images that were built by
 Devloop so as to avoid affecting system- or infrastructure-level containers such as proxy sidecars.

**Was Devloop able to recognize the image?**
...
Automatic debugging of Go and Python applications requires that Devloop
...
Devloop cannot debug Go and Python applications that use user-created
...
Devloop configures the containers to run-on-start (sometimes called
...
- `devloop debug` usually recognizes already-configured container images
...
`devloop debug` has some limitations.

### Unsupported Container Entrypoints

`devloop debug` requires being able to examine and alter the
...
`devloop debug` is only supported with the `kubectl`, `kustomize`, and `helm` deployers.
...
`devloop debug` does not support deprecated versions of Workload API objects:
...
`artifact` is the corresponding artifact's image name in the `devloop.yaml`.
...
Devloop's event mechanism ([gRPC]({{< ref "/docs/references/api/grpc#debuggingcontainerevent" >}}),
[REST]({{< ref "/docs/references/api/swagger#/DevloopService/Events" >}})).

<details>
<summary>`/v1/events` stream of `devloop debug` within `examples/jib`</summary>

In this example, we do a `devloop debug`, and then kill the deployed pod.  The deployment starts a new pod.
...
The API's _state_ ([gRPC]({{< relref "/docs/references/api/grpc#devloopservice" >}}),
[REST]({{< relref "/docs/references/api/swagger#/DevloopService/GetState" >}})) also includes a list of debuggable containers.
