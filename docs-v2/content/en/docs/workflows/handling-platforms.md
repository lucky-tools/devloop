...
Devloop has a lot of intelligence built-in to simplify working with ARM workloads. Whether developing on an Apple Silicon Macbook that uses an ARM based chip, or deploying to a GKE Kubernetes cluster having ARM nodes, Devloop can take away the complexities that arise when the architecture of your development machine and Kubernetes cluster don't match.
...
🎉 *Devloop provides an opionated way to handle all these cases effectively.* 🎉

## Devloop can set the image architecture automatically

When running Devloop in an interactive mode like `devloop dev`, `devloop debug` or `devloop run` where the intention is to build an image from the application code, and immediately deploy it to a Kubernetes cluster, Devloop will check the active Kubernetes cluster node architecture and provide that as an argument to the respective image builder. If the cluster has multiple architecture nodes, then Devloop will also create appropriate Kubernetes [`affinity`](https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/#affinity-and-anti-affinity) rules so that the Kubernetes Pods with these images are assigned to matching architecture nodes.

{{< alert title="Note" >}}
Devloop will create platform node `affinity` rules only for clusters having multiple architecture nodes. You can also force this using the flag `--enable-platform-node-affinity=true` to always create these affinity rules in the Kubernetes manifests for built images.
{{< /alert >}}

Let's test this in a [sample Golang](https://github.com/lucky-tools/devloop/tree/main/examples/cross-platform-builds) project:

* The `devloop.yaml` file defines a single [Docker build artifact](https://github.com/lucky-tools/devloop/blob/main/examples/cross-platform-builds/Dockerfile) and deploys it in a [`Kubernetes Pod`](https://github.com/lucky-tools/devloop/blob/main/examples/cross-platform-builds/k8s-pod.yaml).

* First set the active Kubernetes context to a cluster having only `linux/amd64` nodes, and run:

  ```cmd
  devloop dev --default-repo=your/container/registy
  ```

  Devloop will detect the cluster node platform `linux/amd64` and build the image for this platform:

  ```cmd
  devloop dev --default-repo=gcr.io/k8s-devloop
  Listing files to watch...
  - devloop-example
  Generating tags...
  - devloop-example -> gcr.io/k8s-devloop/devloop-example:latest
  Starting build...
  Building [devloop-example]...
  Target platforms: [linux/amd64]
  ...
  Build [devloop-example] succeeded
  Starting deploy...
...
* Now set the active Kubernetes context to a cluster containing both `linux/arm64` and `linux/amd64` nodes. You can create a GKE cluster with 2 node pools, one having `linux/amd64` nodes, and the other having `linux/arm64` nodes.
  ...
  Re-run the `dev` command but with an explicit platform target this time via the `--platform` flag. If we don't provide the target platform explicitly then Devloop will choose one between `linux/amd64` and `linux/arm64`, trying to match your local dev machine architecture.

  ```cmd
  devloop dev --default-repo=your/container/registy --platform=linux/amd64
  ```

  Devloop will build a `linux/amd64` image and insert a `nodeAffinity` definition to the `Pod` so that it gets scheduled on the matching architecture node.

  ```cmd
  devloop dev --default-repo=gcr.io/k8s-devloop --platform=linux/amd64
  ...
  ...
  [getting-started] Hello world! Running on linux/amd64
  ```
...
* Run this command to build for the target architectures `linux/amd64` and `linux/arm64`:
  ...
  => => naming to gcr.io/k8s-devloop/devloop-example:latest_linux_amd64
  ...
  ...
  => => naming to gcr.io/k8s-devloop/devloop-example:latest_linux_arm64
  ...
  4e0c2525c370: Pushed
...
* Validate that the image just built was multi-arch, by running the following `docker` command:
  ...
  ```cmd
  docker manifest inspect your/container/registry/devloop-example:latest | grep -A 3 "platform"
  ```
...
* Now if we render the Kubernetes Pod manifest for this multi-arch image, then it'll have platform affinity definition targeting both `linux/amd64` and `linux/arm64` architectures.
  ...
  ```cmd
  devloop render --default-repo=your/container/registry --enable-platform-node-affinity
  ```
        ...
        requiredDuringSchedulingIgnoredDuringExecution:
          ...
          - matchExpressions:
            ...
            - key: kubernetes.io/arch
              ...
              - arm64
    containers:
    - image: gcr.io/k8s-devloop/devloop-example:latest@sha256:9ecf4e52f7ff64b35deacf9d6eedc03f35d69e0b4bf3679b97ba492f4389f784
      name: getting-started
  ```
...
* For interactive modes like `devloop dev` and `devloop debug` requiring fast and repeated `build-render-deploy` iterations, Devloop will choose only one build architecture and build a single-platform image, even if you specify multiple target platforms.

* If you need to build a multi-arch image with an interactive mode then use `devloop run`. This will build the multi-arch image and deploy it to the active Kubernetes cluster.
{{< /alert >}}
