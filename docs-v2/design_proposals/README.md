...
large code changes to `devloop`. By large we mean,  large in impact or large in
size.
...
1. The feature/change is aligned with the Devloop roadmap and the team's general
...
3. When changing any existing feature, the implementation plan adheres to
   [Devloop Deprecation Policy](http://devloop.dev/docs/references/deprecation/).
