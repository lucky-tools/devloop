...
Ideally, the CLI and `devloop.yaml` options are already enough for the great majority of Devloop users.

## Background

So far, Devloop always uses the currently active kubecontext when interacting with a Kubernetes cluster.
This is problematic when users want to deploy multiple projects with different kubecontexts, because the user needs to manually switch the context before starting Devloop.
...
- Allow option to specify the kubectl context ([#511](https://github.com/lucky-tools/devloop/issues/511))
- Support kube.config and kube.context for specifying alternative Kubernetes config file or context ([#2325](https://github.com/lucky-tools/devloop/issues/2325))
- Feature: Support regex in profile activation via kubeContext ([#1677](https://github.com/lucky-tools/devloop/issues/1677))
- Devloop.yaml files are not portable ([#480](https://github.com/lucky-tools/devloop/issues/480))
- Support forcing a context and a namespace for a profile/command ([#2426](https://github.com/lucky-tools/devloop/issues/2426))

There also was an attempt to add a configuration option to `devloop.yaml` (Support for overriding kubectl context during deployment [#1540](https://github.com/lucky-tools/devloop/pull/1540)).

The goal of this document is to create an agreement on what options should be supported and identify edge cases.

### Recommended use-cases

##### As Devloop user, I want to define the kubecontext for a single devloop run.
...
Use the kubecontext configuration in `devloop.yaml`.
...
Use kubecontext setting in the global Devloop config (via `devloop config set ...`).

##### As Devloop user with multiple profiles, I want to use different kubecontexts for different profiles.
Use the kubecontext configuration in `devloop.yaml`.
    ...
    <tbody>
        <tr>
            ...
            <td>
              ...
              However, in Devloop this term is so overloaded that it should more precisely be named <code>--kube-context</code>.
        ...
        <tr>
            ...
            <td>Env variable</td>
            <td>
              <code>DEVLOOP_KUBE_CONTEXT</code>, similar to other Devloop flags.
            </td>
        </tr>
        <tr>
            <td>3.</td>
            <td><code>devloop.yaml</code></td>
            <td>
...
### Detailed discussion
#### Option in `devloop.yaml`
A configuration option in `devloop.yaml` has the advantage of being most discoverable:
it is in the place where users configure all aspects of their pipeline.
In addition, it allows to define the kubecontext per Devloop profile.

A natural place for the config in `devloop.yaml` is in `latest.DeployConfig`, resulting in a json path `deploy.kubeContext`.
...
- Identify projects by a new `metadata.name` entry in `devloop.yaml` (see also [#2200](https://github.com/lucky-tools/devloop/issues/2200)).
...
Currently, the Devloop config uses the kubecontext as identifier.
...
1. Reverse the mapping project/kubecontext and save as list under `kube-context` entries:
   ...
   - kube-context: context2
     ...
     - my-project
   ```
2. Add a new top-level entry in Devloop config:
   ```yaml
   global: {}
   kubecontexts: []
   devloopConfigs:
     ...
     '*': default-context
   ```
   This option will be more complex to implement wrt `devloop config`.

### Open Issues/Questions

**\<What Devloop config structure has the best tradeoffs?\>**

Resolution: The top-level entry (option 2) overall has the better trade-offs.

## Implementation plan
1. Implement the CLI flag and env var variant first. This should also be the most important for the IDE integration. [#2447](https://github.com/lucky-tools/devloop/pull/2447)
2. Implement `devloop.yaml` variant. [#2510](https://github.com/lucky-tools/devloop/pull/2510)
3. ~~Implement the global Devloop config variant to override a kubecontext for a devloop config (`devloopConfigs`).~~ [#2558](https://github.com/lucky-tools/devloop/pull/2558)
4. ~~Implement the global Devloop config variant to set a default kubecontext (`devloopConfigs['*']`).~~ [#2558](https://github.com/lucky-tools/devloop/pull/2558)
5. ~~Implement the namespace functionality.~~ (out of scope)

## Integration test plan

A single test covers the overall kubecontext override functionality sufficiently (part of [#2447](https://github.com/lucky-tools/devloop/pull/2447)).
