...
Devloop can only transform manifests from a non-extensible allowlist. When
using any CRD out of this allowlist, devloop can not transform it.

Open issues concerning this problem:

* Image not recognized in crd k8s manifest ([#4081](https://github.com/lucky-tools/devloop/issues/4081))
...
and specification to extend `devloop.kubernetes.manifest.transformableAllowList`.

## Design

Add configuration `deploy.config.transformableAllowList` in `devloop.yaml`:
...
refactor [current allowlist](https://github.com/lucky-tools/devloop/blob/27c38228ab929ddaf2636637b43f17fda1686652/pkg/devloop/kubernetes/manifest/visitor.go#L28-L43).

Is there any need to work out a deny list?

## Implementation plan

1. `pkg/devloop/schema/latest/config.go` - Add config option
`transformableAllowList` to `DeployConfig`.
2. `pkg/devloop/kubernetes/manifest/visitor.go` - Refactor allowlist and add
...
it to existing coded `transformableAllowList`
    ...
    - Support `image` field
3. `pkg/devloop/kubernetes/manifest/images.go` - Add new parameter to `*ManifestList.ReplaceImages()`
