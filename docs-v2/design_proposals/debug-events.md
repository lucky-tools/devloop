...
Cloud Code for IntelliJ and VS Code currently launch `devloop debug --port-forward` to start
an application in debugging mode.  `debug` modifies the podspecs to explicitly expose the debug port used
by the underlying language runtime via `containerPort` definitions.  Devloop monitors the cluster
...
When a new debuggable container is created, Devloop will
...
Although such information is published by `devloop debug` via pod annotations, obtaining this
information would require that the IDEs actively track new pods and containers.  This tracking would
duplicate functionality already being performed by Devloop, and also requiring this functionality to be
re-implemented in each IDE.

This design proposal recommends that Devloop instead also watch and issue events when debuggable
containers are started and terminated.

----
## Design

This design proposes that Devloop issue events to inform the IDEs of the following conditions:
...
- The appearance of a container that is debuggable.  This event will provide:
    ...
    - the contents of the `debug.cloud.google.com/config` configuration block for the container
      (a JSON object) including (! indicates new items)
         ...
         - ! the image name (which should allow cross-referencing with the `devloop.yaml`)
         - the port(s)
...
- The disappearance of a container that was debuggable.  This event will provide:
    ...
    - the contents of the `debug.cloud.google.com/config` configuration block for the container
      (a JSON object) including (! indicates new items)
         - the language runtime type (`go`, `jvm`, `nodejs`, `python`)
         - ! the corresponding artifact's `name` (which should allow cross-referencing with the `devloop.yaml`)
         - the port(s)
         - ! the container image's working directory
         - ! (optional) the container image's _remote root_ for source file resolving 
         
Devloop will also maintain state and report on the set of debuggable containers.

With this information, the IDE should be able to:

- Identify the corresponding artifact within the `devloop.yaml`
  ...
  "result": {
    ...
    "event": {
      "debugEvent": {
        ...
        "namespace": "default",
        "runtime": "nodejs",
        "configuration": "{\"name\":\"gcr.io/artifact/name/from/devloop.yaml\",\"ports\":{\"devtools\":9229},\"runtime\":\"nodejs\",\"workingDir\":\"/app\"}"
      }
    },
...
$ curl -s localhost:50052/v1/state | jq .
{
  "buildState": {
    "artifacts": {
      "gcr.io/k8s-devloop/devloop-jib": "Complete"
    }
  },
  "deployState": {
    "status": "Complete"
  ...
  "debuggingContainers": [
    {
      ...
      "namespace": "default",
      "runtime": "jvm",
      "configuration": "{\"name\":\"gcr.io/artifact/name/from/devloop.yaml\",\"ports\":{\"jdwp\":5005},\"runtime\":\"jvm\",\"workingDir\":\"/app\"}"
    }
  ]
}
```
...
The idea here is that `devloop debug` internally configures a watcher that looks for and notifies
of pods with debuggable containers, similar to how the [port-forwarding manager](https://github.com/lucky-tools/devloop/blob/main/pkg/devloop/kubernetes/portforward/pod_forwarder.go) 
listens for pods or services.

1. `devloop debug` will set a `DebugMode` option to `true`. This value will be used in
  `DevloopRunner.Dev()` to install the debug container watch manager.
