...
So far, Devloop supports multiple taggers or tag policies:
...
The default tagger, if none is specified in the `devloop.yaml`, is the `git` tagger.

Here are some rules about how tagging currently works:

 + **Image tags are computed before the images are built**. In early versions of Devloop, we tried
...
## Proposal
 ...
 + An `inputDigest` is added. It uses the digest of the artifact's inputs as the tag.
   [#2301](https://github.com/lucky-tools/devloop/pull/2301) tried to implement
 ...
 + An `userGenerated` is added. This is for special cases where the user can pass command 
   ...
   with maven, gradle (and probably other build tools) that can fallow more precisely 
   dependency graph of a given executable. With time these plugins could become integrated 
   into a devloop code base. Or at least be officially supported. It is also important to note 
 ...
 + **No matter the tagger, Devloop will keep on using immutable references in manifests**.
