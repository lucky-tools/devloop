...
We are open to introduce a streaming/reactive design in Devloop on the long run if we have a strong enough argument/featureset for it. For this redesign I don't see that it is worthwhile. I am closing this and the related PRs. 
...
The second issue here comes more from bad execution rather than bad design. Some fixes for this have already been proposed and/or merged ([#1786](https://github.com/lucky-tools/devloop/pull/1786) and [#1801](https://github.com/lucky-tools/devloop/pull/1801)), so progress has already been made, but we can probably improve on this more.

The first issue is what I'll be focusing on in this issue. @dgageot has already merged one improvement to the design [here](https://github.com/lucky-tools/devloop/pull/1829), but this is only improving on an already flawed design. My goal for this redesign will be to reduce and potentially eliminate the overhead the event API has on maintainers and contributors to continue evolving the devloop codebase.
...
Every build or deploy that devloop performs always comes through the runner, so it would follow that the runner could handle all of the eventing for builds and deploys, as long as it can retrieve the necessary information from the builder or deployer.
...
To address this, **I propose that we change the Build() interface to return a list of build results, one for each artifact, with information about the original artifact devloop attempted to build, and either the built artifact, or the error the builder encountered.**
...
The current integration tests around eventing and retrieving devloop state should be sufficient to cover this new change. Devloop runs are triggered, the state is retrieved at various times during the run, and the tests verify the correct results are retrieved.
