...
* Reason: 
  ...
  We discussed this proposal in Devloop Community Hours and decided at this
  moment, the design proposal is just an entry point for running a script.
  It is not built up on Devloop's run knowledge so far.
  With that said, the core team agrees Devloop does not a solution
  for [#2561](https://github.com/lucky-tools/devloop/issues/2561)
  and [#992](https://github.com/lucky-tools/devloop/issues/992) and we want to
  explore this space.
          
## Background

Currently, devloop has no support for running integration/unit tests 
that are part of a built artifact. Since this is a crucial part of a CI/CD
pipeline it would be good to support this feature so devloop can be used
in every step of a pipeline.

Proof-of-concept:
- [Integration test command (\#2594)](https://github.com/lucky-tools/devloop/pull/2594)

Related issues:
- [Integrationtest phase (\#2561)](https://github.com/lucky-tools/devloop/issues/2561)
- [Add \`test\` phase to devloop runner (\#992)](https://github.com/lucky-tools/devloop/issues/992)
___

## Design

#### New configuration options in devloop.yaml

```yaml
integrationtest:
    podSelector: 'app=devloop-integration'
    testCommand: 'pytest tests/pass.py'
```

A top-level key called `integrationtest` holds the configuration for running
integration tests with devloop.
...
#### New devloop CLI command

A new command called `devloop integrationtest` is implemented that takes
the values from `devloop.yaml` to execute the tests and report the output
and outcome.
...
* build: `devloop build --file-output build.json`
* test: 
  * `devloop deploy -n $(pipeline_id) -a build.json`
  * `devloop integrationtest`
  * `devloop delete -n $(pipeline_id)`
* deploy: `devloop deploy -a build.json`
