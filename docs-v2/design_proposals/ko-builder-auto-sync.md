...
* Date: 2022-09-26
* Status: Draft
* go/devloop-ko-devmode

## tl;dr

Make Go development on a Kubernetes cluster feel as fast and responsive as on
a developer workstation. Iterate on your code, and let Devloop and ko rebuild
...
Implement
[auto sync](https://devloop.dev/docs/filesync/#auto-sync-mode)
for the
[Devloop ko builder](https://devloop.dev/docs/builders/ko/) in
[`dev`](https://devloop.dev/docs/references/cli/#devloop-dev) mode.

On source code changes, Devloop rebuilds the Go binary locally using ko, and
...
Devloop has a `dev` mode that rebuilds and reloads the application running on a
...
The Devloop
[Buildpacks builder](https://devloop.dev/docs/builders/buildpacks/)
supports
[hot reloading](https://devloop.dev/docs/filesync/#buildpacks)
with
[Google Cloud Buildpacks](https://github.com/GoogleCloudPlatform/buildpacks).

Devloop watches for source file changes on the developer workstation and
...
User runs `devloop dev` for an artifact configured to use the ko builder, and
`sync.auto` is `true`:
...
2.  Rewrite the Kubernetes pod spec and specify watchexec as the container
    ...
    Devloop debug manifest rewriting implementation.

3.  When a change event takes place for local source code files, determine
    the platform for the rebuilt binary. Use the field from `devloop.yaml` or
...
5.  When constructing the
    ...
    ensure that ko doesn't download the base image again. Devloop is only
...
7.  Use the existing Devloop sync feature to sync the rebuilt binary from
...
2.  Should the URL used to download `watchexec` be exposed as a field in the
    ...
    running `devloop dev`. Devloop checks for the presence of the
