...
This proposal adds a new `ko` builder to Devloop, based on the `ko build`
...
- [the Cloud Native buildpacks builder](https://devloop.dev/docs/builders/buildpacks/),
  the ko builder is
  [fast](https://cloud.google.com/blog/topics/developers-practitioners/ship-your-go-applications-faster-cloud-run-ko),
...
- [the Docker builder](https://devloop.dev/docs/builders/docker/),
...
- [the Kaniko builder](https://devloop.dev/docs/builders/docker/#dockerfile-in-cluster-with-kaniko),
  the ko builder doesn't need a Kubernetes cluster, and avoids the
  previously-mentioned artisanal `Dockerfile`s.


- [the custom builder](https://devloop.dev/docs/builders/custom/),
  the ko builder is portable:

  1.  The Devloop config can be shared with other developers and ops teams,
      ...
      path to adoption of Devloop and reduces friction for users, both for
      local development, and for anyone using Devloop in CI/CD pipelines.
...
The ko builder supports and enhances these Devloop
[features](https://devloop.dev/docs/):

- _fast local workflow_: building with ko is
  [fast](https://cloud.google.com/blog/topics/developers-practitioners/ship-your-go-applications-faster-cloud-run-ko).

- _share with other developers_: no additional tools are required to
  `devloop run` with the ko builder, not even Docker. Though if we don't embed
  ko in Devloop, ko will be a tool that all developers in a team would have to
  install.

- works great as a _CI/CD building block_: when using the ko builder, pipeline
  steps can run using the default Devloop container image, without
...
For instance, to build Devloop using ko, from the repository root directory:

```sh
ko build ./cmd/devloop
```

or

```sh
ko build github.com/lucky-tools/devloop/cmd/devloop
```
...
`ko://github.com/lucky-tools/devloop/cmd/devloop`.
...
- The last path segment (e.g., `devloop`), followed by a hyphen and a MD5
  hash. This is the default behavior of the `ko build` command.

- The last path segment (e.g., `devloop`) only, if `ko build` is invoked
  with the `-B` or `--base-import-paths` flag.

- The full import path, lowercased (e.g.,
  `github.com/googlecontainertools/devloop/cmd/devloop`), if `ko build` is
...
## Supporting existing Devloop users

The Devloop ko builder follows the existing Devloop image naming logic. This
means that the image naming behavior doesn't change for existing Devloop users
...
Devloop
[default repo](https://devloop.dev/docs/environment/image-registries/) value
is `gcr.io/k8s-devloop` and the value of the `image` field in `devloop.yaml`
is `devloop`, the resulting image name will be `gcr.io/k8s-devloop/devloop`.
...
`go build ./cmd/devloop`. Using the `main` field results in deterministic
...
[`context`](https://devloop.dev/docs/references/yaml/#build-artifacts-context)
(a.k.a.
[`Workspace`](https://github.com/lucky-tools/devloop/blob/v1.27.0/pkg/devloop/schema/latest/config.go#L832))
directory.

For example, to build Devloop itself, with `package main` in the
`./cmd/devloop/` subdirectory, the config would be as follows:
...
[`QualifyImport()`](https://github.com/lucky-tools/devloop/blob/953594000be68fa8fad0ec4636ab03f8153a1c08/pkg/devloop/build/ko/build.go#L93).
...
To support existing ko users moving to Devloop, the ko builder also supports
`image` names in `devloop.yaml` that use the Go import path, prefixed by the
...
In the case of `ko://`-prefixed image names, the Devloop ko builder
...
2.  Transforming the import path to a valid image name using the function
    [`SanitizeImageName()`](https://github.com/lucky-tools/devloop/blob/v1.27.0/pkg/devloop/docker/reference.go#L83)
    (from the package
    `github.com/lucky-tools/devloop/pkg/devloop/docker`).
3.  Combining the Devloop default repo with the transformed import path as per
    existing Devloop image naming logic.

This will result in image names that match those produced by the `ko` CLI when
using the `-P` or `--preserve-import-paths` flag. For example, if the Devloop
default repo is `gcr.io/k8s-devloop` and the `image` name in `devloop.yaml`
is `ko://github.com/lucky-tools/devloop/cmd/devloop`, the resulting
image name will be
`gcr.io/k8s-devloop/github.com/googlecontainertools/devloop/cmd/devloop`.
...
If the `image` field in `devloop.yaml` starts with the `ko://` scheme prefix,
the Devloop ko builder uses the Go import path that follows the prefix. For
example, to build Devloop itself, with `package main` in the `./cmd/devloop/`
...
build:
  artifacts:
  - image: ko://github.com/lucky-tools/devloop/cmd/devloop
    context: .
    ko: {}
```
...
## Debugging ko images using Devloop
...
The ko builder sets `DisableOptimizations` to `true` when the Devloop
`runMode` is `Debug`.

Devloop can
[recognize Go-based container images](https://devloop.dev/docs/workflows/debug/#go-runtime-go-protocols-dlv)
...
This allows Devloop to transform Pod specifications to enable remote
debugging.

The ko builder implementation work will add `KO_DATA_PATH` to
[the set of environment variables used to detect Go-based applications](https://github.com/lucky-tools/devloop/blob/c75c55133e709b2fee906eb158be13c7ccfa72cd/pkg/devloop/debug/transform_go.go#L67-L72)
and updating the associated unit tests and
[documentation](https://github.com/lucky-tools/devloop/blob/main/docs-v2/content/en/docs/workflows/debug.md#L245).

## Design

Adding the ko builder requires making config changes to the Devloop schema.
...
1.  Add a `KoArtifact` type:
    ...
    type KoArtifact struct {
        ...
        // Dependencies are the file dependencies that Devloop should watch for both
        ...
        // For example: `{"org.opencontainers.image.source":"https://github.com/lucky-tools/devloop"}`.
...
3.  Define `KoDependencies`:
    ...
    type KoDependencies struct {
        ...
        // so that the Devloop file watcher knows when to rebuild and perform file synchronization.
        // Defaults to ["**/*.go"].
...
build:
  artifacts:
  - image: devloop-example-ko-comprehensive
    ko:
...
environment variable to specify where container images are pushed. The Devloop
[default repo](https://devloop.dev/docs/environment/image-registries/)
maps directly to this value.

### Resolved questions

1.  Should Devloop embed ko as a Go module, or shell out?

     __Resolved:__ Embed as a Go module

    Benefits of embedding:

    - Devloop can pin the ko version it supports in its `go.mod` file. Users
      wouldn't raise bugs/issues for incompatible version pairings of Devloop
      and ko.

    - Reduce toolchain maintenance toil for users. Devloop users wouldn't need
      to synchronize ko versions used by different team members or in their CI
      build, since the Devloop version determines the ko version.

    - Portability. Devloop+ko users only need one tool for their container
      image building needs: the `devloop` binary. (Plus the Go distribution,
      of course.) The current `gcr.io/k8s-devloop/devloop` container image
    ...
    guarantees for the most ko interfaces that Devloop would use, such as
    ...
    - It's an established pattern used by other Devloop builders.

    - It would allow Devloop to support a range of ko versions. On the other
      hand, these versions would need to be tracked and documented.

    - No need to resolve dependency version differences between Devloop and
      ko.

    - If a new ko version provided a significant bug fix, there would be no
      need to release a new version of Devloop for this fix.
...
2.  Should Devloop use base image settings from
    [`.ko.yaml`](https://github.com/google/ko#configuration) if the ko builder
    definition in `devloop.yaml` doesn't specify a base image?

    __Resolved:__ Yes, to simplify adoption of Devloop for existing ko users.
...
3.  If a config value is set both as an environment variable, and as a config
    ...
    __Resolved:__ Follow existing Devloop patterns.
...
5.  Should we default dependency paths to `["go.mod", "**.go"]` instead of
    ...
    [#6617](https://github.com/lucky-tools/devloop/pull/6617#discussion_r719804744).


    By embedding ko as a module, there is no need for a ko-specific Devloop
    builder image.
...
1.  Should the ko builder be the default for `devloop init`, instead of
    buildpacks, for Go apps, when there's no Dockerfile
    file?

    Suggest yes, to make Devloop a compelling choice for Go developers.

    If no, we can still consider configuring the ko builder if `devloop init`
...
2.  Implement the core ko build and publish logic, including unit tests in the
    package `github.com/lucky-tools/devloop/pkg/devloop/build/ko`.
...
3.  Add integration test for the ko builder to the `integration` package, and
    ...
    package `github.com/lucky-tools/devloop/pkg/devloop/schema` that
...
5.  Plumb the code in the package `pkg/devloop/build/ko` into the other parts
    of the Devloop codebase that interacts with the config schema.

    E.g., a `case` statement for `a.KoArtifact` in the `newPerArtifactBuilder`
    function in `pkg/devloop/build/local` (file `types.go`).

6.  When we are ready to add the ko builder as an alpha feature to an upcoming
    Devloop release, set the `KoArtifact` YAML flag key to `ko` and add `KO`
    to the `BuilderType` enum.

## Implementation plan

1.  [Done] Define integration points in the ko codebase that allows ko to be
    used from Devloop without duplicating existing ko CLI code.
    ...
    Add build and publish options to support Devloop config propagating to
...
2.  Add ko builder with support for existing ko config options. Provide
    ...
    Example `devloop.yaml` supported at this stage:
...
3.  Implement Devloop config support for additional ko config options not
    currently supported by ko:

    -   `asmflags`
    -   `gcflags`

    Provide this as a feature in an upcoming Devloop release.
...
- Support for the Devloop lifecycle subcommands that build images:
  - `build`
  - `debug`
  - `dev`
  - `run`
...
- Local builder only (no `cluster` builder at this stage).
...
- Image names following standard Devloop naming, for existing Devloop
  users.
...
- `devloop init` support, behind a `--enableKoInit` flag.
...
1.  Unit and integration tests for ko builder, similar to other builders.
    ...
    changes to ko interfaces break the Devloop ko builder.
