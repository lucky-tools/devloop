...
Currently devloop does not support `sync` for files that are generated
during a build. For example when syncing java files to a container, one
would normally expect `.class` files to be sync'd, but devloop is
really only aware of `.java` files in a build.

1. Why is this required?
  - We would like to support the following features
      - Devloop can sync files that are not watched (these files are not
        ...
        would be picked up by Devloop and copied over, and the app would pick
        up the changes.
      - Devloop can sync files that are generated by a buildscript. Motivating
        example: user changes java files, Devloop watchers notices this, runs the
      ...
      - And one or both of the following:
          - Devloop can be configured to run a custom script to when certain files change.
          ...
          - Devloop can sync an externally generated tar file to a remote container (see
...
2. If this is a redesign, what are the drawbacks of the current implementation?
  - This is not a redesign, but a new sync mode that may or may not be available
    in the devloop.yaml directly to users. It could simply be an internal API
...
2. No way to tell the system to sync non build inputs. Devloop is
...
### Hack it
To get close to the functionality we want, without modifying devloop at all, a
...
with a devloop sync block that looks like:
```
sync:
  manual:
...
Devloop can expose an API that can accept a complex configuration on how
devloop should be doing synchronization.
...
#### Jib - Devloop Sync interface

How a tool like Jib might surface the necessary information to Devloop

I would expect to add a task like `_jibDevloopSyncMap` that will produce
json output for the devloop-jib intergration to consume and forward to the sync
...
- the test would do the following steps: 
    - spin up a simple java app built by jib with devloop dev + autosync enabled
    - get pod name
...
2. Doesn't really provide a good base for exposing the `auto` sync configuration
   block. As devloop takes care of fewer things, the user must be responsible
   for implementing it.

#### Delegate generation of sync tar to builder

One option is to complete hand over the detection of container updates to the
builder which would provide devloop with a tar to synchronize.

A potential runthrough might look like:

1. Devloop detects changes
1. Devloop asks build system if sync should happen
1. Build system can reply
  1. Yes: and here is the `tar` to sync
  1. No: this will tell devloop it should do a rebuild
...
1. If we should rebuild -> tell devloop to rebuild, so output something like
  ```
  REBUILD
  ```
1. If we can sync
  ...
  3. Tell devloop where to find that tar
  ```
