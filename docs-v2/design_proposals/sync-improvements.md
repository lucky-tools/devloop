...
Currently, devloop config supports `artifact.sync` as a way to sync files
...
Furthermore, builders should be able to inform Devloop about additional sync paths (#1704).
...
Devloop sync shall have three different modes:

1. _manual_: the user specifies both the source and destination.
3. _auto-inferred destination_: the user specifies the syncable files, and Devloop infers their destination in the container, e.g based on Dockerfile
...
### Goals
The sync mechanism in devloop should be able to infer the destination location of local files, when the user opts-in to do so.
...
This information needs to be pulled from the builders and made available in Devloop.
...
For that, the `Builder` interface (pkg/devloop/build/build.go) will receive the new method `SyncMap`.
...
**\<Other builders\>**
We don't know how to obtain a mapping of local paths to remote paths for other builders (jib).
...
Therefore, we could consider to advertise this new functionality when running Devloop or when an old-style sync rule as found.
...
6. (future) Add sync map logic for examples.
