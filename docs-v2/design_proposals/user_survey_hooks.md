...
Currently, we get feedback on Devloop via Devloop HaTS survey. 
...
See [#6166](https://github.com/lucky-tools/devloop/issues/6166)
...
type config struct {
	...
	// For example: `As a Helm user, we are requesting your feedback on a proposed change to Devloop's integration with Helm.`
	promptText   string
	// startsAt mentions the date after the users survey should be prompted. This will ensure, devloop team can finalize the survey 
...
```
{
  ...
  isRelevantFn: func(cfgs []util.VersionedConfig, command string) bool {
	for _, cfg := range cfgs {
		if v1Cfg, ok  := cfg.(*latestV1.DevloopConfig) ; ok {
...
The user survey information will be tracked in the existing `Survey` config in the devloop global config
```
...
If both active relevant user survey is available or HaTS survey was not taken, then Devloop will give preference to user survey over hats survey.
...
- [X] add `UserSurvey` struct to devloop global config
