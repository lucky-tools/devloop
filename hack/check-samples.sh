...
MISSING_DEVLOOP_YAMLs="false"
# to whitelist an example from having a devloop.yaml, add a `grep -v "example_name"` here
cat $EXAMPLES | grep -v "compose" | grep -v "buildpacks-node-tutorial" | grep -v "jaeger-devloop-trace" | grep -v "jib-sync" | while read e; do
  NUM_DEVLOOP_YAML=$(find "examples/$e" -type f -name devloop.yaml | wc -l)
  if [[ NUM_DEVLOOP_YAML -eq 0 ]]; then
    echo "examples/$e doesn't have a devloop.yaml!"
    MISSING_DEVLOOP_YAMLs="true"
  fi
done

if [[ "$MISSING_DEVLOOP_YAMLs" == "true" ]]; then
  exit 1
fi

# /examples should use the latest released version
LATEST_RELEASED="devloop/$(go run ./hack/versions/cmd/latest_released/version.go)"

EXIT_CODE=0

for EXAMPLE in $(find examples -name devloop*.yaml); do
    if [ "1" != "$(grep -c "apiVersion: ${LATEST_RELEASED}" "${EXAMPLE}")" ]; then
        echo "devloop version in ${EXAMPLE} should be ${LATEST_RELEASED}"
        EXIT_CODE=1
    fi
done

# /integration/examples should use the latest (even if not released) version
LATEST="devloop/$(go run ./hack/versions/cmd/latest/version.go)"

for EXAMPLE in $(find integration/examples -name devloop*.yaml); do
    if [ "1" != "$(grep -c "apiVersion: ${LATEST}" "${EXAMPLE}")" ]; then
        echo "devloop version in ${EXAMPLE} should be ${LATEST}"
        EXIT_CODE=1
    fi
done
