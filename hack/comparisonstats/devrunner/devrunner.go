...
import (
	...
	v1 "github.com/lucky-tools/devloop/proto/v1"
)
...
func Dev(ctx context.Context, app types.Application, devloopBinaryPath string, eventsFileAbsPath string, flagOpts ...string) (*DevInfo, error) {
	logrus.Infof("Starting devloop dev on %s...", app.Name)
	...
	cmdArgs := []string{
		"dev", fmt.Sprintf("--rpc-port=%v", port),
	...
	cmd := exec.CommandContext(ctx, devloopBinaryPath, cmdArgs...)
	...
	go func() {
		...
		if err := cmd.Run(); err != nil {
			...
			logrus.Fatalf("devloop dev failed: %v, %v", err, buf.String())
		}
	}()
	...
	logrus.Infof("successfully ran %d inner dev loop(s), killing devloop...", app.DevIterations)
	if err := cmd.Process.Signal(os.Interrupt); err != nil {
		return nil, fmt.Errorf("killing devloop: %w", err)
	}
...
func waitForDevLoopComplete(ctx context.Context, iteration, port int) error {
	var (
		...
		err    error
		client v1.DevloopServiceClient
	)
	...
	if err := wait.Poll(time.Second, 2*time.Minute, func() (bool, error) {
		...
		client = v1.NewDevloopServiceClient(conn)
		return true, nil
	}); err != nil {
	...
	var stream v1.DevloopService_EventsClient
	for i := 0; i < 10; i++ {
