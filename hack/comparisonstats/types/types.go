...
import (
	...
	"github.com/lucky-tools/devloop/hack/comparisonstats/util"
)

type ComparisonStatsCmdArgs struct {
	DevloopBinaries []string
...
func ParseComparisonStatsCmdArgs(args []string) ComparisonStatsCmdArgs {
	return ComparisonStatsCmdArgs{
		DevloopBinaries: []string{args[0], args[1]},
...
type Config struct {
	...
	FirstDevloopFlags  string `yaml:"mainBranchDevloopFlags"` // NOTE: names mismatched for make vs gh action UX usage
	SecondDevloopFlags string `yaml:"thisPRDevloopFlags"`
...
// Dev describes necessary info for running `devloop dev` on a test application
type Dev struct {
