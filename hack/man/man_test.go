...
func TestPrintMan(t *testing.T) {
	...
	testutil.CheckContains(t, "devloop build", output)
	testutil.CheckContains(t, "devloop run", output)
	testutil.CheckContains(t, "devloop dev", output)
	...
	if string(expected) != string(header)+output {
		t.Error("You have devloop command changes but haven't generated the CLI reference docs. Please run ./hack/generate-man.sh and commit the results!")
	}
}
