...
// TestBinAuthZWithDeploy targets the integration-tests-binauthz cluster on k8s-devloop which has '--binauthz-evaluation-mode=PROJECT_SINGLETON_POLICY_ENFORCE' set
func TestBinAuthZWithDeploy(t *testing.T) {
	...
	output, err := devloop.Deploy("--images", "index.docker.io/library/busybox:1", "--default-repo=").InDir("examples/kustomize").InNs(ns.Name).RunWithCombinedOutput(t)
	if err == nil {
