	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			...
			if test.failure == "" {
				...
				devloop.Build(test.args...).InDir("testdata/build-dependencies").RunOrFail(t)
				checkImagesExist(t)
			} else {
				if out, err := devloop.Build(test.args...).InDir("testdata/build-dependencies").RunWithCombinedOutput(t); err == nil {
...
func TestBuildDependenciesCache(t *testing.T) {
	...
	devloop.Build("--cache-artifacts=true", "-p", "concurrency-0").InDir("testdata/build-dependencies").RunOrFail(t)
	checkImagesExist(t)
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			...
			out, err := devloop.Build("--cache-artifacts=true", "-p", "concurrency-0").InDir("testdata/build-dependencies").RunWithCombinedOutput(t)
			if err != nil {
...
func checkImagesExist(t *testing.T) {
	checkImageExists(t, "us-central1-docker.pkg.dev/k8s-devloop/testing/image1:latest")
	checkImageExists(t, "us-central1-docker.pkg.dev/k8s-devloop/testing/image2:latest")
	checkImageExists(t, "us-central1-docker.pkg.dev/k8s-devloop/testing/image3:latest")
	checkImageExists(t, "us-central1-docker.pkg.dev/k8s-devloop/testing/image4:latest")
}
