...
const imageName = "us-central1-docker.pkg.dev/k8s-devloop/testing/simple-build:"
...
func TestBuild(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			...
			devloop.Build(append(test.args, "--cache-artifacts=false")...).InDir(test.dir).RunOrFail(t)

			// Run with artifact caching
			devloop.Build(append(test.args, "--cache-artifacts=true")...).InDir(test.dir).RunOrFail(t)

			// Run a second time with artifact caching
			out := devloop.Build(append(test.args, "--cache-artifacts=true")...).InDir(test.dir).RunOrFailOutput(t)
...
func TestBuildWithWithPlatform(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			...
			devloop.Build(args...).InDir(test.dir).RunOrFail(t)
...
// TestExpectedBuildFailures verifies that `devloop build` fails in expected ways
func TestExpectedBuildFailures(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expected:    "Could not find goal '_devloop-fail-if-jib-out-of-date' in plugin com.google.cloud.tools:jib-maven-plugin:1.3.0",
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			MarkIntegrationTest(t, NeedsGcp)
			if out, err := devloop.Build(test.args...).InDir(test.dir).RunWithCombinedOutput(t); err == nil {
...
func TestRunWithDockerAndBuildArgs(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			devloopArgs:   []string{"--kube-context", "default"},
			dockerRunArgs: []string{"run", "child:latest"},
			wantOutput:    "IMAGE_REPO: gcr.io/k8s-devloop, IMAGE_NAME: devloop, IMAGE_TAG:latest",
		},
		{
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			defer devloop.Delete().InDir(test.projectDir).Run(t.T)
			if err := devloop.Build(test.devloopArgs...).InDir(test.projectDir).Run(t.T); err != nil {
				t.Errorf("devloop build args: %v  working directory:%s returned unexpected error: %v", test.devloopArgs, test.projectDir, err)
			}
