...
func TestCacheAPITriggers(t *testing.T) {
	...
	devloop.Build().InDir("examples/getting-started").RunOrFail(t)

	ns, client := SetupNamespace(t)
	rpcAddr := randomPort()

	// Disable caching to ensure we get a "build in progress" event each time.
	devloop.Dev("--cache-artifacts=false", "--rpc-port", rpcAddr).InDir("examples/getting-started").InNs(ns.Name).RunBackground(t)
	...
	failNowIfError(t, waitForEvent(90*time.Second, entries, func(e *proto.LogEntry) bool {
		return e.GetEvent().GetBuildEvent().GetArtifact() == "devloop-example"
	}))
}
...
func TestCacheHits(t *testing.T) {
	...
	testutil.Run(t, "TestCacheHits", func(t *testutil.T) {
		// Run devloop build first to fail quickly on a build failure
		devloop.Build().InDir("examples/getting-started").RunOrFail(t.T)

		ns, _ := SetupNamespace(t.T)
		rpcAddr := randomPort()

		// Rebuild with a different tag so that we get a cache hit.
		out := devloop.Build("--tag", ns.Name, "--rpc-port", rpcAddr).InDir("examples/getting-started").RunOrFailOutput(t.T)
		t.CheckContains("devloop-example: Found. Tagging", string(out))
	})
}
...
func TestCacheIfBuildFail(t *testing.T) {
	...
	devloop.Build("--cache-file", cacheFile).InDir(testDir).InNs(ns.Name).Run(t)
