...
func TestConfigListForContext(t *testing.T) {
	...
	out := devloop.Config("list", "-c", "testdata/config/config.yaml", "-k", "test-context").RunOrFailOutput(t)
...
func TestConfigListForAll(t *testing.T) {
	...
	out := devloop.Config("list", "-c", "testdata/config/config.yaml", "--all").RunOrFailOutput(t)
...
func TestFailToSetUnrecognizedValue(t *testing.T) {
	...
	err := devloop.Config("set", "doubt-this-will-ever-be-a-config-key", "VALUE", "-c", "testdata/config/config.yaml", "--global").Run(t)
...
func TestSetDefaultRepoForContext(t *testing.T) {
	...
	devloop.Config("set", "default-repo", "REPO1", "-c", file, "-k", "test-context").RunOrFail(t)
	out := devloop.Config("list", "-c", file, "-k", "test-context").RunOrFailOutput(t)
...
func TestSetMultiLevelRepoForContext(t *testing.T) {
	...
	devloop.Config("set", "multi-level-repo", "false", "-c", file, "-k", "test-context").RunOrFail(t)
	out := devloop.Config("list", "-c", file, "-k", "test-context").RunOrFailOutput(t)
...
func TestSetGlobalDefaultRepo(t *testing.T) {
	...
	devloop.Config("set", "default-repo", "REPO2", "-c", file, "--global").RunOrFail(t)
	out := devloop.Config("list", "-c", file, "--all").RunOrFailOutput(t)
...
func TestSetGlobalMultiLevelRepo(t *testing.T) {
	...
	devloop.Config("set", "multi-level-repo", "true", "-c", file, "--global").RunOrFail(t)
	out := devloop.Config("list", "-c", file, "--all").RunOrFailOutput(t)
