...
func TestControlAPIManualTriggers(t *testing.T) {
	...
	out := bytes.Buffer{}
	rpcAddr := randomPort()
	devloop.Dev("--auto-build=false", "--auto-sync=false", "--auto-deploy=false", "--rpc-port", rpcAddr, "--cache-artifacts=false").InDir("testdata/dev").InNs(ns.Name).RunInBackgroundWithOutput(t, &out)
	...
	// Give devloop some time to register a file change.
