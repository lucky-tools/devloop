...
func TestCustomTest(t *testing.T) {
	...
	devloop.Build().InDir(testDir).WithConfig(config).RunOrFail(t)

	ns, client := SetupNamespace(t)

	rpcAddr := randomPort()
	devloop.Dev("--rpc-port", rpcAddr).InDir(testDir).WithConfig(config).InNs(ns.Name).RunBackground(t)
	...
	os.WriteFile(depFile, []byte("foo"), 0o644)
