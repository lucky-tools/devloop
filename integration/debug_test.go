...
func TestDebug(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			...
			devloop.Build(test.args...).InDir(test.dir).RunOrFail(t)

			ns, client := SetupNamespace(t)

			devloop.Debug(test.args...).InDir(test.dir).InNs(ns.Name).RunBackground(t)
...
func TestDockerDebug(t *testing.T) {
	t.Run("debug docker deployment", func(t *testing.T) {
		MarkIntegrationTest(t, CanRunWithoutGcp)
		devloop.Build("-p", "docker").InDir("testdata/debug").RunOrFail(t)

		devloop.Debug("-p", "docker").InDir("testdata/debug").RunBackground(t)
		defer devloop.Delete("-p", "docker").InDir("testdata/debug").RunBackground(t)
...
func checkSupportContainer(containers []container.Summary, found *bool) {
	...
	for _, c := range containers {
		if strings.Contains(c.Image, "gcr.io/k8s-devloop/devloop-debug-support") {
			*found = true
		}
	}
}
...
func TestFilterWithDebugging(t *testing.T) {
	...
	renderedOutput := devloop.Render("--digest-source=local").InDir("examples/getting-started").RunOrFailOutput(t)

	testutil.Run(t, "no --build-artifacts should transform all images", func(t *testutil.T) {
		transformedOutput := devloop.Filter("--debugging").InDir("examples/getting-started").WithStdin(renderedOutput).RunOrFailOutput(t.T)
	...
	testutil.Run(t, "--build-artifacts=file should result in specific transforms", func(t *testutil.T) {
		...
		transformedOutput := devloop.Filter("--debugging", "--build-artifacts="+buildFile).InDir("examples/getting-started").WithStdin(renderedOutput).RunOrFailOutput(t.T)
		transformedYaml := string(transformedOutput)
		if strings.Contains(transformedYaml, "/dbg/go/bin/dlv") {
...
func TestDebugEventsRPC_StatusCheck(t *testing.T) {
	t.Skipf("Disable the test dues to flakyness. https://github.com/lucky-tools/devloop/issues/7405")
	MarkIntegrationTest(t, CanRunWithoutGcp)

	// Run devloop build first to fail quickly on a build failure
	devloop.Build().InDir("testdata/jib").RunOrFail(t)

	ns, client := SetupNamespace(t)

	rpcAddr := randomPort()
	devloop.Debug("--rpc-port", rpcAddr).InDir("testdata/jib").InNs(ns.Name).RunBackground(t)
...
func TestDebugEventsRPC_NoStatusCheck(t *testing.T) {
	...
	rpcAddr := randomPort()
	devloop.Debug("--rpc-port", rpcAddr, "--status-check=false").InDir("testdata/jib").InNs(ns.Name).RunBackground(t)

	waitForDebugEvent(t, client, rpcAddr)
}

// nolint add lint back after https://github.com/lucky-tools/devloop/issues/7405
