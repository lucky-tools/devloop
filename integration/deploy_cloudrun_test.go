...
func TestDeployCloudRun(t *testing.T) {
	...
	devloop.Deploy().InDir("testdata/deploy-cloudrun").RunOrFail(t)
	ctx := context.Background()
	svc, err := getRunService(ctx, "k8s-devloop", "us-central1", "devloop-test")
	if err != nil {
		t.Fatal(err)
	}
...
func TestDeployCloudRunWithHooks(t *testing.T) {
	...
	testutil.Run(t, "cloud run deploy with hooks", func(t *testutil.T) {
		...
		out := devloop.Run().InDir("testdata/deploy-cloudrun-with-hooks").RunOrFailOutput(t.T)
...
func TestDeployCloudRunWorkerPool(t *testing.T) {
	...
	// Other integration tests run with the --default-repo option.
	// This one explicitly specifies the full image name.
	devloop.Deploy().InDir("testdata/deploy-cloudrun-workerpool").RunOrFail(t)
	ctx := context.Background()
	workerpool, err := getWorkerPool(ctx, "k8s-devloop", "us-central1", "devloop-test-wp")
	if err != nil {
		t.Fatal(err)
	}
  ...
  cloudrun:
	...
	for _, test := range tests {
		testutil.Run(t, test.descrition, func(t *testutil.T) {
			projectID := "k8s-devloop"
			region := "us-central1"
			jobName := fmt.Sprintf("job-%v", uuid.New().String())
			devloopCfg := fmt.Sprintf(test.devloopCfg, latest.Version, projectID, region)
			jobManifest := fmt.Sprintf(test.jobManifest, jobName)

			tmpDir := t.NewTempDir()
			tmpDir.Write("devloop.yaml", devloopCfg)
			tmpDir.Write("job.yaml", jobManifest)

			devloop.Run().InDir(tmpDir.Root()).RunOrFail(t.T)
			t.Cleanup(func() {
				devloop.Delete(test.args...).InDir(tmpDir.Root()).RunOrFail(t.T)
			})
