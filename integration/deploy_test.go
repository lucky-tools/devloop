...
import (
	...
	"github.com/lucky-tools/devloop/pkg/devloop/walk"
...
func TestBuildDeploy(t *testing.T) {
	...
	outputBytes := devloop.Build("--quiet", "--platform=linux/arm64,linux/amd64", "--cache-artifacts=false", "--tag", uuid.New().String()).InDir("examples/nodejs").InNs(ns.Name).RunOrFailOutput(t)
	...
	// See https://github.com/lucky-tools/devloop/issues/2372 on why status-check=false
	devloop.Deploy("--build-artifacts", buildOutputFile, "--status-check=false").InDir("examples/nodejs").InNs(ns.Name).RunOrFail(t)
...
func TestDeploy(t *testing.T) {
	...
	devloop.Deploy("--images", "index.docker.io/library/busybox:1", "--default-repo=").InDir("examples/kustomize").InNs(ns.Name).RunOrFail(t)
...
func TestDeployWithBuildArtifacts(t *testing.T) {
	...
	devloop.Build("--file-output=images.json", "--default-repo=").InDir("examples/getting-started").RunOrFail(t)

	// `--default-repo=` is used to cancel the default repo that is set by default.
	devloop.Deploy("--build-artifacts=images.json", "--default-repo=", "--load-images=true").InDir("examples/getting-started").InNs(ns.Name).RunOrFail(t)

	pod := client.GetPod("getting-started")
	testutil.CheckContains(t, "devloop-example", pod.Spec.Containers[0].Image)
}
...
func TestDeployWithImages(t *testing.T) {
	...
	// first build the artifacts and output to file
	devloop.Build("--file-output=artifacts.json", "--default-repo=").InDir("examples/getting-started").RunOrFail(t)
	...
	// `--default-repo=` is used to cancel the default repo that is set by default.
	devloop.Deploy("--images="+strings.Join(images, ","), "--default-repo=", "--load-images=true").InDir("examples/getting-started").InNs(ns.Name).RunOrFail(t)
...
func TestDeployTail(t *testing.T) {
	...
	// `--default-repo=` is used to cancel the default repo that is set by default.
	out := devloop.Deploy("--tail", "--images", "busybox:latest", "--default-repo=").InDir("testdata/deploy-hello-tail").InNs(ns.Name).RunLive(t)
...
func TestDeployTailDefaultNamespace(t *testing.T) {
	...
	// `--default-repo=` is used to cancel the default repo that is set by default.
	out := devloop.Deploy("--tail", "--images", "busybox:latest", "--default-repo=").InDir("testdata/deploy-hello-tail").RunLive(t)

	defer devloop.Delete().InDir("testdata/deploy-hello-tail").Run(t)
	WaitForLogs(t, out, "Hello world!")
}
...
func TestDeployWithInCorrectConfig(t *testing.T) {
	...
	output, err := devloop.Deploy().InDir("examples/getting-started").InNs(ns.Name).RunWithCombinedOutput(t)
	if err == nil {
		t.Errorf("expected to see an error since not every image tag is provided: %s", output)
	} else if !strings.Contains(string(output), "no tag provided for image [devloop-example]") {
		t.Errorf("failed without saying the reason: %s", output)
	}
}

// Verify that we can deploy without artifact details (https://github.com/lucky-tools/devloop/issues/4616)
func TestDeployWithoutWorkspaces(t *testing.T) {
	MarkIntegrationTest(t, NeedsGcp)

	ns, _ := SetupNamespace(t)

	outputBytes := devloop.Build("--quiet").InDir("examples/nodejs").InNs(ns.Name).RunOrFailOutput(t)
	// Parse the Build Output
	buildArtifacts, err := flags.ParseBuildOutput(outputBytes)
	failNowIfError(t, err)
	...
	buildOutputFile := tmpDir.Path("build.out")
	tmpDir.Write("build.out", string(outputBytes))
	copyFiles(tmpDir.Root(), "examples/nodejs/devloop.yaml")
	copyFiles(tmpDir.Root(), "examples/nodejs/k8s")

	// Run Deploy using the build output
	// See https://github.com/lucky-tools/devloop/issues/2372 on why status-check=false
	devloop.Deploy("--build-artifacts", buildOutputFile, "--status-check=false").InDir(tmpDir.Root()).InNs(ns.Name).RunOrFail(t)
}
...
func TestDeployDependenciesOrder(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			...
			ns, _ := SetupNamespace(t)
			outputBytes := devloop.Run(targetModule...).InDir(test.dir).InNs(ns.Name).RunOrFailOutput(t)
			defer devloop.Delete().InDir(test.dir).InNs(ns.Name).RunOrFail(t)
