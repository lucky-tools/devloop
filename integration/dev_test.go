...
import (
	...
	V2proto "github.com/lucky-tools/devloop/proto/v2"
...
func TestDevNotification(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			...
			rpcAddr := randomPort()
			devloop.Dev("--rpc-port", rpcAddr, "--trigger", test.trigger).InDir("testdata/dev").InNs(ns.Name).RunBackground(t)
...
func TestDevGracefulCancel(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			ns, client := SetupNamespace(t)
			p, _ := devloop.Dev("-vtrace").InDir(test.dir).InNs(ns.Name).StartWithProcess(t)
...
func TestDevCancelWithDockerDeployer(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			MarkIntegrationTest(t, CanRunWithoutGcp)
			p, err := devloop.Dev().InDir(test.dir).StartWithProcess(t)
			if err != nil {
				t.Fatalf("error starting devloop dev process")
			}
...
func TestDevAPIBuildTrigger(t *testing.T) {
	...
	rpcAddr := randomPort()
	devloop.Dev("--auto-build=false", "--auto-sync=false", "--auto-deploy=false", "--rpc-port", rpcAddr, "--cache-artifacts=false").InDir("testdata/dev").InNs(ns.Name).RunBackground(t)
...
func TestDevApiDeployTrigger(t *testing.T) {
	...
	rpcAddr := randomPort()
	devloop.Dev("--auto-deploy=false", "--rpc-port", rpcAddr, "--cache-artifacts=false").InDir("testdata/dev").InNs(ns.Name).RunBackground(t)
...
func TestDevPortForward(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			rpcAddr := randomPort()
			devloop.Dev("--status-check=false", "--port-forward", "--rpc-port", rpcAddr).InDir(test.dir).InNs(ns.Name).RunBackground(t)
...
func TestDevPortForwardDefaultNamespace(t *testing.T) {
	...
	// Run devloop build first to fail quickly on a build failure
	devloop.Build().InDir("examples/microservices").RunOrFail(t)

	rpcAddr := randomPort()
	devloop.Dev("--status-check=false", "--port-forward", "--rpc-port", rpcAddr).InDir("examples/microservices").RunBackground(t)
	defer devloop.Delete().InDir("examples/microservices").Run(t)
	_, entries := apiEvents(t, rpcAddr)

	// No namespace was provided to `devloop dev`, so we assume "default"
...
func TestDevPortForwardGKELoadBalancer(t *testing.T) {
	...
	// Run devloop build first to fail quickly on a build failure
	devloop.Build().InDir("testdata/gke_loadbalancer").RunOrFail(t)

	ns, _ := SetupNamespace(t)

	rpcAddr := randomPort()
	env := []string{fmt.Sprintf("TEST_NS=%s", ns.Name)}
	devloop.Dev("--port-forward", "--rpc-port", rpcAddr).InDir("testdata/gke_loadbalancer").InNs(ns.Name).WithEnv(env).RunBackground(t)
...
func TestDev_WithKubecontextOverride(t *testing.T) {
	...
	testutil.Run(t, "devloop run with kubecontext override", func(t *testutil.T) {
		...
		devloop.Run("--kube-context", kubecontext).InDir("examples/getting-started").WithEnv(env).InNs(ns.Name).RunOrFail(t.T)
