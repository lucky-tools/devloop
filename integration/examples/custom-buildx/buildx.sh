...
#
#  (1) delete the buildx builder named `devloop-builder`, and
...
# This script creates a `devloop-builder` as required.
if ! docker buildx inspect devloop-builder >/dev/null 2>&1; then
  docker buildx create --name devloop-builder --platform $PLATFORMS
fi
...
docker buildx build --builder devloop-builder --tag $IMAGE $args "$BUILD_CONTEXT"
