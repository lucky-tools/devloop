# Devloop Developer Journey with Buildpacks Tutorial

## Introduction

### What is this project?

Devloop allows developers to easily transition from local development on minikube to remote development on an enterprise Kubernetes cluster managed by IT. During the transition from local to remote deployment, a security team might ask a developer to patch a library with a specific vulnerability in it. This is where Devloop's support for buildpacks comes in handy. In this tutorial, you'll start out deploying an application locally, swap out buildpacks in the **devloop.yaml** file to use the latest libraries, and then deploy the application to a remote Kubernetes cluster.

For a guided Cloud Shell tutorial on how a developer's journey might look in adopting Devloop, follow:

[![Open in Cloud Shell](https://gstatic.com/cloudssh/images/open-btn.svg)](https://ssh.cloud.google.com/cloudshell/editor?cloudshell_git_repo=https://github.com/lucky-tools/devloop&cloudshell_workspace=examples/dev-journey-buildpacks&cloudshell_tutorial=tutorial.md)
...
### Create a Project
Create a project called **devloop-tutorial** with a valid billing account in the Google Cloud [Manage Resources Console](https://console.cloud.google.com/cloud-resource-manager). In the shell run:
```bash
gcloud config set project devloop-tutorial
```
...
* Install the latest version of devloop. Although Devloop is already installed on cloud shell, we'll install the latest version for good measure.
```bash
...
## Run the App to Minikube Using Devloop

### Deploy the App
Start devloop in development mode which will constantly monitor the current directory for changes and kick off a new build and deploy whenever changes are detected.
```bash
devloop dev
```
...
```
Change the return line to **return "Hello, Devloop!"**.
...
## Deploy the App to Enterprise GKE Cluster Using Devloop
...
```bash
devloop dev --default-repo=gcr.io/$(gcloud config get-value project)
```
...
I hope this tutorial was informative. Good luck on your journey with Devloop!