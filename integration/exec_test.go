...
func TestExec_LocalActions(t *testing.T) {
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			out, err := devloop.Exec(args...).InDir("testdata/custom-actions-local").RunWithCombinedOutput(t.T)
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			if test.shouldBuild {
				...
				devloop.Build("--file-output", tmpfile).InDir(dir).RunOrFail(t.T)
				args = append(args, "--build-artifacts", tmpfile)
			}

			out, err := devloop.Exec(args...).InDir(dir).RunWithCombinedOutput(t.T)
			t.CheckError(test.shouldErr, err)
...
func TestExec_ActionsEvents(t *testing.T) {
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			_, err := devloop.Exec(args...).InDir("testdata/custom-actions-local").RunWithCombinedOutput(t.T)
