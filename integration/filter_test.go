...
func TestFilterPassthrough(t *testing.T) {
	...
	testutil.Run(t, "no filters should just pass through", func(t *testutil.T) {
		transformedOutput := devloop.Filter().InDir("examples/getting-started").WithStdin(renderedOutput).RunOrFailOutput(t.T)
	...
	testutil.Run(t, "--build-artifacts=file with no filters should just pass through", func(t *testutil.T) {
		...
		transformedOutput := devloop.Filter("--build-artifacts=" + buildFile).InDir("examples/getting-started").WithStdin(renderedOutput).RunOrFailOutput(t.T)
