...
func TestFixExclusiveOptions(t *testing.T) {
	...
	out := devloop.Fix().InDir("testdata/fix").RunOrFailOutput(t)
	out, err := devloop.Fix("--overwrite", "--output=ignored").WithConfig("-").InDir("testdata/fix").
...
func TestFixStdout(t *testing.T) {
	...
	out := devloop.Fix().InDir("testdata/fix").RunOrFailOutput(t)
	devloop.Run().WithConfig("-").InDir("testdata/fix").InNs(ns.Name).WithStdin(out).RunOrFail(t)
}

func TestFixV2Beta29ToV3Alpha1PatchProfiles(t *testing.T) {
	MarkIntegrationTest(t, CanRunWithoutGcp)

	out, err := devloop.Fix().InDir("testdata/fix-v2beta29").RunWithCombinedOutput(t)
	testutil.CheckError(t, false, err)
	testutil.CheckContains(t, "/manifests/helm/releases/0/setValueTemplates/image.repository", string(out))
	testutil.CheckContains(t, "{{.IMAGE_REPO_devloop_helm_v2}}", string(out))
	testutil.CheckContains(t, "/manifests/helm/releases/0/setValueTemplates/image.tag", string(out))
	testutil.CheckContains(t, "{{.IMAGE_TAG_devloop_helm_v2}}@{{.IMAGE_DIGEST_devloop_helm_v2}}", string(out))
}

func TestFixOutputFile(t *testing.T) {
	MarkIntegrationTest(t, CanRunWithoutGcp)

	out := devloop.Fix("--output", "updated.yaml").InDir("testdata/fix").RunOrFailOutput(t)
