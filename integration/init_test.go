...
func TestInit(t *testing.T) {
	tests := []struct {
		...
		/*
			// Fix after https://github.com/lucky-tools/devloop/issues/6722
				{
					name: "compose",
	...
	for _, test := range tests {
		testutil.Run(t, test.name, func(t *testutil.T) {
			...
			devloop.Init(initArgs...).InDir(test.dir).WithConfig("devloop.yaml.out").RunOrFail(t.T)
			checkGeneratedConfig(t, test.dir)

			// Make sure the devloop yaml and the kubernetes manifests created by kompose are ok
			devloop.Run().InDir(test.dir).WithConfig("devloop.yaml.out").InNs(ns.Name).RunOrFail(t.T)
			defer devloop.Delete().InDir(test.dir).WithConfig("devloop.yaml.out").InNs(ns.Name)
		})
	}
}

func TestInitManifestGeneration(t *testing.T) {
	t.Skipf("Fix after https://github.com/lucky-tools/devloop/issues/6722")
	...
	for _, test := range tests {
		testutil.Run(t, test.name, func(t *testutil.T) {
			...
			// Make sure the devloop yaml and the kubernetes manifests created by kompose are ok
			devloop.Run().InDir(test.dir).WithConfig("devloop.yaml.out").InNs(ns.Name).RunOrFail(t.T)
			devloop.Delete().InDir(test.dir).WithConfig("devloop.yaml.out").InNs(ns.Name).RunOrFail(t.T)
		})
	}
}
...
func TestInitKustomize(t *testing.T) {
	...
	testutil.Run(t, "kustomize init", func(t *testutil.T) {
		...
		defer func() {
			path := filepath.Join(dir, "devloop.yaml.out")
			_, err := os.Stat(path)
		...
		devloop.Init(initArgs...).InDir(dir).WithConfig("devloop.yaml.out").RunOrFail(t.T)

		checkGeneratedConfig(t, dir)

		devloop.Run().InDir(dir).WithConfig("devloop.yaml.out").InNs(ns.Name).RunOrFail(t.T)
		devloop.Delete().InDir(dir).WithConfig("devloop.yaml.out").InNs(ns.Name).RunOrFail(t.T)
	})
}
...
func TestInitWithCLIArtifact(t *testing.T) {
	...
	testutil.Run(t, "init with cli artifact", func(t *testutil.T) {
		...
		// Make sure the devloop yaml is ok
		devloop.Run().InDir(dir).WithConfig("devloop.yaml.out").InNs(ns.Name).RunOrFail(t.T)
		devloop.Delete().InDir(dir).WithConfig("devloop.yaml.out").InNs(ns.Name).RunOrFail(t.T)
	})
}
...
func checkGeneratedConfig(t *testutil.T, dir string) {
	expectedOutput, err := os.ReadFile(filepath.Join(dir, "devloop.yaml"))
	t.CheckNoError(err)

	output, err := os.ReadFile(filepath.Join(dir, "devloop.yaml.out"))
	t.CheckNoError(err)
...
func TestInitFailures(t *testing.T) {
	...
	testutil.Run(t, "no builder", func(t *testutil.T) {
		out, err := devloop.Init().InDir("testdata/init/no-builder").RunWithCombinedOutput(t.T)
...
func TestInitWithDirWithoutReadPerms(t *testing.T) {
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			os.MkdirAll(dirWithoutReadPerms, 0o377)
			defer os.Remove(dirWithoutReadPerms)

			output, err := devloop.Init(test.flags...).InDir(tmpDir.Root()).RunWithCombinedOutput(t.T)
