...
const (
	defaultRepo       = "us-central1-docker.pkg.dev/k8s-devloop/testing"
...
func TestMultiPlatformWithRun(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			images:            []image{{name: "devloop-example", pod: "getting-started"}},
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			...
			devloop.Run(args...).InDir(test.dir).InNs(ns.Name).RunOrFail(t)
...
func TestMultiplatformWithDevAndDebug(t *testing.T) {
	...
	tests := []struct {
		...
		images            []image
		tag               string
		command           func(args ...string) *devloop.RunBuilder
		expectedPlatforms []v1.Platform
	}{
		{
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			...
			defer devloop.Delete().InDir(test.dir).InNs(ns.Name).Run(t)
...
func TestMultiplatformWithDeploy(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			...
			devloop.Build(argsBuild...).InDir(test.dir).RunOrFail(t)
			ns, client := SetupNamespace(t)
			devloop.Deploy(argsDeploy...).InDir(test.dir).InNs(ns.Name).RunOrFail(t)
			defer devloop.Delete().InDir(test.dir).InNs(ns.Name).RunOrFail(t)
