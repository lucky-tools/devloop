...
import (
	...
	"github.com/lucky-tools/devloop/pkg/devloop/render"
	"github.com/lucky-tools/devloop/pkg/devloop/render/renderer/kubectl"
...
func TestKubectlRenderOutput(t *testing.T) {
	...
	test := struct {
		...
		builds: []graph.Artifact{
			{
				ImageName: "us-central1-docker.pkg.dev/k8s-devloop/testing/devloop",
				Tag:       "us-central1-docker.pkg.dev/k8s-devloop/testing/devloop:test",
			},
		},
		input: `apiVersion: v1
kind: Pod
spec:
  containers:
  - image: us-central1-docker.pkg.dev/k8s-devloop/testing/devloop
    name: devloop
`,
...
spec:
  containers:
  - image: us-central1-docker.pkg.dev/k8s-devloop/testing/devloop:test
    name: devloop`, ns.Name),
	}
...
metadata:
  labels:
    ...
    devloop.dev/run-id: phony-run-id
...
metadata:
  labels:
    app: devloop-helm
    chart: devloop-helm-0.1.0
    heritage: Helm
    release: devloop-helm
    devloop.dev/run-id: phony-run-id
  name: devloop-helm-devloop-helm
spec:
  ports:
  - name: nginx
    port: 80
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - image: us-central1-docker.pkg.dev/k8s-devloop/testing/devloop-helm:sha256-nonsenselettersandnumbers
...
`,
		...
		{
			description:      "Template with Release.namespace set from devloop.yaml file deploy.helm.releases.namespace",
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - image: devloop-helm:latest
        name: devloop-helm
`,
		},
		{
			description:      "Template with Release.namespace set from devloop.yaml file manifests.helm.releases.namespace",
...
`,
		...
		{
			description:      "Template with Release.namespace set from devloop.yaml file manifests.helm.releases.namespace and deploy.helm.releases.namespace",
...
`,
		...
		{
			description:      "Template with Release.namespace set from devloop.yaml file deploy.helm.releases.namespace - v1 devloop schema",
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - image: us-central1-docker.pkg.dev/k8s-devloop/testing/devloop-helm:latest
        imagePullPolicy: always
        name: devloop-helm
        ports:
        - containerPort: 80
...
`,
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			...
			args := []string{"--default-repo=", "--label=devloop.dev/run-id=phony-run-id"}
			if !test.withoutBuildJSON {
				args = append(args, "--build-artifacts=builds.out.json")
			}
			args = append(args, test.args...)

			out := devloop.Render(args...).InDir(test.dir).RunOrFailOutput(t)
...
`,
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			MarkIntegrationTest(t.T, CanRunWithoutGcp)
			tmpDir := t.NewTempDir()
			tmpDir.Write("devloop.yaml", test.config)
			...
			if test.offline {
				...
				devloop.Render(args...).WithEnv(env).RunOrFail(t.T)
			} else {
				devloop.Render(args...).RunOrFail(t.T)
			}
			...
			fileContentReplaced := regexp.MustCompile("(?m)(devloop.dev/run-id|devloop.dev/docker-api-version): .+$").ReplaceAll(fileContent, []byte("$1: SOMEDYNAMICVALUE"))
...
kind: Deployment
metadata:
  name: devloop-kustomize
  labels:
    app: devloop-kustomize # from-param: ${app1}
spec:
  selector:
    matchLabels:
      app: devloop-kustomize
  template:
    metadata:
      labels:
        app: devloop-kustomize
...
`,
				"charts/Chart.yaml": `
apiVersion: v1
description: Devloop example with Helm
name: devloop-helm
version: 0.1.0
`,
				"charts/values.yaml": `
...
`,
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			tmpDir.Chdir()
			output := devloop.Render(test.args...).RunOrFailOutput(t.T)
...
spec:
  containers:
  - image: gcr.io/k8s-devloop/devloop-example:customtag
    name: getting-started
`,
		},
		{
...
spec:
  containers:
  - image: gcr.io/k8s-devloop/multi-config-module1:customtag
    name: module1
---
apiVersion: v1
kind: Pod
...
spec:
  containers:
  - image: gcr.io/k8s-devloop/multi-config-module2:customtag
    name: module2
`,
		},
		{
...
`,
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			args := []string{"--tag", "customtag", "--default-repo", "gcr.io/k8s-devloop"}

			if test.namespaceFlag != "" {
				args = append(args, "--namespace", test.namespaceFlag)
			}

			output := devloop.Render(args...).InDir(test.projectDir).RunOrFailOutput(t.T)
...
spec:
  containers:
  - image: us-central1-docker.pkg.dev/k8s-devloop/testing/multi-config-module1:customtag
    name: module1
`,
		},
		{
...
spec:
  containers:
    - image: us-central1-docker.pkg.dev/k8s-devloop/testing/multi-config-module2:customtag
      name: module2
`,
		},
		{
...
Completed post-render hooks`,
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			devloop.Render(test.args...).InDir(test.projectDir).RunWithStdoutAndStderrOrFail(t.T, stdout, stderr)
...
`,
			input: map[string]string{
				"set-annotations/Kptfile": `apiVersion: kpt.dev/v1
kind: Kptfile
metadata:
  name: example
pipeline:
...
`,
			input: map[string]string{
				"apply-simple/Kptfile": `apiVersion: kpt.dev/v1
kind: Kptfile
metadata:
  name: apply-setters-simple
upstream:
...
func TestHelmRenderWithImagesFlag(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			dir:         "testdata/helm-render",
			args:        []string{"--profile=helm-render", "--images=us-central1-docker.pkg.dev/k8s-devloop/testing/devloop-helm:latest@sha256:3e8981b13fadcbb5f4d42d00fdf52a9de128feea5280f0a1f7fb542cf31f1a06"},
			expectedOut: `apiVersion: v1
kind: Service
metadata:
  labels:
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - image: us-central1-docker.pkg.dev/k8s-devloop/testing/devloop-helm:latest@sha256:3e8981b13fadcbb5f4d42d00fdf52a9de128feea5280f0a1f7fb542cf31f1a06
        imagePullPolicy: always
        name: devloop-helm
        ports:
        - containerPort: 80
...
`,
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			MarkIntegrationTest(t, CanRunWithoutGcp)
			out := devloop.Render(append([]string{"--default-repo=", "--label=devloop.dev/run-id=phony-run-id"}, test.args...)...).InDir(test.dir).RunOrFailOutput(t)
...
func TestRenderRemoteKustomize(t *testing.T) {
	testutil.Run(t, "render remote kustomize resource success", func(t *testutil.T) {
		MarkIntegrationTest(t.T, CanRunWithoutGcp)
		devloop.Render().InDir("testdata/kustomize/remote").RunOrFail(t.T)
	})
}
