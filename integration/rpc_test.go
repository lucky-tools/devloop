...
import (
	...
	event "github.com/lucky-tools/devloop/pkg/devloop/event/v2"
	"github.com/lucky-tools/devloop/proto/v1"
	protoV2 "github.com/lucky-tools/devloop/proto/v2"
...
func TestEventsRPC(t *testing.T) {
	...
	rpcAddr := randomPort()
	setupDevloopWithArgs(t, "--rpc-port", rpcAddr, "--status-check=false")

	// start a grpc client and make sure we can connect properly
	var (
		conn   *grpc.ClientConn
		err    error
		client proto.DevloopServiceClient
	)

	// connect to the devloop grpc server
	for i := 0; i < connectionRetries; i++ {
		conn, err = grpc.Dial(fmt.Sprintf(":%s", rpcAddr), grpc.WithInsecure())
		if err != nil {
			t.Logf("unable to establish devloop grpc connection: retrying...")
			time.Sleep(waitTime)
			continue
		}
		defer conn.Close()

		client = proto.NewDevloopServiceClient(conn)
		break
	}

	if client == nil {
		t.Fatalf("error establishing devloop grpc connection")
	}

	ctx, ctxCancel := context.WithCancel(context.Background())
	defer ctxCancel()

	// read the event log stream from the devloop grpc server
	var stream proto.DevloopService_EventsClient
...
func TestEventLogHTTP(t *testing.T) {
	tests := []struct {
		...
		{
			// TODO deprecate (https://github.com/lucky-tools/devloop/issues/3168)
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			MarkIntegrationTest(t, CanRunWithoutGcp)
			httpAddr := randomPort()
			setupDevloopWithArgs(t, "--rpc-http-port", httpAddr, "--status-check=false")
			time.Sleep(500 * time.Millisecond) // give devloop time to process all events
...
func TestGetStateRPC(t *testing.T) {
	...
	rpcAddr := randomPort()
	// start a devloop dev loop on an example
	setupDevloopWithArgs(t, "--rpc-port", rpcAddr)
	...
	if !success {
		t.Errorf("devloop build or deploy not complete. state: %+v\n", grpcState)
	}
}
...
func TestGetStateHTTP(t *testing.T) {
	...
	httpAddr := randomPort()
	setupDevloopWithArgs(t, "--rpc-http-port", httpAddr)
	time.Sleep(3 * time.Second) // give devloop time to process all events
...
func retrieveRPCState(ctx context.Context, t *testing.T, client proto.DevloopServiceClient) (state *proto.State) {
	var err error
...
func setupDevloopWithArgs(t *testing.T, args ...string) {
	Run(t, "testdata/dev", "sh", "-c", "echo foo > foo")

	// Run devloop build first to fail quickly on a build failure
	devloop.Build().InDir("testdata/dev").RunOrFail(t)

	// start a devloop dev loop on an example
	ns, _ := SetupNamespace(t)

	devloop.Dev(append([]string{"--cache-artifacts=false"}, args...)...).InDir("testdata/dev").InNs(ns.Name).RunBackground(t)
...
func apiEvents(t *testing.T, rpcAddr string) (proto.DevloopServiceClient, chan *proto.LogEntry) { // nolint
...
func readEventAPIStream(client proto.DevloopServiceClient, t *testing.T, retries int) (proto.DevloopService_EventLogClient, error) {
	t.Helper()
	// read the event log stream from the devloop grpc server
	var stream proto.DevloopService_EventLogClient
	var err error
	for i := 0; i < retries; i++ {
...
func setupRPCClient(t *testing.T, port string) proto.DevloopServiceClient {
	// start a grpc client
	var (
		conn   *grpc.ClientConn
		err    error
...
func setupV2RPCClient(t *testing.T, port string) protoV2.DevloopV2ServiceClient {
	// start a grpc client
	var (
		conn   *grpc.ClientConn
		err    error
		client protoV2.DevloopV2ServiceClient
	)
	...
	for i := 0; i < connectionRetries; i++ {
		...
		client = protoV2.NewDevloopV2ServiceClient(conn)
		break
	}
...
func readV2EventAPIStream(client protoV2.DevloopV2ServiceClient, t *testing.T, retries int) (protoV2.DevloopV2Service_EventsClient, error) {
	t.Helper()
	// read the event log stream from the devloop grpc server
	var stream protoV2.DevloopV2Service_EventsClient
	var err error
...
func v2apiEvents(t *testing.T, rpcAddr string) (protoV2.DevloopV2ServiceClient, chan *protoV2.Event) { // nolint
