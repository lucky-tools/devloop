...
// `devloop-builder` builder configured and a registry to push to.
...
var tests = []struct {
	...
	},
	{
		...
		// See https://github.com/lucky-tools/devloop/issues/2372
	...
	},
	{
		...
		deployments: []string{"devloop-helm"},
		env:         []string{"FOO=devloop-helm"},
	},
	...
	// },
	{
		...
		deployments: []string{"devloop-kustomize-dev"},
		targetLog:   "Hello world!",
	},
	{
		description: "helm",
...
func TestRunTail(t *testing.T) {
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			...
			out := devloop.Run(args...).InDir(test.dir).InNs(ns.Name).WithEnv(test.env).RunLive(t)
...
func TestRunTailDefaultNamespace(t *testing.T) {
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			...
			args := append(test.args, "--tail")
			out := devloop.Run(args...).InDir(test.dir).WithEnv(test.env).RunLive(t)
			defer devloop.Delete().InDir(test.dir).WithEnv(test.env).RunOrFail(t)
			WaitForLogs(t, out, test.targetLog)
		})
	}
}
...
func TestRunGCPOnly(t *testing.T) {
	...
	for _, test := range tests {
		...
		t.Run(test.description, func(t *testing.T) {
			...
			devloop.Run(test.args...).InDir(test.dir).InNs(ns.Name).RunOrFail(t)

			client.WaitForPodsReady(test.pods...)
			client.WaitForDeploymentsToStabilize(test.deployments...)

			devloop.Delete().InDir(test.dir).InNs(ns.Name).RunOrFail(t)
		})
	}
}
...
func TestRunIdempotent(t *testing.T) {
	...
	// The first `devloop run` creates resources (deployment.apps/leeroy-web, service/leeroy-app, deployment.apps/leeroy-app)
	out := devloop.Run("-l", "devloop.dev/run-id=notunique").InDir("examples/microservices").InNs(ns.Name).RunOrFailOutput(t)
	firstOut := string(out)
	if strings.Count(firstOut, "created") == 0 {
		t.Errorf("resources should have been created: %s", firstOut)
	}

	// Because we use the same custom `run-id`, the second `devloop run` is idempotent:
...
func TestRunUnstableChecked(t *testing.T) {
	...
	output, err := devloop.Run().InDir("testdata/unstable-deployment").InNs(ns.Name).RunWithCombinedOutput(t)
	if err == nil {
...
func TestRunUnstableNotChecked(t *testing.T) {
	...
	devloop.Run("--status-check=false").InDir("testdata/unstable-deployment").InNs(ns.Name).RunOrFail(t)
}
...
func TestRunTailPod(t *testing.T) {
	...
	out := devloop.Run("--tail", "-p", "pod").InDir("testdata/hello").InNs(ns.Name).RunLive(t)
...
func TestRunTailDeployment(t *testing.T) {
	...
	out := devloop.Run("--tail", "-p", "deployment").InDir("testdata/hello").InNs(ns.Name).RunLive(t)
...
func TestRunTest(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			...
			devloop.Build().InDir(test.testDir).RunOrFail(t)

			ns, client := SetupNamespace(t)
			devloop.Run(test.args...).InDir(test.testDir).InNs(ns.Name).RunBackground(t)
...
func TestRunNoOptFlags(t *testing.T) {
	...
	t.Run(test.description, func(t *testing.T) {
		...
		args := append(test.args, "--tail")
		out := devloop.Run(args...).InDir(test.dir).InNs(ns.Name).RunLive(t)
...
func TestRunKubectlDefaultNamespace(t *testing.T) {
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			devloop.Run().InDir(test.projectDir).RunOrFail(t.T)
