...
func TestDevSync(t *testing.T) {
	for _, test := range syncTests {
		t.Run(test.description, func(t *testing.T) {
			...
			devloop.Dev("--rpc-port", rpcAddr, "--trigger", test.trigger).InDir("testdata/file-sync").WithConfig(test.config).InNs(ns.Name).RunBackground(t)
			...
			os.WriteFile("testdata/file-sync/foo", []byte("foo"), 0o644)
...
func TestDevSyncDefaultNamespace(t *testing.T) {
	for _, test := range syncTests {
		t.Run(test.description, func(t *testing.T) {
			...
			defer os.WriteFile("testdata/file-sync/pod.yaml", manifest, 0o644)
			if err != nil {
...
- op: replace
  ...
  value: ` + id)
			...
			os.WriteFile("testdata/file-sync/pod.yaml", nm, 0o644)

			_, client := DefaultNamespace(t)

			rpcAddr := randomPort()

			devloop.Dev("--rpc-port", rpcAddr, "--trigger", test.trigger).InDir("testdata/file-sync").WithConfig(test.config).RunBackground(t)

			defer devloop.Delete().InDir("testdata/file-sync").WithConfig(test.config).Run(t)
...
func TestDevAutoSync(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			configFile:  "devloop-maven.yaml",
			uniqueStr:   "maven-maven",
		},
		{
			description: "jib gradle auto sync",
			configFile:  "devloop-gradle.yaml",
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			...
			rpcAddr := randomPort()
			output := devloop.Dev("--trigger", "notify", "--rpc-port", rpcAddr).WithConfig(test.configFile).InDir(dir).InNs(ns.Name).RunLive(t)
			...
			for scanner.Scan() {
				...
				if strings.Contains(line, "Started Application") {
					err := output.Close()
					if err != nil {
						t.Fatal("failed to close devloop dev output reader during test")
					}
					return
				}
			}
			...
			if err := os.WriteFile(directFilePath, []byte(directFileData), 0o644); err != nil {
			...
			if oldContents, err := os.ReadFile(generatedFileSrc); err != nil {
				...
				if err := os.WriteFile(generatedFileSrc, []byte(newContents), 0o644); err != nil {
					t.Fatalf("Failed to write new contents to file %s", generatedFileSrc)
				}
				defer func() {
					os.WriteFile(generatedFileSrc, oldContents, 0o644)
				}()
			}
			err = wait.PollImmediate(time.Millisecond*500, 1*time.Minute, func() (bool, error) {
...
func TestDevSyncAPITrigger(t *testing.T) {
	...
	devloop.Build().InDir("testdata/file-sync").WithConfig("devloop-manual.yaml").InNs(ns.Name).RunOrFail(t)

	rpcAddr := randomPort()
	devloop.Dev("--auto-sync=false", "--rpc-port", rpcAddr).InDir("testdata/file-sync").WithConfig("devloop-manual.yaml").InNs(ns.Name).RunBackground(t)
...
func TestDevAutoSyncAPITrigger(t *testing.T) {
	...
	os.WriteFile("testdata/file-sync/foo", []byte("bar"), 0o644)
	defer func() { os.Truncate("testdata/file-sync/foo", 0) }()
