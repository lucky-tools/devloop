...
func TestTestEvents(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			...
			devloop.Build(test.args...).InDir(test.testDir).WithConfig(test.config).RunOrFail(t)
			...
			devloop.Dev(args...).InDir(test.testDir).WithConfig(test.config).InNs(ns.Name).RunLive(t)
