...
func TestBuildAndTest(t *testing.T) {
	...
	outputBytes := devloop.Build("--quiet").InDir("examples/structure-tests").InNs(ns.Name).RunOrFailOutput(t)
	...
	var devloopExampleTag string
	for _, a := range buildArtifacts.Builds {
		if a.ImageName == "devloop-example" {
			devloopExampleTag = a.Tag
		}
	}
	if devloopExampleTag == "" {
		t.Fatalf("expected to find a tag for devloop-example but found %s", devloopExampleTag)
	}
	...
	devloop.Test("--build-artifacts", buildOutputFile).InDir("examples/structure-tests").InNs(ns.Name).RunOrFail(t)
}
...
func TestTestWithInCorrectConfig(t *testing.T) {
	...
	output, err := devloop.Test().InDir("examples/getting-started").InNs(ns.Name).RunWithCombinedOutput(t)
