...
func TestLocalVerifyPassingTestsWithEnvVar(t *testing.T) {
	...
	out, err := devloop.Verify("--default-repo=", "--rpc-port", rpcPort,
...
func TestVerifyWithNotCreatedNetwork(t *testing.T) {
	MarkIntegrationTest(t, CanRunWithoutGcp)
	// `--default-repo=` is used to cancel the default repo that is set by default.
	logs, err := devloop.Verify("--default-repo=", "--env-file", "verify.env", "--docker-network", "not-created-network").InDir("testdata/verify-succeed").RunWithCombinedOutput(t)
...
func TestVerifyNoTestsFails(t *testing.T) {
	MarkIntegrationTest(t, CanRunWithoutGcp)
	// `--default-repo=` is used to cancel the default repo that is set by default.
	out, err := devloop.Verify("--default-repo=").InDir("testdata/verify-no-tests").RunWithCombinedOutput(t)
	logs := string(out)
...
func TestNoDuplicateLogsLocal(t *testing.T) {
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			out, err := devloop.Verify(args...).InDir(test.dir).RunWithCombinedOutput(t.T)
...
func TestNoDuplicateLogsK8SJobs(t *testing.T) {
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			out, err := devloop.Verify(args...).InNs(ns.Name).InDir(test.dir).RunWithCombinedOutput(t.T)
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			if test.shouldBuild {
				...
				devloop.Build(append(args, "--file-output", tmpfile)...).InDir(test.dir).RunOrFail(t.T)
				args = append(args, "--build-artifacts", tmpfile)
			}

			out, err := devloop.Verify(args...).InDir(test.dir).InNs(ns.Name).RunWithCombinedOutput(t.T)
			logs := string(out)
