...
func TestMakeAuthSuggestionsForRepo(t *testing.T) {
	...
	}, makeAuthSuggestionsForRepo("us-docker.pkg.dev/k8s-devloop/devloop"), protocmp.Transform())
}
...
func TestBuildProblems(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			err:         fmt.Errorf("devloop build failed: could not push image: denied: push access to resource"),
			...
			expectedAE: &proto.ActionableErr{
				...
				Message: "devloop build failed: could not push image: denied: push access to resource",
		...
		},
		{
			...
			optRepo:     "gcr.io/test",
			err:         fmt.Errorf("devloop build failed: could not push image image1 : denied: push access to resource"),
			...
			expectedAE: &proto.ActionableErr{
				ErrCode: proto.StatusCode_BUILD_PUSH_ACCESS_DENIED,
				Message: "devloop build failed: could not push image image1 : denied: push access to resource",
				Suggestions: []*proto.Suggestion{
					{
		...
		},
		{
			...
			err:         fmt.Errorf("devloop build failed: could not push image: denied: push access to resource"),
			expected:    "Build Failed. No push access to specified image repository. Check your default-repo setting in devloop config or try `docker login docker.io`.",
			expectedAE: &proto.ActionableErr{
				ErrCode: proto.StatusCode_BUILD_PUSH_ACCESS_DENIED,
				Message: "devloop build failed: could not push image: denied: push access to resource",
				Suggestions: []*proto.Suggestion{
					{
						...
						Action:         "Check your default-repo setting in devloop config",
					}, {
						SuggestionCode: proto.SuggestionCode_DOCKER_AUTH_CONFIGURE,
