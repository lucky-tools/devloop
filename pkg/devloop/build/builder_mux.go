...
// useBuildKitEnabled reports whether any pipeline enables buildkit via `useBuildKit: true`.
func useBuildKitEnabled(cfg Config) bool {
	for _, p := range cfg.GetPipelines() {
		lb := p.Build.LocalBuild
		if lb == nil {
			continue
		}
		if (lb.CLI != nil && lb.CLI.UseBuildKit) || (lb.API != nil && lb.API.UseBuildKit) {
			return true
		}
	}
	return false
...
func NewBuilderMux(cfg Config, store ArtifactStore, cache Cache, builder func(p latest.Pipeline) (PipelineBuilder, error)) (*BuilderMux, error) {
	...
	buildkit := useBuildKitEnabled(cfg)
	return &BuilderMux{builders: pbs, byImageName: m, store: store, concurrency: concurrency, buildkit: buildkit, cache: cache}, nil
}
...
func (b *BuilderMux) Build(ctx context.Context, out io.Writer, tags tag.ImageTags, resolver platform.Resolver, artifacts []*latest.Artifact) ([]graph.Artifact, error) {
	...
	builderF := func(ctx context.Context, out io.Writer, artifact *latest.Artifact, tag string, platforms platform.Matcher) (string, error) {
		...
		// buildkit creates multiplatform images via buildkit directly
		if platforms.IsMultiPlatform() && !SupportsMultiPlatformBuild(*artifact) && !b.buildkit {
...
// filterBuildEnvSupportedPlatforms filters the target platforms to those supported by the selected build environment (local/cluster).
...
func getConcurrency(pbs []PipelineBuilder, cliConcurrency int) int {
	...
	if minConcurrency < 0 {
		...
		// set default concurrency to 1 for local builder. For cluster build the default value is 0
