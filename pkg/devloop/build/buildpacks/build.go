...
func (b *Builder) Build(ctx context.Context, out io.Writer, artifact *latest.Artifact, tag string, matcher platform.Matcher) (string, error) {
	if matcher.IsMultiPlatform() {
		...
		log.Entry(ctx).Warnf("multiple target platforms %q found for artifact %q. Devloop doesn't yet support multi-platform builds for the buildpacks builder. Consider specifying a single target platform explicitly. See https://devloop.dev/docs/pipeline-stages/builders/#cross-platform-build-support", matcher.String(), artifact.ImageName)
	}
