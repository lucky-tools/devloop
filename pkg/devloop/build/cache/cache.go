...
func NewCache(ctx context.Context, cfg Config, isLocalImage func(imageName string) (bool, error), dependencies DependencyLister, graph graph.ArtifactGraph, store build.ArtifactStore) (Cache, error) {
	...
	if err != nil {
		log.Entry(context.TODO()).Warnf("Error resolving cache file, not using devloop cache: %v", err)
		return &noCache{}, nil
	}

	artifactCache, err := retrieveArtifactCache(cacheFile)
	if err != nil {
		log.Entry(context.TODO()).Warnf("Error retrieving artifact cache, not using devloop cache: %v", err)
		return &noCache{}, nil
	}
	...
	// buildkit is enabled directly from the devloop.yaml via `useBuildKit: true`.
	buildkit := false
	for _, p := range cfg.GetPipelines() {
...
func resolveCacheFile(cacheFile string) (string, error) {
	...
	defaultFile := filepath.Join(home, constants.DefaultDevloopDir, constants.DefaultCacheFile)
...
func saveArtifactCache(cacheFile string, contents ArtifactCache) error {
	...
	return os.WriteFile(cacheFile, data, 0o755)
}
