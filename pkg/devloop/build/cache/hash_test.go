...
var testTag = "gcr.io/k8s-devloop/devloop:latest"
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			if test.artifact.DockerArtifact != nil {
				tmpDir := t.NewTempDir()
				tmpDir.Write("./Dockerfile", "ARG DEVLOOP_GO_GCFLAGS\nFROM foo")
...
func TestBuildArgs(t *testing.T) {
	tests := []struct {
		...
		{
			mode:     config.RunModes.Debug,
			expected: "b8490bee61ca82830472a694e9a5e7a1fb410933383658ebab8e9a5b7a401a47",
		},
		{
			mode:     config.RunModes.Dev,
...
func TestBuildArgsImageInfoSubstitution(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			originalTag: "gcr.io/k8s-devloop/devloop:latest",
			newTag:      "us.gcr.io/k8s-devloop/devloop:latest",
		},
		{
			...
			originalTag: "gcr.io/k8s-devloop/devloop:latest",
			newTag:      "gcr.io/k8s-devloop/new-devloop:latest",
		},
		{
			...
			originalTag: "gcr.io/k8s-devloop/devloop:v1",
			newTag:      "gcr.io/k8s-devloop/devloop:v2",
		},
	}
...
func TestHashBuildArgs(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			mode:     config.RunModes.Debug,
			expected: []string{"DEVLOOP_GO_GCFLAGS=all=-N -l", "foo=bar"},
		},
		{
			...
			mode:     config.RunModes.Debug,
			expected: []string{"DEVLOOP_GO_GCFLAGS=all=-N -l"},
		},
		{
		...
		},
		{
			...
			mode:     config.RunModes.Debug,
			expected: []string{"DEVLOOP_GO_GCFLAGS=all=-N -l", "IMAGE_REPO=gcr.io/k8s-devloop", "IMAGE_NAME=devloop", "IMAGE_TAG=latest"},
		},
		{
			...
			mode:     config.RunModes.Debug,
			expected: []string{"DEVLOOP_GO_GCFLAGS=all=-N -l", "IMAGE_TAG=latest"},
		},
	}
