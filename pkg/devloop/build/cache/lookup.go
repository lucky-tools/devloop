...
func (c *cache) lookupLocal(ctx context.Context, hash, tag string, entry ImageDetails) cacheDetails {
	...
	if err != nil {
		// Rely on actionable errors thrown from pkg/devloop/docker.LocalDaemon api.
...
func (c *cache) tryImport(ctx context.Context, a *latest.Artifact, tag string, hash string, pl v1.Platform) (ImageDetails, error) {
	...
	// under buildkit, docker daemon is not really needed and could be disabled
	load := true
	if c.buildkit {
		_, err := c.client.ServerVersion(ctx)
		load = err == nil
		if !load {
			log.Entry(ctx).Debugf("Docker client error, disabling image load as using buildkit: %v", err)
		}
	}
	if load {
