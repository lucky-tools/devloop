...
import (
	...
	"github.com/lucky-tools/devloop/pkg/devloop/docker/progress"
...
func (b *Builder) buildWithKaniko(ctx context.Context, out io.Writer, workspace string, artifactName string, artifact *latest.KanikoArtifact, tag string, requiredImages map[string]*string, platforms platform.Matcher) (string, error) {
	...
	if platforms.IsMultiPlatform() {
		log.Entry(ctx).Warnf("multiple target platforms %q found for artifact %q. Devloop doesn't yet support multi-platform builds for the docker builder. Consider specifying a single target platform explicitly. See https://devloop.dev/docs/pipeline-stages/builders/#cross-platform-build-support", platforms.String(), artifactName)
	}
