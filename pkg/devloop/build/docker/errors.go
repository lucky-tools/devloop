...
// The input errors are assumed to be from the Devloop docker invocation.
func newBuildError(err error, cfg docker.Config) error {
	if sErrors.IsDevloopErr(err) {
		return err
	}
		...
		if noSpaceLeft.MatchString(err.Error()) {
			...
			if !cfg.Prune() && (cfg.Mode() == config.RunModes.Dev || cfg.Mode() == config.RunModes.Debug) {
				suggestions = append(suggestions, &proto.Suggestion{
					...
					Action:         fmt.Sprintf("Run devloop %s with --cleanup=true to clean up images built by devloop", cfg.Mode()),
				})
			}
		}
...
func dockerfileNotFound(err error, artifact string) error {
	return sErrors.NewError(err,
		&proto.ActionableErr{
			...
			Suggestions: []*proto.Suggestion{
				{
					SuggestionCode: proto.SuggestionCode_FIX_DEVLOOP_CONFIG_DOCKERFILE,
					Action: fmt.Sprintf("Dockerfile not found. Please check config `dockerfile` for artifact %s."+
						"\nRefer https://devloop.dev/docs/references/yaml/#build-artifacts-docker for details.", artifact),
				},
			},
		})
}
...
func tryExecFormatErrBuildKit(err error, stdErr bytes.Buffer) error {
	if !execFormatErr.MatchString(stdErr.String()) {
		return err
	}
	return sErrors.NewError(err,
