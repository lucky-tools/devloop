...
func (b *Builder) Build(ctx context.Context, out io.Writer, a *latest.Artifact, ref string, platforms platform.Matcher) (string, error) {
	if b.pushImages && strings.HasPrefix(ref, build.StrictScheme) {
		return "", fmt.Errorf("default repo must be set when using the 'ko://' prefix and pushing to a registry: %s, see https://devloop.dev/docs/environment/image-registries/", a.ImageName)
	}
...
//
// If the image name from the Devloop config has the prefix `ko://`, then
...
// it will allow ko users to easily migrate to Devloop without changing their
