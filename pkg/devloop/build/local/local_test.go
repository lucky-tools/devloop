...
func TestLocalRun(t *testing.T) {
	tests := []struct {
		...
		{
			description: "single build (remote) with delete local image after push",
			artifact: &latest.Artifact{
				ImageName: "gcr.io/test/image",
				ArtifactType: latest.ArtifactType{
					DockerArtifact: &latest.DockerArtifact{},
			...
			expectedRemoved: []string{"gcr.io/test/image:tag"},
		},
		{
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			if test.deleteLocalImage {
				localBuild.Delete = util.Ptr(true)
			}
			builder, err := NewBuilder(context.Background(), &mockBuilderContext{artifactStore: artifactStore, mode: test.mode}, localBuild)
			t.CheckNoError(err)
			...
			if test.expectedRemoved != nil {
				t.CheckDeepEqual(test.expectedRemoved, fDockerDaemon.GetRemovedImages())
			}
...
func (fd *fakeDockerDaemon) ImageRemove(_ context.Context, img string, _ client.ImageRemoveOptions) ([]image.DeleteResponse, error) {
	fd.mu.Lock()
	defer fd.mu.Unlock()
	fd.RemovedImages = append(fd.RemovedImages, img)
	return nil, nil
}
...
func (fd *fakeDockerDaemon) GetRemovedImages() []string {
	fd.mu.Lock()
	defer fd.mu.Unlock()
	return fd.RemovedImages
}
