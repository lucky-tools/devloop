...
import (
	...
	testEvent "github.com/lucky-tools/devloop/testutil/event"
)
...
func TestGetBuild(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			tags: tag.ImageTags{
				"devloop/image1": "devloop/image1:v0.0.1",
				"devloop/image2": "devloop/image2:v0.0.2",
			},
			expectedTag: "devloop/image1:v0.0.1@sha256:abac",
		...
		},
		{
			...
			tags: tag.ImageTags{
				"devloop/image1": "ko://github.com/lucky-tools/devloop/cmd/devloop:v0.0.1",
			},
			expectedTag: "github.com/lucky-tools/devloop/cmd/devloop:v0.0.1@sha256:abac",
			expectedOut: "build succeeds",
		},
		{
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			artifact := &latest.Artifact{ImageName: "devloop/image1"}
...
func TestFormatResults(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			artifacts: []*latest.Artifact{
				{ImageName: "devloop/image1"},
				{ImageName: "devloop/image2"},
			},
			expected: []graph.Artifact{
				{ImageName: "devloop/image1", Tag: "devloop/image1:v0.0.1@sha256:abac"},
				{ImageName: "devloop/image2", Tag: "devloop/image2:v0.0.2@sha256:abac"},
			},
			results: map[string]interface{}{
				"devloop/image1": "devloop/image1:v0.0.1@sha256:abac",
				"devloop/image2": "devloop/image2:v0.0.2@sha256:abac",
			},
		},
		{
...
func TestInOrder(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected:    "Building 2 artifacts in parallel\nBuilding [devloop/image1]...\nshort\nBuild [devloop/image1] succeeded\nBuilding [devloop/image2]...\nshort\nBuild [devloop/image2] succeeded\n",
...
`,
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			artifacts := []*latest.Artifact{
				{ImageName: "devloop/image1"},
				{ImageName: "devloop/image2", Dependencies: []*latest.ArtifactDependency{{ImageName: "devloop/image1"}}},
			}
			tags := tag.ImageTags{
				"devloop/image1": "devloop/image1:v0.0.1",
				"devloop/image2": "devloop/image2:v0.0.2",
	...
	for _, test := range tests {
		testutil.Run(t, fmt.Sprintf("%d artifacts, max concurrency=%d", test.artifacts, test.limit), func(t *testutil.T) {
			...
			for i := 0; i < test.artifacts; i++ {
				imageName := fmt.Sprintf("devloop/image%d", i)
				tag := fmt.Sprintf("devloop/image%d:tag", i)
