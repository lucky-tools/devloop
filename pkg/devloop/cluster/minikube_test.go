...
func TestClientImpl_IsMinikube(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			kubeContext:             "test-cluster",
			minikubeProfileCmd:      testutil.CmdRunOut("minikube profile list -o json --user=devloop", fmt.Sprintf(profileStr, "test-cluster", "docker", "172.17.0.3", 8443)),
		...
		},
		{
			...
			minikubeProfileCmd: testutil.CmdRunOut("minikube profile list -o json --user=devloop", fmt.Sprintf(profileStr, "test-cluster", "hyperkit", "192.168.64.10", 8443)),
			expected:           true,
		},
		{
			description:        "cannot parse minikube profile list",
			kubeContext:        "test-cluster",
			minikubeProfileCmd: testutil.CmdRunOut("minikube profile list -o json --user=devloop", `Random error`),
			expected:           false,
		},
		{
			...
			kubeContext:        "test-cluster",
			serverURL:          "https://192.168.64.10:8443",
			minikubeProfileCmd: testutil.CmdRunOut("minikube profile list -o json --user=devloop", fmt.Sprintf(profileStr, "test-cluster", "hyperkit", "192.168.64.11", 8443)),
			expected:           false,
		},
	}
...
func TestMinikubeExec(t *testing.T) {
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			if test.hasUserArg {
				expected = append(expected, "--user=devloop")
			}
