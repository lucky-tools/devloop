...
const (
	// clone missing remote dependencies and sync them on each devloop run
	always = "always"
...
func GetRemoteCacheDir(opts DevloopOptions) (string, error) {
	if opts.RemoteCacheDir != "" {
		return opts.RemoteCacheDir, nil
	}

	// cache location unspecified, use ~/.devloop/remote-cache
	home, err := homedir.Dir()
	if err != nil {
		return "", fmt.Errorf("retrieving home directory: %w", err)
	}
	return filepath.Join(home, constants.DefaultDevloopDir, "remote-cache"), nil
}
