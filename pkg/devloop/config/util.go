...
import (
	...
	kubeclient "github.com/lucky-tools/devloop/pkg/devloop/kubernetes/client"
...
var (
	...
	updateLastTaken = "devloop config set --survey --global last-taken %s"
	updateUserTaken = "devloop config set --survey --global --id %s taken true"
	// update global config with the time the survey was last prompted
	updateLastPrompted = "devloop config set --survey --global last-prompted %s"
)
...
func readConfigFileCached(filename string) (*GlobalConfig, error) {
	configFileOnce.Do(func() {
		...
		if err != nil {
			configFileErr = err
			log.Entry(context.TODO()).Warnf("Could not load global Devloop defaults. Error resolving config file %q", filenameOrDefault)
			return
		}
		configFile, configFileErr = ReadConfigFileNoCache(filenameOrDefault)
		if configFileErr == nil {
			log.Entry(context.TODO()).Infof("Loaded Devloop defaults from %q", filenameOrDefault)
		}
	})
...
func ReadConfigFileNoCache(configFile string) (*GlobalConfig, error) {
	...
	if err != nil {
		log.Entry(context.TODO()).Warnf("Could not load global Devloop defaults. Error encounter while reading file %q", configFile)
		return nil, fmt.Errorf("reading global config: %w", err)
	}
	config := GlobalConfig{}
	if err := yaml.Unmarshal(contents, &config); err != nil {
		log.Entry(context.TODO()).Warnf("Could not load global Devloop defaults. Error encounter while unmarshalling the contents of file %q", configFile)
		return nil, fmt.Errorf("unmarshalling global devloop config: %w", err)
	}
	return &config, nil
}
