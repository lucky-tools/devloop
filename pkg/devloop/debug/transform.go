...
These support files are provided through a set of support images defined at `gcr.io/k8s-devloop/devloop-debug-support/`
...
`artifact` recording the corresponding artifact's `image` in the devloop.yaml,
...
// It also includes a "artifact", usually containing the corresponding artifact's' image name from `devloop.yaml`.
type ImageConfiguration struct {
	// Artifact is the corresponding Artifact's image name (`pkg/devloop/build.Artifact.ImageName`)
	Artifact    string
...
func exposePort(entries []types.ContainerPort, portName string, port int32) []types.ContainerPort {
	...
	for i := 0; i < len(entries); {
		...
		case entries[i].Name == portName:
			...
			log.Entry(context.TODO()).Warnf("devloop debug needs to expose port %d with name %s. Replacing clashing port definition %d (%s)", port, portName, entries[i].ContainerPort, entries[i].Name)
		...
		case entries[i].ContainerPort == port:
			...
			log.Entry(context.TODO()).Warnf("devloop debug needs to expose port %d for %s. Removing clashing port definition %d (%s)", port, portName, entries[i].ContainerPort, entries[i].Name)
