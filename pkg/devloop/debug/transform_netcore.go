...
func (t netcoreTransformer) MatchRuntime(config ImageConfiguration) bool {
	if config.RuntimeType == types.Runtimes.NetCore {
		log.Entry(context.TODO()).Infof("Artifact %q has netcore runtime: specified by user in devloop config", config.Artifact)
		return true
	}
	return false
}
