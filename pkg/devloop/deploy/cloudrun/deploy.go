...
import (
	...
	"github.com/lucky-tools/devloop/pkg/devloop/gcp"
	...
	logger "github.com/lucky-tools/devloop/pkg/devloop/output/log"
...
// NewDeployer creates a new Deployer for Cloud Run from the Devloop deploy config.
...
func (d *Deployer) deployService(crclient *run.APIService, manifest []byte, out io.Writer) (*RunResourceName, error) {
	...
	if d.Project != "" {
		...
		return nil, sErrors.NewError(fmt.Errorf("unable to detect project for Cloud Run"), &proto.ActionableErr{
			Message: "No Google Cloud project found in Cloud Run Manifest or Devloop Config",
			ErrCode: proto.StatusCode_DEPLOY_READ_MANIFEST_ERR,
		})
	}
	// we need to strip "devloop.dev" from the run-id label because gcp labels don't support domains
	runID, foundID := service.Metadata.Labels["devloop.dev/run-id"]
	if foundID {
		delete(service.Metadata.Labels, "devloop.dev/run-id")
		service.Metadata.Labels["run-id"] = runID
	}
	if service.Spec != nil && service.Spec.Template != nil && service.Spec.Template.Metadata != nil {
		runID, foundID = service.Spec.Template.Metadata.Labels["devloop.dev/run-id"]
		if foundID {
			delete(service.Spec.Template.Metadata.Labels, "devloop.dev/run-id")
...
func (d *Deployer) deployJob(crclient *run.APIService, manifest []byte, out io.Writer) (*RunResourceName, error) {
	...
	// we need to strip "devloop.dev" from the run-id label because gcp labels don't support domains
	runID, foundID := job.Metadata.Labels["devloop.dev/run-id"]
	if foundID {
		delete(job.Metadata.Labels, "devloop.dev/run-id")
		job.Metadata.Labels["run-id"] = runID
	}
	if job.Spec != nil && job.Spec.Template != nil && job.Spec.Template.Metadata != nil {
		runID, foundID = job.Spec.Template.Metadata.Labels["devloop.dev/run-id"]
		if foundID {
			delete(job.Spec.Template.Metadata.Labels, "devloop.dev/run-id")
...
func (d *Deployer) deployWorkerPool(crclient *run.APIService, manifest []byte, out io.Writer) (*RunResourceName, error) {
	...
	// we need to strip "devloop.dev" from the run-id label because gcp labels don't support domains
	runID, foundID := workerpool.Metadata.Labels["devloop.dev/run-id"]
	if foundID {
		delete(workerpool.Metadata.Labels, "devloop.dev/run-id")
	...
	if workerpool.Spec != nil && workerpool.Spec.Template != nil && workerpool.Spec.Template.Metadata != nil {
		runID, foundID = workerpool.Spec.Template.Metadata.Labels["devloop.dev/run-id"]
		if foundID {
			delete(workerpool.Spec.Template.Metadata.Labels, "devloop.dev/run-id")
...
func (d *Deployer) deleteRunService(crclient *run.APIService, out io.Writer, dryRun bool, manifest []byte) error {
	...
	default:
		...
		return sErrors.NewError(fmt.Errorf("unable to determine Google Cloud Project"), &proto.ActionableErr{
			Message: "No Google Cloud Project found in Cloud Run manifest or Devloop Manifest.",
			ErrCode: proto.StatusCode_DEPLOY_READ_MANIFEST_ERR,
		})
	}
