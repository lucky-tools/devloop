...
import (
	...
	dockerport "github.com/lucky-tools/devloop/pkg/devloop/deploy/docker/port"
	deployerr "github.com/lucky-tools/devloop/pkg/devloop/deploy/error"
	"github.com/lucky-tools/devloop/pkg/devloop/deploy/label"
	dockerutil "github.com/lucky-tools/devloop/pkg/devloop/docker"
	"github.com/lucky-tools/devloop/pkg/devloop/docker/debugger"
	"github.com/lucky-tools/devloop/pkg/devloop/docker/logger"
	"github.com/lucky-tools/devloop/pkg/devloop/docker/tracker"
	"github.com/lucky-tools/devloop/pkg/devloop/graph"
	"github.com/lucky-tools/devloop/pkg/devloop/kubernetes/manifest"
	"github.com/lucky-tools/devloop/pkg/devloop/log"
	"github.com/lucky-tools/devloop/pkg/devloop/output"
	olog "github.com/lucky-tools/devloop/pkg/devloop/output/log"
	"github.com/lucky-tools/devloop/pkg/devloop/schema/latest"
	"github.com/lucky-tools/devloop/pkg/devloop/status"
	pkgsync "github.com/lucky-tools/devloop/pkg/devloop/sync"
...
func NewDeployer(ctx context.Context, cfg dockerutil.Config, labeller *label.DefaultLabeller, d *latest.DockerDeploy, resources []*latest.PortForwardResource, configName string) (*Deployer, error) {
	...
	return &Deployer{
		...
		cfg:                d,
		client:             client,
		network:            fmt.Sprintf("devloop-network-%s", labeller.GetRunID()),
...
func (d *Deployer) Deploy(ctx context.Context, out io.Writer, builds []graph.Artifact, _ manifest.ManifestListByConfig) error {
	...
	if err != nil {
		return fmt.Errorf("creating devloop network %s: %w", d.network, err)
	}
...
func (d *Deployer) deploy(ctx context.Context, out io.Writer, artifact graph.Artifact) error {
	if !stringslice.Contains(d.cfg.Images, artifact.ImageName) {
		...
		olog.Entry(ctx).Warnf("skipping deploy for image %s since it was not built by Devloop", artifact.ImageName)
		return nil
	}
	...
	if d.cfg.UseCompose {
		...
		return fmt.Errorf("docker compose not yet supported by devloop")
	}
...
func (d *Deployer) containerConfigFromImage(ctx context.Context, taggedImage string) (*container.Config, error) {
	var ociConfig image.InspectResponse
	var err error
...
func (d *Deployer) Cleanup(ctx context.Context, out io.Writer, dryRun bool, _ manifest.ManifestListByConfig) error {
	...
	if err := d.client.NetworkRemove(ctx, d.network); d.networkDeployed && err != nil {
		return errors.Wrap(err, "cleaning up devloop created network")
	}
...
func (d *Deployer) cleanPreviousDeployments(ctx context.Context) error {
	...
	dctd, err := d.debugContainersToDelete(ctx, vtd)
	if err != nil {
		return err
	}
...
func (d *Deployer) containersToDelete(ctx context.Context) ([]container.Summary, error) {
	csToDelete := []container.Summary{}
	for _, img := range d.cfg.Images {
		cs, err := d.getContainersCreated(ctx, img)
		if err != nil {
			return nil, err
		}
...
func (d *Deployer) debugContainersToDelete(ctx context.Context, volumes map[string]bool) ([]container.Summary, error) {
	containersFilters := client.Filters{}.Add("label", label.RunIDLabel).Add("label", label.DebugContainerLabel)
	for v := range volumes {
		containersFilters.Add("volume", v)
	}
