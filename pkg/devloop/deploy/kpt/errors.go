...
// pkgInitErr raises when devloop fails to run `kpt pkg init` which creates the Kptfile under the given dir.
...
// liveInitErr raises when devloop fails to init the "inventory" field in the Kptfile.
...
// liveApplyErr raises when devloop fails to live apply the manifests to the cluster. The Action gives the solutions
			...
			Suggestions: []*proto.Suggestion{
				{
					...
					Action: fmt.Sprintln("if you encounter an inventory mismatch (or can't adopt) error, it indicates " +
						...
						"#1: Update your devloop.yaml by adding the `--inventory-policy=adopt` in the " +
						...
						"and update the `.deploy.kpt.name`, `.deploy.kpt.namespace` and `.deploy.kpt.inventoryID` in your devloop.yaml"),
				},
			},
		})
}

// liveDestroyErr raises when devloop fails to run `kpt live destroy`, which is expected to delete the resource on
// the cluster side.
