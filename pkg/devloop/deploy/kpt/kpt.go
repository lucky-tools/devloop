...
import (
	...
	kstatus "github.com/lucky-tools/devloop/pkg/devloop/kubernetes/status"
	"github.com/lucky-tools/devloop/pkg/devloop/log"
	olog "github.com/lucky-tools/devloop/pkg/devloop/output/log"
	"github.com/lucky-tools/devloop/pkg/devloop/render/kptfile"
...
func NewDeployer(cfg Config, labeller *label.DefaultLabeller, d *latest.KptDeploy, opts config.DevloopOptions, configName string, customResourceSelectors []manifest.GroupKindSelector) (*Deployer, error) {
	defaultNamespace := ""
...
func kptfileInitIfNot(ctx context.Context, out io.Writer, k *Deployer) error {
	...
	if kfConfig.Inventory == nil {
		...
		if err = os.WriteFile(kptFilePath, configByte, 0o644); err != nil {
			return err
		}
	}
	return nil
