...
import (
	...
	kubectlR "github.com/lucky-tools/devloop/pkg/devloop/render/renderer/kubectl"
...
func TestKubectlV1RenderDeploy(t *testing.T) {
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			r, err := kubectlR.New(mockCfg, rc, map[string]string{}, configName, devloopNamespaceOption, nil, !test.skipDevloopNamespaceOption)
			t.CheckNoError(err)
			var b bytes.Buffer
...
func TestKubectlCleanup(t *testing.T) {
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			k, err := NewDeployer(&kubectlConfig{
				workingDir: ".",
				RunContext: runcontext.RunContext{Opts: config.DevloopOptions{Namespace: TestNamespace}},
			}, &label.DefaultLabeller{}, &test.kubectl, nil, configName, nil)
			t.RequireNoError(err)
			rc := latest.RenderConfig{Generate: test.generate}
			mockCfg := &kubectlConfig{
