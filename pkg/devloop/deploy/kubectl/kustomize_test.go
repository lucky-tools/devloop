...
import (
	...
	"github.com/lucky-tools/devloop/pkg/devloop/kubernetes/client"
	"github.com/lucky-tools/devloop/pkg/devloop/render/generate"
	"github.com/lucky-tools/devloop/pkg/devloop/render/renderer/kustomize"
...
func TestKustomizeRenderDeploy(t *testing.T) {
	tests := []struct {
		...
		shouldErr                  bool
		forceDeploy                bool
		skipDevloopNamespaceOption bool
		...
		},
		{
			...
			forceDeploy:                true,
			skipDevloopNamespaceOption: true,
		},
		{
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			tmpDir.Chdir()
			devloopNamespaceOption := ""
			if !test.skipDevloopNamespaceOption {
				devloopNamespaceOption = TestNamespace
			}
			...
			r, err := kustomize.New(mockCfg, rc, map[string]string{}, configName, devloopNamespaceOption, nil, !test.skipDevloopNamespaceOption)
			t.CheckNoError(err)
			var b bytes.Buffer
			...
			k, err := NewDeployer(&kubectlConfig{
				...
				RunContext: runcontext.RunContext{Opts: config.DevloopOptions{
					Namespace: devloopNamespaceOption,
				}},
			}, &label.DefaultLabeller{}, &test.kDeploy, nil, "default", nil)
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			if !test.renderErr {
				...
				}, &label.DefaultLabeller{}, &latest.KubectlDeploy{}, nil, "default", nil)
				t.RequireNoError(err)
...
- path: path/patch2.json`},
		...
		},
		{
			...
			kustomizations: map[string]string{
				"kustomization.yaml": `patchesJson6902:
- path: patch1.json`,
			},
			expected: []string{"kustomization.yaml"},
		},
