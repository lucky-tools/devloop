...
const (
	RunIDLabel          = "devloop.dev/run-id"
	DebugContainerLabel = "devloop.dev/debug"
)
...
type DefaultLabeller struct {
	addDevloopLabels bool
	customLabels     []string
	runID            string
}

func NewLabeller(addDevloopLabels bool, customLabels []string, runID string) *DefaultLabeller {
	return &DefaultLabeller{
		addDevloopLabels: addDevloopLabels,
