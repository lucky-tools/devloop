...
func TestWithLogFile(t *testing.T) {
	...
	logFilename := "- writing logs to " + filepath.Join(os.TempDir(), "devloop", "deploy", "deploy.log")
...
func TestWithStatusCheckLogFile(t *testing.T) {
	...
	logFilename := "- writing logs to " + filepath.Join(os.TempDir(), "devloop", "status-check", "status-check.log")
