...
import (
	...
	"github.com/lucky-tools/devloop/pkg/devloop/util/stringset"
	timeutil "github.com/lucky-tools/devloop/pkg/devloop/util/time"
)

var confirmHydrationDirOverride = prompt.ConfirmHydrationDirOverride
...
func AddTagsToPodSelector(runnerBuilds []graph.Artifact, deployerArtifacts []graph.Artifact, podSelector *kubernetes.ImageList) {
	...
	// probably because at that time the team didn't want to stream logs from images not built by Devloop, e.g. images from docker hub, but this
	...
	// parsed out from devloop config files in v1 and tag was not available if not specified. Now deployers don't own render responsibilities
...
func GetHydrationDir(opts config.DevloopOptions, workingDir string, promptIfNeeded bool, isKptRendererOrDeployerUsed bool) (string, error) {
