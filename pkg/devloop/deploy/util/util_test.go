...
import (
	...
	renderutil "github.com/lucky-tools/devloop/pkg/devloop/render/renderer/util"
...
func TestGetHydrationDir_Default(t *testing.T) {
	testutil.Run(t, "default to <WORKDIR>/.kpt-pipeline", func(t *testutil.T) {
		...
		actual, err := GetHydrationDir(
			config.DevloopOptions{HydrationDir: constants.DefaultHydrationDir, AssumeYes: true},
...
func TestGetHydrationDir_CustomHydrationDir(t *testing.T) {
	testutil.Run(t, "--hydration-dir flag is given", func(t *testutil.T) {
		...
		actual, err := GetHydrationDir(
			config.DevloopOptions{HydrationDir: expected, AssumeYes: true}, "", false, true)
		t.CheckNoError(err)
...
func TestConsolidateTransformConfiguration(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expected: func(allow map[schema.GroupKind]latest.ResourceFilter, deny map[schema.GroupKind]latest.ResourceFilter) (map[schema.GroupKind]latest.ResourceFilter, map[schema.GroupKind]latest.ResourceFilter) {
				// Test.devloop.dev added to allowlist as flag config outprioritizes schema config
				allow[schema.GroupKind{Group: "devloop.dev", Kind: "Test"}] = latest.ResourceFilter{GroupKind: "Test.devloop.dev"}
				return allow, deny
			},
		},
		{
			...
			expected: func(allow map[schema.GroupKind]latest.ResourceFilter, deny map[schema.GroupKind]latest.ResourceFilter) (map[schema.GroupKind]latest.ResourceFilter, map[schema.GroupKind]latest.ResourceFilter) {
				// Test.devloop.dev added to denylist as deny config outprioritizes allow config for same priority config source (both flag config)
				deny[schema.GroupKind{Group: "devloop.dev", Kind: "Test"}] = latest.ResourceFilter{GroupKind: "Test.devloop.dev"}
				return allow, deny
			},
		},
	}
