...
func TestNewEnvClient(t *testing.T) {
	tests := []struct {
		...
		{
			description: "DOCKER_HOST not set, ssh host from docker context",
			command:     testutil.CmdRunOut("docker context inspect --format {{.Endpoints.docker.Host}}", "ssh://127.0.0.1"),
			shouldErr:   false,
		},
		{
