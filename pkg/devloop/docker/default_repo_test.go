...
func TestImageReplaceDefaultRepo(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			image:         "gcr.io/k8s-devloop/devloop-example",
			defaultRepo:   "gcr.io/k8s-devloop",
			expectedImage: "gcr.io/k8s-devloop/devloop-example",
		},
		{
			description:   "image has shared prefix with defaultRepo, but not gcr",
			image:         "myrepo/devloop-example",
			defaultRepo:   "myrepo",
			expectedImage: "myrepo/devloop-example",
		},
		{
