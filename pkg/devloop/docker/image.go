...
func (l *localDaemon) CheckCompatible(a *latest.DockerArtifact) error {
	if len(a.CliFlags) > 0 || len(a.CacheTo) > 0 || len(a.Secrets) > 0 || a.SSH != "" {
		return fmt.Errorf("docker build options (cliFlags, cacheTo, secrets, ssh) are only supported by the `cli` backend - use `build.local.cli` in your config")
	}
	return nil
}
...
// So, the solution we chose is to create a tag, just for Devloop, from
