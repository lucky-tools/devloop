...
// InitializeState instantiates the global state of the devloop runner, as well as the event log.
...
func DeployFailed(err error) {
	...
	handler.handleDeployEvent(&proto.DeployEvent{
		Status:        Failed,
		Err:           err.Error(),
...
func TestFailed(imageName string, err error) {
	...
	handler.stateLock.Unlock()
	handler.handleTestEvent(&proto.TestEvent{
		Status:        Failed,
		ActionableErr: aiErr,
	})
}
...
func FileSyncFailed(fileCount int, image string, err error) {
	...
	handler.handleFileSyncEvent(&proto.FileSyncEvent{
		FileCount: int32(fileCount), Image: image, Status: Failed,
		...
		Event: &proto.Event{
			EventType: &proto.Event_MetaEvent{
				MetaEvent: &proto.MetaEvent{
					Entry:    fmt.Sprintf("Starting Devloop: %+v", version.Get()),
					Metadata: metadata,
				},
			},
		},
