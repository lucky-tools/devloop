...
// SendErrorMessageOnce sends an error message to devloop log events stream only once.
...
func (ev *eventHandler) sendErrorMessage(task constants.Phase, subtask string, err error) {
	...
	ev.errorOnce.Do(func() {
		ev.handleDevloopLogEvent(&proto.DevloopLogEvent{
...
func SaveEventsToFile(fp string) error {
	...
	f, err := os.OpenFile(fp, os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0o600)
	if err != nil {
...
func SaveLastLog(fp string) error {
	...
	f, err := os.OpenFile(fp, os.O_TRUNC|os.O_WRONLY|os.O_CREATE, 0o600)
	if err != nil {
		return fmt.Errorf("opening %s: %w", fp, err)
	}
	defer f.Close()

	// Iterate over events, grabbing contents only from DevloopLogEvents
	var contents bytes.Buffer
	for _, ev := range handler.eventLog {
		if sle := ev.GetDevloopLogEvent(); sle != nil {
...
func lastLogFile(fp string) (string, error) {
	...
	// last log location unspecified, use ~/.devloop/last.log
	home, err := homedir.Dir()
	if err != nil {
		return "", fmt.Errorf("retrieving home directory: %w", err)
	}
	return filepath.Join(home, constants.DefaultDevloopDir, "last.log"), nil
}
