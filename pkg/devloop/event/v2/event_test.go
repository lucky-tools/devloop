...
func TestGetLogEvents(t *testing.T) {
	for step := 0; step < 1000; step++ {
		...
		ev.logEvent(&proto.Event{
			EventType: &proto.Event_DevloopLogEvent{
				DevloopLogEvent: &proto.DevloopLogEvent{Message: "OLD"},
			},
		})
		go func() {
			ev.logEvent(&proto.Event{
				EventType: &proto.Event_DevloopLogEvent{
					DevloopLogEvent: &proto.DevloopLogEvent{Message: "FRESH"},
				},
			})
			ev.logEvent(&proto.Event{
				EventType: &proto.Event_DevloopLogEvent{
					DevloopLogEvent: &proto.DevloopLogEvent{Message: "POISON PILL"},
				},
			})
		}()
		...
		ev.forEachEvent(func(e *proto.Event) error {
			if e.GetDevloopLogEvent().Message == "POISON PILL" {
	...
	handler.eventLog = []*proto.Event{
		{
			...
			EventType: &proto.Event_DevloopLogEvent{
				DevloopLogEvent: &proto.DevloopLogEvent{Message: "Message 1\n"},
			},
		}, {
			EventType: &proto.Event_DeploySubtaskEvent{},
		}, {
			EventType: &proto.Event_DevloopLogEvent{
				DevloopLogEvent: &proto.DevloopLogEvent{Message: "Message 2\n"},
			},
		}, {
...
func TestLastLogFile(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			fp:       "",
			expected: filepath.Join(homeDir, ".devloop", "last.log"),
		},
		{
