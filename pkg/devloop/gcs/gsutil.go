...
import (
	...
	"github.com/lucky-tools/devloop/pkg/devloop/gcs/client"
...
// SyncObjects syncs the target Google Cloud Storage objects with devloop's local cache and returns the local path to the objects.
func SyncObjects(ctx context.Context, g latest.GoogleCloudStorageInfo, opts config.DevloopOptions) (string, error) {
	remoteCacheDir, err := config.GetRemoteCacheDir(opts)
	if err != nil {
		return "", fmt.Errorf("failed determining remote cache directory: %w", err)
	}
	if err := os.MkdirAll(remoteCacheDir, 0o700); err != nil {
