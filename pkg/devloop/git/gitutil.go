...
// SyncRepo syncs the target git repository with devloop's local cache and returns the path to the repository root directory.
var (
	SyncRepo = syncRepo
	findGit  = func() (string, error) { return exec.LookPath("git") }
)
...
func syncRepo(ctx context.Context, g Config, opts config.DevloopOptions) (string, error) {
	devloopCacheDir, err := config.GetRemoteCacheDir(opts)
	r := gitCmd{Dir: devloopCacheDir}
	if err != nil {
		return "", fmt.Errorf("failed to clone repo %s: %w", g.Repo, err)
	}
	if err := os.MkdirAll(devloopCacheDir, 0o700); err != nil {
		return "", fmt.Errorf(
	...
	repoCacheDir := filepath.Join(devloopCacheDir, hash)
