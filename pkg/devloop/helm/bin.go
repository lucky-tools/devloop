...
var (
	...
	// OSExecutable allows for replacing the devloop binary for testing purposes
...
func PreparePostRenderer(ctx context.Context, h Client, devloopBinary string, helmVersion semver.Version) (func(), []string, error) {
	if helmVersion.LT(Helm4PostRendererVersion) {
		return nil, []string{"--post-renderer", devloopBinary}, nil
	}

	// Helm v4 logic

	devloopBinaryAsYaml, err := yaml.Marshal(devloopBinary)
	if err != nil {
		return nil, nil, PluginErr("Failed to fill Devloop executable into Helm plugin manifest", err)
	}

	var helmPluginDir string
	if PluginInstallDir == "" {
		helmPluginDir, err = os.MkdirTemp("", "devloop-render")
		if err != nil {
	...
	err = os.WriteFile(path.Join(helmPluginDir, "plugin.yaml"), []byte(fmt.Sprintf(PostRendererTemplate, helmPluginName, devloopBinaryAsYaml)), 0o644)
	if err != nil &&
...
func PrepareDevloopFilter(h Client, builds []graph.Artifact, flags []string) (devloopBinary string, env []string, cleanup func(), err error) {
	devloopBinary, err = OSExecutable()
	if err != nil {
		return "", nil, nil, fmt.Errorf("cannot locate this Devloop binary: %w", err)
	}
	...
	cmdLine := generateDevloopFilter(h, buildsFile, flags)
	env = append(env, fmt.Sprintf("DEVLOOP_CMDLINE=%s", shell.Join(cmdLine...)))
	env = append(env, fmt.Sprintf("DEVLOOP_FILENAME=%s", h.ConfigFile()))
	return
}

// generateDevloopFilter creates a "devloop filter" command-line for applying the various
// Devloop manifest filters, such a debugging, image replacement, and applying labels.
func generateDevloopFilter(h Client, buildsFile string, flags []string) []string {
