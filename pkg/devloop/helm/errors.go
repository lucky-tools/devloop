...
var versionErrorString = "failed to determine binary version: %w"
...
func MinVersionErr(minVer string) error {
	return sErrors.NewErrorWithStatusCode(
		&proto.ActionableErr{
			Message: fmt.Sprintf("devloop requires Helm version %s or greater", minVer),
...
func CreateNamespaceErr(version string) error {
	return sErrors.NewErrorWithStatusCode(
		&proto.ActionableErr{
			Message: fmt.Sprintf("Devloop config options `createNamespace` is not available in the current Helm version %s", version),
			...
			Suggestions: []*proto.Suggestion{
				...
				{
					SuggestionCode: proto.SuggestionCode_FIX_DEVLOOP_CONFIG_HELM_CREATE_NAMESPACE,
