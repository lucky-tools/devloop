...
func TestDeployHooks(t *testing.T) {
	testutil.Run(t, "TestDeployHooks", func(t *testutil.T) {
			...
			PreHooks: []latest.DeployHookItem{
				{
					HostHook: &latest.HostHook{
						...
						Command: []string{"sh", "-c", "echo pre-hook running with DEVLOOP_RUN_ID=$DEVLOOP_RUN_ID,DEVLOOP_KUBE_CONTEXT=$DEVLOOP_KUBE_CONTEXT,DEVLOOP_NAMESPACES=$DEVLOOP_NAMESPACES"},
					},
				},
				{
					HostHook: &latest.HostHook{
						OS:      []string{"windows"},
						Command: []string{"cmd.exe", "/C", "echo pre-hook running with DEVLOOP_RUN_ID=%DEVLOOP_RUN_ID%,DEVLOOP_KUBE_CONTEXT=%DEVLOOP_KUBE_CONTEXT%,DEVLOOP_NAMESPACES=%DEVLOOP_NAMESPACES%"},
					},
				},
				{
			...
			PostHooks: []latest.DeployHookItem{
				{
					HostHook: &latest.HostHook{
						OS:      []string{"linux", "darwin"},
						Command: []string{"sh", "-c", "echo post-hook running with DEVLOOP_RUN_ID=$DEVLOOP_RUN_ID,DEVLOOP_KUBE_CONTEXT=$DEVLOOP_KUBE_CONTEXT,DEVLOOP_NAMESPACES=$DEVLOOP_NAMESPACES"},
					},
				},
				{
					HostHook: &latest.HostHook{
						OS:      []string{"windows"},
						Command: []string{"cmd.exe", "/C", "echo post-hook running with DEVLOOP_RUN_ID=%DEVLOOP_RUN_ID%,DEVLOOP_KUBE_CONTEXT=%DEVLOOP_KUBE_CONTEXT%,DEVLOOP_NAMESPACES=%DEVLOOP_NAMESPACES%"},
					},
				},
				{
					ContainerHook: &latest.NamedContainerHook{
		...
		preHostHookOut := "pre-hook running with DEVLOOP_RUN_ID=run_id,DEVLOOP_KUBE_CONTEXT=context1,DEVLOOP_NAMESPACES=np1,np2"
		preContainerHookOut := "container pre-hook succeeded"
		postHostHookOut := "post-hook running with DEVLOOP_RUN_ID=run_id,DEVLOOP_KUBE_CONTEXT=context1,DEVLOOP_NAMESPACES=np1,np2"
