...
// Each field name is converted from CamelCase to SCREAMING_SNAKE_CASE and prefixed with `DEVLOOP`.
// For example the field `KubeContext` with value `kind` becomes `DEVLOOP_KUBE_CONTEXT=kind`
func getEnv(optsStruct interface{}) []string {
	...
	for i := 0; i < t.NumField(); i++ {
		...
		env = append(env, fmt.Sprintf("DEVLOOP_%s=%v", toScreamingSnakeCase(f.Name), v.Interface()))
	}
	return env
}
