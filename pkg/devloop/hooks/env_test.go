...
func TestGetEnv(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected: []string{
				"DEVLOOP_DEFAULT_REPO=gcr.io/foo",
				"DEVLOOP_MULTI_LEVEL_REPO=true",
				"DEVLOOP_RPC_PORT=8080",
				"DEVLOOP_HTTP_PORT=8081",
				"DEVLOOP_WORK_DIR=./foo",
			},
		},
		{
		...
		},
		{
			...
			expected: []string{
				"DEVLOOP_IMAGE=foo",
				"DEVLOOP_PUSH_IMAGE=true",
				"DEVLOOP_IMAGE_REPO=gcr.io/foo",
				"DEVLOOP_IMAGE_TAG=latest",
				"DEVLOOP_BUILD_CONTEXT=./foo",
			},
		},
		{
			...
			expected: []string{
				"DEVLOOP_IMAGE=foo",
				"DEVLOOP_FILES_ADDED_OR_MODIFIED=./foo/1;./foo/2",
				"DEVLOOP_FILES_DELETED=./foo/3;./foo/4",
				"DEVLOOP_KUBE_CONTEXT=minikube",
				"DEVLOOP_NAMESPACES=np1,np2,np3",
				"DEVLOOP_BUILD_CONTEXT=./foo",
			},
		},
		{
