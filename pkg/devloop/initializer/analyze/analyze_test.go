...
func TestAnalyze(t *testing.T) {
	...
	tests := []struct {
		...
		{
			description: "should not error when devloop.config present and force = true",
			filesWithContents: map[string]string{
				"devloop.yaml": `apiVersion: devloop/v1
		kind: Config
		deploy:
		  kubectl: {}`,
				"config/test.yaml":  validK8sManifest,
		...
		{
			description: "should error when devloop.config present and force = false",
			filesWithContents: map[string]string{
				"config/test.yaml":  validK8sManifest,
				"k8pod.yml":         validK8sManifest,
				"README":            emptyFile,
...
deploy:
  kubectl: {}`,
		...
		{
			description: "should error when devloop.config present with jib config",
			filesWithContents: map[string]string{
				"config/test.yaml": validK8sManifest,
				"k8pod.yml":        validK8sManifest,
				"README":           emptyFile,
