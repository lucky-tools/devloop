...
func TestGenerateDevloopConfig(t *testing.T) {
	tests := []struct {
		name                  string
		expectedDevloopConfig *latest.DevloopConfig
	...
	for _, test := range tests {
		testutil.Run(t, test.name, func(t *testutil.T) {
			...
			config := generateDevloopConfig(buildInitializer, rendererInitializer, deploymentInitializer)
			t.CheckDeepEqual(config, test.expectedDevloopConfig)
		})
	}
}
