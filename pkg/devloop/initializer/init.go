...
import (
	...
	"github.com/lucky-tools/devloop/cmd/devloop/app/tips"
	...
	"github.com/lucky-tools/devloop/pkg/devloop/initializer/prompt"
...
// DoInit executes the `devloop init` flow.
...
// Initialize uses the information gathered by the analyzer to create a devloop config and generate kubernetes manifests.
...
func Initialize(out io.Writer, c config.Config, a *analyze.ProjectAnalysis) (*latest.DevloopConfig, map[string][]byte, error) {
	...
	return generateDevloopConfig(buildInitializer, renderInitializer, deployInitializer), newManifests, nil
}
...
// WriteData takes the given devloop config and k8s manifests and writes them out to a file or the given io.Writer
func WriteData(out io.Writer, c config.Config, newConfig *latest.DevloopConfig, newManifests map[string][]byte) error {
	...
	if !c.Force && !c.Opts.AutoInit {
		if done, err := prompt.WriteDevloopConfig(out, pipeline, newManifests, c.Opts.ConfigurationFile); done {
			return err
		}
	}
	...
	if err = os.WriteFile(c.Opts.ConfigurationFile, pipeline, 0o644); err != nil {
