	...
	tests := []struct {
		...
		// TODO: mocked kompose test
		{
			...
			config: initconfig.Config{
				...
				Opts: config.DevloopOptions{
					ConfigurationFile: "devloop.yaml.out",
				},
			},
		},
		{
		...
		},
		{
			...
			config: initconfig.Config{
				Force: true,
				CliArtifacts: []string{
					"leeroy-app/Dockerfile=gcr.io/k8s-devloop/leeroy-app",
					"leeroy-web/Dockerfile=gcr.io/k8s-devloop/leeroy-web",
				},
				Opts: config.DevloopOptions{
					ConfigurationFile: "devloop.yaml.out",
				},
		...
		},
		{
			...
			config: initconfig.Config{
				Force: true,
				CliArtifacts: []string{
					`{"builder":"Docker","payload":{"path":"leeroy-app/Dockerfile"},"image":"gcr.io/k8s-devloop/leeroy-app"}`,
					`{"builder":"Docker","payload":{"path":"leeroy-web/Dockerfile"},"image":"gcr.io/k8s-devloop/leeroy-web"}`,
				},
				Opts: config.DevloopOptions{
					ConfigurationFile: "devloop.yaml.out",
				},
		...
		},
		{
			...
			config: initconfig.Config{
				Force: true,
				CliArtifacts: []string{
					`{"builder":"Docker","context":"apps\\web","payload":{"path":"apps\\web\\build\\Dockerfile"},"image":"gcr.io/k8s-devloop/leeroy-web"}`,
				},
				Opts: config.DevloopOptions{
					ConfigurationFile: "devloop.yaml.out",
				},
		...
		},
		{
			...
			expectedError:    "one or more valid Kubernetes manifests are required to run devloop",
		...
		},
		{
			...
			expectedError:    "pre-existing devloop.yaml found",
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedOut: strip(`{
							...
							"images":["gcr.io/k8s-devloop/leeroy-app","gcr.io/k8s-devloop/leeroy-web"]
							}`) + "\n",
		},
		{
			...
			expectedOut: strip(`{
									...
									"images":[
										{"name":"gcr.io/k8s-devloop/leeroy-app","foundMatch":false},
										{"name":"gcr.io/k8s-devloop/leeroy-web","foundMatch":false}]}`) + "\n",
		},
		{
...
func checkGeneratedConfig(t *testutil.T, dir string) {
	expectedOutput, err := schema.ParseConfig(filepath.Join(dir, "devloop.yaml"))
	t.CheckNoError(err)

	output, err := schema.ParseConfig(filepath.Join(dir, "devloop.yaml.out"))
	t.CheckNoError(err)
