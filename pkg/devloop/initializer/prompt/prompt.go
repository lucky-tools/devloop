...
import (
	...
	"github.com/lucky-tools/devloop/pkg/devloop/initializer/util"
...
func WriteDevloopConfig(out io.Writer, pipeline []byte, generatedManifests map[string][]byte, filePath string) (bool, error) {
...
// ConfirmInitOptions prompts the user to confirm that they are okay with what devloop will do if they
// run with the current config
func ConfirmInitOptions(out io.Writer, config *latest.DevloopConfig) (bool, error) {
	builders := strings.Join(util.ListBuilders(&config.Build), ",")
	deployers := strings.Join(util.ListDeployers(&config.Deploy), ",")

	fmt.Fprintf(out, `If you choose to continue, devloop will do the following:
