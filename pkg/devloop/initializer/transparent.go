...
var confirmInitOptions = prompt.ConfirmInitOptions

// Transparent executes the `devloop init` flow, but always enables the --force flag.
// It will also always prompt the user to confirm at the end of the flow.
func Transparent(ctx context.Context, out io.Writer, c initConfig.Config) (*latest.DevloopConfig, error) {
...
func ValidCmd(opts config.DevloopOptions) bool {
