...
func AddLocalBuildEnv(ctx context.Context, out io.Writer, opts inspect.Options) error {
	...
	cfgs, err := inspect.GetConfigSet(ctx, config.DevloopOptions{ConfigurationFile: opts.Filename, ConfigurationFilter: opts.Modules, SkipConfigDefaults: true, MakePathsAbsolute: util.Ptr(false)})
	if err != nil {
...
func constructLocalDefinition(existing *latest.LocalBuild, opts inspect.BuildEnvOptions) *latest.LocalBuild {
	...
	if opts.UseDockerCLI != nil && *opts.UseDockerCLI {
		// CLI backend requested
		if b.CLI == nil {
			b.CLI = &latest.DockerCLIBackend{}
		}
		b.API = nil
	} else if opts.UseDockerCLI != nil && !*opts.UseDockerCLI {
		// API backend requested (also the default) — clear any CLI backend.
		b.CLI = nil
		b.API = nil
	}
	return &b
