...
func TestPrintJobManifestPathsModify(t *testing.T) {
	tests := []struct {
		...
		shouldErr   bool
		expected    string
		originalCfg latest.DevloopConfig
	}{
		{
			...
			expected: `apiVersion: devloop/v1
kind: Config
verify:
  - name: foo
    container:
...
`,
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			t.Override(&getCfgs, func(context.Context, config.DevloopOptions) ([]util.VersionedConfig, error) {
				return []util.VersionedConfig{&test.originalCfg}, nil
			})
			var b bytes.Buffer
			err := Modify(context.Background(), &b, config.DevloopOptions{}, inputFile, "")
