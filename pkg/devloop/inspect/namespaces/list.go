...
import (
	...
	"github.com/lucky-tools/devloop/pkg/webhook/constants"
)
...
func PrintNamespacesList(ctx context.Context, out io.Writer, manifestFile string, opts inspect.Options) error {
	...
	cfgs, err := inspect.GetConfigSet(ctx, config.DevloopOptions{
	...
	for _, c := range cfgs {
		if c.Deploy.KubectlDeploy != nil {
			...
			// NOTE: Cloud Deploy uses `devloop apply` which always uses kubectl deployer.  As such other
			// namespace config should be ignored.
		}
	}
