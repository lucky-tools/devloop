...
// Options holds flag values for the various `devloop inspect` commands
type Options struct {
	// Filename is the `devloop.yaml` file path
	Filename string
	...
	Profile string
	// PropagateProfiles specifies if profiles specified by the '--profile' flag should be searched across config dependencies. Otherwise only profiles defined directly in the target 'devloop.yaml' file are activated.
...
// ModulesOptions holds flag values for various `devloop inspect modules` commands
type ModulesOptions struct {
	// IncludeAll specifies if unnamed modules should be included in the output list
	IncludeAll bool
}

// ProfilesOptions holds flag values for various `devloop inspect profiles` commands
type ProfilesOptions struct {
	// BuildEnv is the build-env filter for command output. One of: local, cluster.
	BuildEnv BuildEnv
}

// BuildEnvOptions holds flag values for various `devloop inspect build-env` commands
