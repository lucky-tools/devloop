...
func TestExportMetrics(t *testing.T) {
	...
	metersBytes, _ := json.Marshal([]devloopMeter{devMeter, debugMeter, buildMeter})
	...
	for _, test := range tests {
		testutil.Run(t, test.name, func(t *testutil.T) {
			var actual []devloopMeter
			var savedMetrics []devloopMeter
			...
			if !test.isOnline {
				...
				if test.shouldFailUnmarshal {
					expected = []devloopMeter{test.meter}
				}
...
// todo refactor
func checkOutput(t *testutil.T, meters []devloopMeter, b []byte) {
	...
	for _, smetric := range r.ScopeMetrics {
		for _, metric := range smetric.Metrics {
			...
			for _, point := range dataPoints {
				l := line{Labels: make(map[string]string)}
				l.Name = metric.Name
				l.Count = point.Value
				l.Max = point.Value
