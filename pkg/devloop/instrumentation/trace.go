...
// InitTraceFromEnvVar initializes the singleton devloop tracer from the DEVLOOP_TRACE env variable.
// The code here is a wrapper around the opentelemetry(otel) trace libs for usability
// When DEVLOOP_TRACE is set, this will setup the proper tracer provider (& exporter),
// configures otel to use this tracer provider and saves the  tracer provider shutdown function
// to be used globally so that it can be run before devloop exits.
func InitTraceFromEnvVar(opts ...TraceExporterOption) (trace.TracerProvider, func(context.Context) error, error) {
	traceInitOnce.Do(func() {
		_, skaffTraceEnv := os.LookupEnv("DEVLOOP_TRACE")
...
// at the end of a devloop run to properly shutdown and export all spans for the singleton.
...
// via the value set in DEVLOOP_TRACE.  Tracing is done via metadata stored in a context.Context. This means that
// to properly get parent/child traces, callers should use the returned context for subsequent calls in devloop.
