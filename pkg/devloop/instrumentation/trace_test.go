...
func TestInitCloudTrace(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "DEVLOOP_TRACE=stdout, verify spans output to stdout and spans are sequential",
			traceEnvVar: "stdout",
			parentSpans: []string{"SequentialSpanOne", "SequentialSpanTwo"},
		},
		{
			name:              "DEVLOOP_TRACE=stdout, verify spans output to stdout and spans are concurrent",
			traceEnvVar:       "stdout",
			parentSpans:       []string{"ConcurrentSpanOne", "ConcurrentSpanTwo"},
			isConcurrentTrace: true,
		},
		{
			name:              "DEVLOOP_TRACE=stdout, verify spans output to stdout and parent/child relationship exists spans",
			traceEnvVar:       "stdout",
	...
	for _, test := range tests {
		testutil.Run(t, test.name, func(t *testutil.T) {
			if len(test.traceEnvVar) > 0 {
				t.Setenv("DEVLOOP_TRACE", test.traceEnvVar)
			}
			var b bytes.Buffer
			func() {
