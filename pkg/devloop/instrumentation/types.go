...
// devloopMeter describes the data used to determine operational metrics.
type devloopMeter struct {
	// ConfigCount is the number of parsed devloop configurations in the current session.
	ConfigCount int

	// ExitCode Exit code returned by Devloop at the end of execution.
	ExitCode int

	// BuildArtifacts Number of artifacts built in the current execution as defined in devloop.yaml.
	BuildArtifacts int

	// Command Command that is used to execute devloop `dev, build, render, run, etc.`
	// without any command-line arguments.
	Command string

	// Version Version of Devloop being used "v1.18.0, v1.19.1, etc.".
	Version string

	// OS OS running Devloop e.g. Windows, Linux, Darwin, etc.
	OS string

	// Arch Architecture running Devloop e.g. amd64, arm64, etc.
	Arch string

	// PlatformType Where Devloop is building artifacts (local, cluster, Google Cloud Build, or a combination of them).
	PlatformType string

	// User indicates the client invoking devloop. Is one of allowedUser i.e. vsc, intellij, gcloud
	User string

	// Deployers All the deployers used in the Devloop execution.
	Deployers []string

	// EnumFlags Enum values for flags passed into Devloop that have a pre-defined list of
	...
	// StartTime Start time of the Devloop program, used to track how long Devloop took to finish executing.
	StartTime time.Time

	// Duration Time Devloop took to finish executing in milliseconds.
	Duration time.Duration

	// ErrorCode Devloop reports [error codes](/docs/references/api/grpc/#statuscode)
	...
	// CI represents the platform Devloop is running on
	CISystem string
}
