...
import (
	...
	logstream "github.com/lucky-tools/devloop/pkg/devloop/log/stream"
	...
	go func() {
		...
		if waitErr := wait.Poll(backoff.Duration, RetryTimeout, func() (bool, error) {
			...
			w, err := clientset.CoreV1().Pods(namespace).Watch(ctx,
				metav1.ListOptions{
					LabelSelector: labels.Set(map[string]string{"job-name": id, "devloop.dev/run-id": l.labeller.GetRunID()}).String(),
				})
			if err != nil {
				return false, nil
			}
