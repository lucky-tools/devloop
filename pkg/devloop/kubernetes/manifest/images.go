...
func (r *imageReplacer) Visit(gk apimachinery.GroupKind, navpath string, o map[string]interface{}, k string, v interface{}, rs ResourceSelector) bool {
	...
	parsed, err := docker.ParseReference(image)
	if err != nil {
		// this is a hack to properly support `imageStrategy=helm` & `imageStrategy=helm+explicitRegistry` from Devloop v1.X.X
		// which have the form:
		...
		// when the artifact name has a fully qualified path - gcr.io/example-repo/devloop-helm-image
	...
	// this is a hack to properly support `imageStrategy=helm+explicitRegistry` from Devloop v1.X.X which has the form:
	// image: "{{.Values.image.registry}}/{{.Values.image.repository}}:{{.Values.image.tag}}"
	// when the artifact name is not fully qualified - devloop-helm-image
	// works by looking for intermediate helm replacement of the form image:
...
// imageSelector represents a strategy for matching the container `image` defined in a kubernetes manifest with the correct devloop artifact.
