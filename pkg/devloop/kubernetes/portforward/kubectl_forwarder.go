...
// It restarts the command if it was not cancelled by devloop
...
func (k *KubectlForwarder) forward(ctx context.Context, pfe *portForwardEntry, errChan chan error) {
	...
	for {
		...
		if !isPortFree(util.Loopback, pfe.localPort) {
			// Assuming that Devloop brokered ports don't overlap, this has to be an external process that started
...
func portForwardArgs(ctx context.Context, kubeContext string, pfe *portForwardEntry) []string {
	...
	_, disableServiceForwarding := os.LookupEnv("DEVLOOP_DISABLE_SERVICE_FORWARDING")
	switch {
	case pfe.resource.Type == "service" && !disableServiceForwarding:
		// Services need special handling: https://github.com/lucky-tools/devloop/issues/4522
...
// used by kubectl (see https://github.com/lucky-tools/devloop/issues/4522 for details).
