...
import (
	...
	"github.com/lucky-tools/devloop/pkg/devloop/kubernetes/status/resource"
	...
	"github.com/lucky-tools/devloop/pkg/diag"
	"github.com/lucky-tools/devloop/pkg/diag/validator"
...
// Check runs the status checks on selected resource rollouts in current devloop dev iteration.
...
func (s *monitor) statusCheck(ctx context.Context, out io.Writer) (proto.StatusCode, error) {
	...
	wg.Wait()
	return getDevloopDeployStatus(ctx, c, exitStatus)
}
...
func pollResourceStatus(ctx context.Context, cfg Config, r *resource.Resource) {
	...
	for {
		...
		case <-ticker.C:
			...
			// TODO: https://github.com/lucky-tools/devloop/pull/4591
...
func getDevloopDeployStatus(ctx context.Context, c *counter, sc proto.StatusCode) (proto.StatusCode, error) {
	...
	if sc == proto.StatusCode_STATUSCHECK_SUCCESS || sc == 0 {
		log.Entry(ctx).Debugf("found statuscode %s. setting devloop deploy status to STATUSCHECK_INTERNAL_ERROR.", sc)
		return proto.StatusCode_STATUSCHECK_INTERNAL_ERROR, err
	}
	log.Entry(ctx).Debugf("setting devloop deploy status to %s.", sc)
	return sc, err
}
