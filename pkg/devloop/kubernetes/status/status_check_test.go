...
func TestGetDeployments(t *testing.T) {
	...
	tests := []struct {
		...
		{
			description: "deployment in correct namespace but not deployed by devloop",
			deps: []*appsv1.Deployment{
				{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "dep1",
		...
		{
			description: "deployment in correct namespace deployed by devloop but different run",
			deps: []*appsv1.Deployment{
				{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "dep1",
...
func TestGetDeployStatus(t *testing.T) {
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			actual, err := getDevloopDeployStatus(context.Background(), test.counter, test.sc)
