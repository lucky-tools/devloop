...
// for testing
var (
	getDockerDependenciesForEachFromTo = docker.GetDependenciesByDockerCopyFromTo
	dockerfileRules                    = &dockerfileLintRules
)
...
var dockerfileLintRules = []Rule{
	{
		...
		ExplanationTemplate: `Found docker 'COPY' command where the source directory "{{index .FieldMap "src"}}" has over 1000 files.  This has the potential to dramatically slow 'devloop dev' down ` +
			`as devloop watches all sources files referenced in dockerfile COPY directives for changes. ` +
			`If you notice devloop rebuilding images unnecessarily when non-image-critical files are ` +
			...
			`or adding a .dockerignore file (https://docs.docker.com/engine/reference/builder/#dockerignore-file) ignoring non-image-critical files.  devloop respects files ignored via the .dockerignore`,
	...
	},
	{
		...
		ExplanationTemplate: `Found docker 'COPY' command where the source directory "{{index .FieldMap "src"}}" contains a '.git' directory at {{index .FieldMap "gitDirectoryAbsPath"}}.  This has the potential to dramatically slow 'devloop dev' down ` +
			`as devloop will watch all of the files in the .git directory as devloop watches all sources files referenced in dockerfile COPY directives for changes. ` +
			`devloop will likely rebuild images unnecessarily when non-image-critical files are ` +
			`modified during any git related operation. Consider adding a .dockerignore file (https://docs.docker.com/engine/reference/builder/#dockerignore-file) ignoring the '.git' directory. devloop respects files ignored via the .dockerignore`,
		ExplanationPopulator: func(params InputParams) (explanationInfo, error) {
	...
	for _, c := range cfgs {
		for _, a := range c.Build.Artifacts {
			if a.DockerArtifact != nil {
				...
				// TODO(aaron-prindle) currently this dep map is computed twice; here and in devloopyamls.go, make a singleton/share-the-info
