...
func TestGetDockerfilesLintResults(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			moduleAndDevloopYamls: map[string]string{"cfg0": testDevloopYaml},
			modules:               []string{"cfg0"},
			dockerFromTo: []docker.FromTo{
				{
					From:      ".",
				...
				"cfg0": {
					{
						...
						Explanation: `Found docker 'COPY' command where the source directory "." has over 1000 files.  This has the potential ` +
							`to dramatically slow 'devloop dev' down as devloop watches all sources files referenced in dockerfile COPY directives ` +
							`for changes. If you notice devloop rebuilding images unnecessarily when non-image-critical files are modified, consider ` +
							`changing this to 'COPY $REQUIRED_SOURCE_FILE(s) /' for each required source file instead of or adding a .dockerignore file ` +
							`(https://docs.docker.com/engine/reference/builder/#dockerignore-file) ignoring non-image-critical files.  devloop respects ` +
				...
				"cfg0": {
					{
						...
						Explanation: `Found docker 'COPY' command where the source directory "." contains a '.git' directory at .git.  This has the potential ` +
							`to dramatically slow 'devloop dev' down as devloop will watch all of the files in the .git directory as devloop watches all sources ` +
							`files referenced in dockerfile COPY directives for changes. devloop will likely rebuild images unnecessarily when non-image-critical ` +
							`files are modified during any git related operation. Consider adding a .dockerignore file ` +
							`(https://docs.docker.com/engine/reference/builder/#dockerignore-file) ignoring the '.git' directory. devloop respects files ignored ` +
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			for i := 0; i < len(test.moduleAndDevloopYamls); i++ {
				...
				err = os.WriteFile(dfp, b.Bytes(), 0o644)
				if err != nil {
				...
				configSet = append(configSet, &parser.DevloopConfigEntry{
					DevloopConfig: &latest.DevloopConfig{
						...
						Pipeline: latest.Pipeline{Build: latest.BuildConfig{Artifacts: []*latest.Artifact{{
							Workspace:    "",
