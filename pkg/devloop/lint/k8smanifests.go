...
var k8sManifestLintRules = []Rule{
	{
		...
		ExplanationTemplate: "Found usage of label 'app.kubernetes.io/managed-by'.  devloop overwrites the 'app.kubernetes.io/managed-by' field to 'app.kubernetes.io/managed-by: devloop'. " +
	...
	for _, c := range cfgs {
		for _, pattern := range c.Render.RawK8s {
			...
			if util.IsURL(pattern) {
				log.Entry(ctx).Debugf("devloop lint found url manifest and is skipping lint rules for: %s", pattern)
				continue
			}
