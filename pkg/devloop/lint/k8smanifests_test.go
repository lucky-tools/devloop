...
func TestGetK8sManifestsLintResults(t *testing.T) {
	...
	tests := []struct {
				...
				"cfg0": {
					{
						...
						StartColumn: 5,
						EndColumn:   0,
						Explanation: `Found usage of label 'app.kubernetes.io/managed-by'.  devloop overwrites the 'app.kubernetes.io/managed-by' ` +
							`field to 'app.kubernetes.io/managed-by: devloop'. and as such is recommended to remove this label`,
					},
				},
			},
		},
		...
		{
			rules:                 []RuleID{},
			description:           "no k8sManifest file for devloop.yaml",
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			for i := 0; i < len(test.moduleAndDevloopYamls); i++ {
				...
				err = os.WriteFile(mp, []byte(test.k8sManifestText), 0o644)
				if err != nil {
