...
import (
	...
	"github.com/lucky-tools/devloop/pkg/devloop/lint"
...
// Handler is the server handler for the devloop LSP.  It implements the LSP spec and supports connection over TCP
type Handler struct {
...
func sendValidationAndLintDiagnostics(ctx context.Context, opts config.DevloopOptions, out io.Writer, req jsonrpc2.Request, createRunner func(ctx context.Context, out io.Writer, opts config.DevloopOptions) (runner.Runner, []schemautil.VersionedConfig, *runcontext.RunContext, error)) error {
	isValidConfig := true
	...
	if isValidConfig {
		...
		// TODO(aaron-prindle) files should be linted even if devloop.yaml file is invalid (for the lint rules that is possible for)
...
func GetHandler(conn jsonrpc2.Conn, out io.Writer, opts config.DevloopOptions, createRunner func(ctx context.Context, out io.Writer, opts config.DevloopOptions) (runner.Runner, []schemautil.VersionedConfig, *runcontext.RunContext, error)) jsonrpc2.Handler {
	h = *NewHandler(conn)
...
func validateFiles(ctx context.Context,
	opts config.DevloopOptions, req jsonrpc2.Request,
) (map[string][]protocol.Diagnostic, error) {
