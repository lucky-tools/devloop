...
import (
	...
	"github.com/lucky-tools/devloop/pkg/devloop/gcs"
	"github.com/lucky-tools/devloop/pkg/devloop/git"
...
// GetAllConfigs returns the list of all devloop configurations parsed from the target config file in addition to all resolved dependency configs.
func GetAllConfigs(ctx context.Context, opts config.DevloopOptions) ([]schemaUtil.VersionedConfig, error) {
	...
	for _, cfg := range set {
		cfgs = append(cfgs, cfg.DevloopConfig)
	}
	return cfgs, nil
}

// GetConfigSet returns the list of all devloop configurations parsed from the target config file in addition to all resolved dependency configs as a `DevloopConfigSet`.
// This struct additionally contains the file location that each devloop configuration is parsed from.
func GetConfigSet(ctx context.Context, opts config.DevloopOptions) (DevloopConfigSet, error) {
	...
	for _, c := range cfgs {
		yinfos, err := configlocations.Parse(c.SourceFile, c.DevloopConfig, fieldsOverrodeByProfile)
		if err != nil {
			return nil, err
		}
		c.YAMLInfos = yinfos
...
// getConfigs recursively parses all configs and their dependencies in the specified `devloop.yaml`
func getConfigs(ctx context.Context, cfgOpts configOpts, opts config.DevloopOptions, r *record) (DevloopConfigSet, map[string]configlocations.YAMLOverrideInfo, error) {
	...
	for _, cfg := range parsed {
		config, ok := cfg.(*latest.DevloopConfig)
		if !ok {
			return nil, nil, sErrors.DevloopConfigUpgradeErr(cfg.GetVersion(), latest.Version)
		}
...
// The `index` parameter specifies the index of the current config in its `devloop.yaml` file. We use the `index` instead of the config `metadata.name` property to uniquely identify each config since not all configs define `name`.
func processEachConfig(ctx context.Context, config *latest.DevloopConfig, cfgOpts configOpts, opts config.DevloopOptions, r *record, index int, fieldsOverrodeByProfile map[string]configlocations.YAMLOverrideInfo) (DevloopConfigSet, error) {
	...
	// This avoids maintaining multiple root directory information since the dependency devloop configs would have their own root directory.
	...
	// This is used when the parsed config is marshalled out (for commands like `devloop diagnose` or `devloop inspect`), we want to retain the original relative paths in the output files.
	...
	if required {
		configs = append(configs, &DevloopConfigEntry{
			DevloopConfig: config,
...
func processEachDependency(ctx context.Context, d latest.ConfigDependency, cfgOpts configOpts, opts config.DevloopOptions, r *record) (DevloopConfigSet, error) {
	...
	if !util.IsURL(path) {
		...
		if err != nil {
			...
			return nil, sErrors.ConfigParsingError(fmt.Errorf("parsing dependencies for devloop config %s: %w", cfgOpts.file, err))
		}
		if fi.IsDir() {
			path = filepath.Join(path, "devloop.yaml")
		}
	}
...
// cacheRepo downloads the referenced git repository to devloop's cache if required and returns the path to the target configuration file in that repository.
func cacheRepo(ctx context.Context, g git.Config, configFilePath string, opts config.DevloopOptions, r *record) (string, error) {
...
// cacheGCSObject downloads the referenced Google Cloud Storage object to devloop's cache if required and returns the path to the target configuration file.
func cacheGCSObject(ctx context.Context, g latest.GoogleCloudStorageInfo, opts config.DevloopOptions, r *record) (string, error) {
	key := g.Source
...
func checkRevisit(config *latest.DevloopConfig, profiles []string, appliedProfiles map[string]string, file string, required bool, index int) (bool, error) {
...
func isMakePathsAbsoluteSet(opts config.DevloopOptions) bool {
...
func getBase(cfgOpts configOpts) (string, error) {
	if cfgOpts.isDependency {
		log.Entry(context.TODO()).Tracef("found %s base dir for absolute path substitution within devloop config %s", filepath.Dir(cfgOpts.file), cfgOpts.file)
		return filepath.Dir(cfgOpts.file), nil
	}
	log.Entry(context.TODO()).Tracef("found cwd as base for absolute path substitution within devloop config %s", cfgOpts.file)
