...
func createCfg(name string, imageName string, workspace string, requires []latest.ConfigDependency) *latest.DevloopConfig {
...
func TestGetAllConfigs(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			documents:   []document{{path: "devloop.yaml", configs: []mockCfg{{name: "cfg00", requiresStanza: ""}}}},
		...
		},
		{
			...
			documents:   []document{{path: "devloop.yaml", configs: []mockCfg{{name: "cfg00", requiresStanza: ""}, {name: "cfg01", requiresStanza: ""}}}},
			expected: func(string) []schemaUtil.VersionedConfig {
				return []schemaUtil.VersionedConfig{
		...
		},
		{
			...
			documents: []document{
				{path: "devloop.yaml", configs: []mockCfg{{name: "cfg00", requiresStanza: `
requires:
  - path: doc1
    configs: [cfg10]
  - path: doc2
...
`}, {name: "cfg01", requiresStanza: ""}}},
				{path: "doc1/devloop.yaml", configs: []mockCfg{{name: "cfg10", requiresStanza: ""}, {name: "cfg11", requiresStanza: ""}}},
				{path: "doc2/devloop.yaml", configs: []mockCfg{{name: "cfg20", requiresStanza: ""}, {name: "cfg21", requiresStanza: ""}}},
			},
			expected: func(base string) []schemaUtil.VersionedConfig {
				return []schemaUtil.VersionedConfig{
...
requires:
  - path: doc1
    configs: [cfg10]
`}, {name: "cfg01", requiresStanza: ""}}},
				{path: "doc1/devloop.yaml", configs: []mockCfg{{name: "cfg10", requiresStanza: `
requires:
  - path: ../doc2
    configs: [cfg21]
...
requires:
  - path: doc1
    configs: [cfg11]
`}}},
				{path: "doc1/devloop.yaml", configs: []mockCfg{{name: "cfg10"}, {name: "cfg11"}, {name: "cfg00"}}},
			},
		...
		},
		{
			...
			documents: []document{
				{path: "devloop.yaml", configs: []mockCfg{{name: "cfg00"}, {name: "cfg00"}}},
			},
...
`}}},
			...
			expected: func(base string) []schemaUtil.VersionedConfig {
				return []schemaUtil.VersionedConfig{
					createCfg("cfg21", "image21", filepath.Join(base, "doc2"), nil),
					createCfg("cfg10", "image10", filepath.Join(base, "doc1"), []latest.ConfigDependency{{GitRepo: &latest.GitInfo{Repo: "doc2", Path: "devloop.yaml", Ref: "main"}, Names: []string{"cfg21"}}}),
...
`}}},
			...
			expected: func(string) []schemaUtil.VersionedConfig {
				return []schemaUtil.VersionedConfig{
					createCfg("cfg21", "image21", ".", nil),
					createCfg("cfg10", "image10", ".", []latest.ConfigDependency{{GitRepo: &latest.GitInfo{Repo: "doc2", Path: "devloop.yaml", Ref: "main"}, Names: []string{"cfg21"}}}),
...
`}, {name: "cfg11", requiresStanza: ""}}},
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			t.Override(&git.SyncRepo, func(ctx context.Context, g latest.GitInfo, _ config.DevloopOptions) (string, error) {
				return g.Repo, nil
			})
			cfgs, err := GetAllConfigs(context.Background(), config.DevloopOptions{
				Command:             "dev",
...
var testDevloopYaml = `apiVersion: devloop/v1
kind: Config
build:
  artifacts:
    - image: app-0
...
func TestConfigLocationsParse(t *testing.T) {
	tests := []struct {
		description      string
		devloopYamlText  string
		profiles         []string
		missingNodeCount int
		expected         [][]kyaml.Filter
	}{
		{
			description:     "find all expected yaml nodes for input devloop.yaml file",
			devloopYamlText: testDevloopYaml,
			profiles:        []string{},
			expected: [][]kyaml.Filter{
		...
		{
			description:     "find all expected yaml nodes for input devloop.yaml file and input profile",
			devloopYamlText: testDevloopYaml,
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			fp := t.TempFile("devloopyaml-", []byte(test.devloopYamlText))
			cfgs, err := GetConfigSet(context.TODO(), config.DevloopOptions{ConfigurationFile: fp, Profiles: test.profiles})
			if err != nil {
				t.Fatal(err.Error())
			}
			root, err := kyaml.Parse(test.devloopYamlText)
			if err != nil {
				t.Fatal(err.Error())
			}
			var seen bool
...
func TestConfigLocationsLocate(t *testing.T) {
	tests := []struct {
		...
		{
			description:     "verify location for DevloopConfig.Build.Artifacts[0] is as expected",
			devloopYamlText: testDevloopYaml,
			profiles:        []string{},
			expected: []configlocations.Location{
				{
		...
		{
			description:     "verify location for DevloopConfig.Build.Artifacts[0] is as expected with active profile with a patch",
			devloopYamlText: testDevloopYaml,
			profiles:        []string{"profile-0"},
			expected: []configlocations.Location{
				{
		...
		{
			description:     "verify location for DevloopConfig.Build.Artifacts[0] is as expected with active profile with no patch",
			devloopYamlText: testDevloopYaml,
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			...
			artifact0Location := cfgs.Locate(cfgs[0].DevloopConfig.Build.Artifacts[0])
