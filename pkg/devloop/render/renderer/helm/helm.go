...
import (
	...
	"github.com/lucky-tools/devloop/pkg/devloop/helm"
	...
	"github.com/lucky-tools/devloop/pkg/devloop/render/renderer/util"
	"github.com/lucky-tools/devloop/pkg/devloop/schema/latest"
	sUtil "github.com/lucky-tools/devloop/pkg/devloop/util"
)
...
func (h Helm) generateHelmManifests(ctx context.Context, builds []graph.Artifact) (manifest.ManifestList, error) {
	...
	if len(builds) > 0 {
		devloopBinary, filterEnv, cleanup, err := helm.PrepareDevloopFilter(h, builds, []string{})
		if err != nil {
			return nil, fmt.Errorf("could not prepare `devloop filter`: %w", err)
		}
		defer cleanup()
		...
		cleanUpPostRenderer, postRendererArgs, err = helm.PreparePostRenderer(ctx, h, devloopBinary, h.helmVersion)
		if err != nil {
			return nil, err
		}
...
func (h Helm) generateHelmManifest(ctx context.Context, builds []graph.Artifact, release latest.HelmRelease, env, additionalArgs []string) ([]byte, error) {
	...
	deleteDevloopOverrides, err := generateDevloopOverrides(release)
	if err != nil {
		return nil, helm.UserErr("cannot construct helm overrides values file", err)
	}
	if deleteDevloopOverrides != nil {
		defer deleteDevloopOverrides()
	}
...
func generateDevloopOverrides(release latest.HelmRelease) (func(), error) {
