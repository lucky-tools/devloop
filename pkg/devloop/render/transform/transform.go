...
// GetDeclarativeValidators transforms and returns the devloop validators defined in devloop.yaml
func (v *Transformer) GetDeclarativeTransformers() ([]kptfile.Function, error) {
	// TODO: guarantee the v.kptFn is updated once users changed devloop.yaml file.
	if v.needRefresh {
...
func validateTransformers(config []latest.Transformer) ([]kptfile.Function, error) {
	...
	for _, c := range config {
		...
		if !ok {
			// TODO: Add links to explain "devloop-managed mode" and "kpt-managed mode".
					...
					Suggestions: []*proto.Suggestion{
						{
							...
							Action: fmt.Sprintf(
								"please only use the following transformers in devloop-managed mode: %v. "+
