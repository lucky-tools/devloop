...
import (
	...
	"github.com/lucky-tools/devloop/pkg/devloop/util/term"
...
func (r *DevloopRunner) doDev(ctx context.Context, out io.Writer) error {
...
func (r *DevloopRunner) getSyncHandler(
...
// Dev watches for changes and runs the devloop build, test and deploy
// config until interrupted by the user.
func (r *DevloopRunner) Dev(ctx context.Context, out io.Writer, artifacts []*latest.Artifact) error {
	event.DevLoopInProgress(r.devIteration)
	eventV2.InitializeState(r.runCtx)
	eventV2.TaskInProgress(constants.DevLoop, "")
	defer func() { r.devIteration++ }()
	...
	for ; err != nil && r.runCtx.Opts.KeepRunningOnFailure; err = r.Deploy(ctx, out, r.Builds, manifests) {
		...
		// users need to fix the problems in their manifests, and devloop needs to re-render them before re-running Deploy Stage in this case.
	...
	if err := r.monitor.Register(
		...
		eventV2.TaskFailed(constants.DevLoop, err)
		endTrace()
		return fmt.Errorf("watching devloop configuration %q: %w", r.runCtx.ConfigurationFile(), err)
	}
