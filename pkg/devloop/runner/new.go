...
import (
	...
	"github.com/lucky-tools/devloop/pkg/devloop/test"
	"github.com/lucky-tools/devloop/pkg/devloop/trigger"
	"github.com/lucky-tools/devloop/pkg/devloop/verify"
)

// NewForConfig returns a new DevloopRunner for a DevloopConfig
func NewForConfig(ctx context.Context, runCtx *runcontext.RunContext) (*DevloopRunner, error) {
	...
	// Always add devloop-specific labels, except during `devloop render`
	labeller := label.NewLabeller(runCtx.AddDevloopLabels(), runCtx.CustomLabels(), runCtx.GetRunID())
	...
	renderer, err := GetRenderer(ctx, runCtx, hydrationDir, labeller.Labels())
	if err != nil {
		endTrace(instrumentation.TraceEndError(err))
		return nil, fmt.Errorf("creating renderer: %w", err)
	}

	deployer, err = GetDeployer(ctx, runCtx, labeller, hydrationDir)
	if err != nil {
		endTrace(instrumentation.TraceEndError(err))
	...
	return &DevloopRunner{
		...
		platforms:          platforms,
		monitor:            monitor,
		listener:           NewDevloopListener(monitor, rtrigger, sourceDependencies, intentChan),
		artifactStore:      store,
...
func isImageLocal(runCtx *runcontext.RunContext, imageName string) (bool, error) {
	...
	case runCtx.Opts.PushImages.Value() != nil:
		log.Entry(context.TODO()).Debugf("push value set via devloop build --push flag, --push=%t", *runCtx.Opts.PushImages.Value())
