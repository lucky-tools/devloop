...
// IsMultiPipeline returns true if there are more than one constituent devloop pipelines.
...
// AddDevloopLabels tells the Runner whether to add devloop-specific labels.
// We only ever skip adding labels during a `devloop render`.
func (rc *RunContext) AddDevloopLabels() bool {
...
func GetRunContext(ctx context.Context, opts config.DevloopOptions, configs []schemaUtil.VersionedConfig) (*RunContext, error) {
	pipelines := make(map[string]latest.Pipeline)
	var orderedConfigs []string

	for _, cfg := range configs {
		if cfg != nil {
			pipeline := cfg.(*latest.DevloopConfig).Pipeline
			cfgName := getConfigName(cfg.(*latest.DevloopConfig).Metadata.Name)
	...
	// TODO(dgageot): this should be the folder containing devloop.yaml. Should also be moved elsewhere.
	cwd, err := os.Getwd()
	if err != nil {
