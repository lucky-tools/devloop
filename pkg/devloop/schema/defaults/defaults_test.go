...
func TestSetDefaultsOnLocalBuild(t *testing.T) {
	cfg1 := &latest.DevloopConfig{Pipeline: latest.Pipeline{Build: latest.BuildConfig{}}}
	cfg2 := &latest.DevloopConfig{Pipeline: latest.Pipeline{Build: latest.BuildConfig{Artifacts: []*latest.Artifact{{ImageName: "foo"}}}}}
