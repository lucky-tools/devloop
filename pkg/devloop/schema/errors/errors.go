...
func ConfigParsingError(err error) error {
	return sErrors.NewError(err,
		&proto.ActionableErr{
			Message: fmt.Sprintf("error parsing devloop configuration file: %v", err),
...
func DependencyConfigFileNotFoundErr(depFile, parentFile string, err error) error {
	return sErrors.NewError(err,
		&proto.ActionableErr{
			Message: fmt.Sprintf("could not find devloop config file %q that is referenced as a dependency in config file %q: %v", depFile, parentFile, err),
...
func DuplicateConfigNamesInSameFileErr(config, file string) error {
	msg := fmt.Sprintf("multiple devloop configs named %q found in file %q", config, file)
	return sErrors.NewError(errors.New(msg),
		&proto.ActionableErr{
			Message: msg,
...
func DuplicateConfigNamesAcrossFilesErr(config, file1, file2 string) error {
	msg := fmt.Sprintf("devloop config named %q found in multiple files: %q and %q", config, file1, file2)
	return sErrors.NewError(errors.New(msg),
		&proto.ActionableErr{
			Message: msg,
			...
			Suggestions: []*proto.Suggestion{
				{
					...
					Action:         fmt.Sprintf("There's an issue with one of the profiles defined in config %q in file %q; refer to the documentation on how to author valid profiles: https://devloop.dev/docs/environment/profiles/", config, file),
				},
			},
		})
}
			...
			Suggestions: []*proto.Suggestion{
				{
					...
					Action:         "Check that all occurrences of the specified config dependency use the same set of profiles; refer to the documentation on how to author config dependencies: https://devloop.dev/docs/design/config/#configuration-dependencies",
				},
			},
		})
}
...
func ConfigUnknownAPIVersionErr(version string) error {
	msg := fmt.Sprintf("unknown devloop config API version %q", version)
	return sErrors.NewError(errors.New(msg),
		&proto.ActionableErr{
			Message: msg,
			...
			Suggestions: []*proto.Suggestion{
				{
					...
					Action:         "Set the config 'apiVersion' to a known value. Check https://devloop.dev/docs/references/yaml/ for the list of valid API versions. Otherwise, check that your devloop version is up-to-date",
				},
			},
		})
}

// DevloopConfigUpgradeErr specifies that devloop config needs to be upgraded to the latest version.
func DevloopConfigUpgradeErr(currentVersion, targetVersion string) error {
	msg := fmt.Sprintf("devloop cannot auto-upgrade the config from version %s to version %s", currentVersion, targetVersion)
	return sErrors.NewError(errors.New(msg),
		&proto.ActionableErr{
			Message: msg,
			...
			Suggestions: []*proto.Suggestion{
				{
					SuggestionCode: proto.SuggestionCode_CONFIG_FIX_DEVLOOP_CONFIG_VERSION,
					Action:         fmt.Sprintf("Upgrade devloop config version to %s manually", targetVersion),
				},
			},
		})
}
