...
const Version string = "devloop/v1"

// NewDevloopConfig creates a DevloopConfig
func NewDevloopConfig() util.VersionedConfig {
	return new(DevloopConfig)
}

// DevloopConfig holds the fields parsed from the Devloop configuration file (devloop.yaml).
type DevloopConfig struct {
	...
	// Env is a list of environment variables, in the `KEY=value` form, that are
	// set in the process environment before the pipeline runs. Variables that are
	// already present in the process environment are left untouched. This provides
	// the same functionality as the `devloop.env` file, but inside the main config.
	// For example: `["DOCKER_HOST=ssh://root@192.168.1.53", "DOCKER_CONTEXT=53-remote"]`.
...
// Pipeline describes a Devloop pipeline.
type Pipeline struct {
	...
	// CustomActions describes a list of user defined actions that can be triggered with `devloop exec`.
	CustomActions []Action `yaml:"customActions,omitempty"`
}

// GitInfo contains information on the origin of devloop configurations cloned from a git repository.
type GitInfo struct {
	// Repo is the git repository the package should be cloned from.  e.g. `https://github.com/lucky-tools/devloop.git`.
	Repo string `yaml:"repo" yamltags:"required"`

	// Path is the relative path from the repo root to the devloop configuration file. eg. `getting-started/devloop.yaml`.
...
// GoogleCloudStorageInfo contains information on the origin of devloop configurations copied from Google Cloud Storage.
type GoogleCloudStorageInfo struct {
	// Source is the Google Cloud Storage objects to copy. e.g. `gs://my-bucket/dir1/dir2/*`.
	Source string `yaml:"source,omitempty"`

	// Path is the relative path from the source to the devloop configuration file. e.g. `configs/devloop.yaml`.
	Path string `yaml:"path,omitempty"`
...
// ConfigDependency describes a dependency on another devloop configuration.
type ConfigDependency struct {
	...
	Path string `yaml:"path,omitempty" devloop:"filepath" yamltags:"oneOf=paths"`
...
func (c *DevloopConfig) GetVersion() string {
	return c.APIVersion
}
...
type PortForwardResource struct {
	...
	Name string `yaml:"resourceName,omitempty" devloop:"template"`

	// Namespace is the namespace of the resource to port forward. Does not apply to local containers.
	Namespace string `yaml:"namespace,omitempty" devloop:"template"`
	...
	// LocalPort is the local port to forward to. If the port is unavailable, Devloop will choose a random open port to forward to. *Optional*.
...
type EnvTemplateTagger struct {
	...
	// For example: `{{.RELEASE}}`.
	Template string `yaml:"template,omitempty" yamltags:"required" devloop:"template"`
}
...
type LocalBuild struct {
	...
	// Delete whether the local image should be deleted after it has been pushed to a registry.
	// If not specified, the local image is kept after pushing.
	...
	DockerBuildBackend `yaml:",inline"`
}

// DockerBuildBackend selects the mechanism used to build Docker artifacts:
...
type DockerCLIBackend struct {
	// UseBuildKit use `docker buildx` to build Docker images. Requires buildkit to be installed.
	UseBuildKit bool `yaml:"useBuildKit,omitempty"`

	// BuildKitBuilder is the name of the buildkit builder to use. If empty, the buildkit default builder is used.
	BuildKitBuilder string `yaml:"buildKitBuilder,omitempty"`

	// CacheTag is the tag used for the cache destination when using buildkit.
	CacheTag string `yaml:"cacheTag,omitempty"`
}
...
// TestCase is a list of tests to run on images that Devloop builds.
type TestCase struct {
	// ImageName is the artifact on which to run those tests.
	// For example: `gcr.io/k8s-devloop/example`.
	ImageName string `yaml:"image" yamltags:"required"`

	// Workspace is the directory containing the test sources.
	// Defaults to `.`.
	Workspace string `yaml:"context,omitempty" devloop:"filepath"`
	...
	// For example: `["./test/*"]`.
	StructureTests []string `yaml:"structureTests,omitempty" devloop:"filepath"`
...
// VerifyTestCase is a list of tests to run on images that Devloop builds.
...
// VerifyContainer is a list of tests to run on images that Devloop builds.
...
type Generate struct {
	...
	RawK8s []string `yaml:"rawYaml,omitempty" devloop:"filepath"`
	...
	Kpt []string `yaml:"kpt,omitempty" devloop:"filepath"`
...
type RemoteManifest struct {
	...
	// KubeContext is the Kubernetes context that Devloop should deploy to.
...
type Kustomize struct {
	...
	// Defaults to `["."]`.
	Paths []string `yaml:"paths,omitempty" devloop:"filepath,template"`
...
type Transformer struct {
	// Name is the transformer name. Can only accept devloop whitelisted tools.
	Name string `yaml:"name" yamltags:"required"`
...
type Validator struct {
	// Name is the Validator name. Can only accept devloop whitelisted tools.
	Name string `yaml:"name" yamltags:"required"`
	// ConfigMap allows users to provide additional config data to the kpt function.
	ConfigMap []string `yaml:"configMap,omitempty"`
}
...
type KptDeploy struct {
	// Dir is equivalent to the dir in `kpt live apply <dir>`. If not provided, devloop deploys from the default
...
type DeployConfig struct {
	...
	// TolerateFailuresUntilDeadline configures the Devloop "status-check" to tolerate failures
...
type KubectlDeploy struct {
	...
	DefaultNamespace *string `yaml:"defaultNamespace,omitempty" devloop:"template"`
...
type HelmRelease struct {
	...
	Name string `yaml:"name,omitempty" yamltags:"required" devloop:"template"`

	// ChartPath is the local path to a packaged Helm chart or an unpacked Helm chart directory.
	ChartPath string `yaml:"chartPath,omitempty" yamltags:"oneOf=chartSource" devloop:"filepath,template"`
	...
	ValuesFiles []string `yaml:"valuesFiles,omitempty" devloop:"filepath,template"`

	// Namespace is the Kubernetes namespace.
	Namespace string `yaml:"namespace,omitempty" devloop:"template"`

	// Version is the version of the chart.
	Version string `yaml:"version,omitempty" devloop:"template"`

	// SetValues are key-value pairs.
	// If present, Devloop will send `--set` flag to Helm CLI and append all pairs after the flag.
	SetValues util.FlatMap `yaml:"setValues,omitempty" devloop:"template"`

	// SetValueTemplates are key-value pairs.
	// If present, Devloop will try to parse the value part of each key-value pair using
	// environment variables in the system, then send `--set` flag to Helm CLI and append
	// all parsed pairs after the flag.
	SetValueTemplates util.FlatMap `yaml:"setValueTemplates,omitempty" devloop:"template"`

	// SetFiles are key-value pairs.
	// If present, Devloop will send `--set-file` flag to Helm CLI and append all pairs after the flag.
	SetFiles map[string]string `yaml:"setFiles,omitempty" devloop:"filepath"`

	// CreateNamespace if `true`, Devloop will send `--create-namespace` flag to Helm CLI.
	// `--create-namespace` flag is available in Helm since version 3.2.
	// Defaults is `false`.
	CreateNamespace *bool `yaml:"createNamespace,omitempty"`

	// Wait if `true`, Devloop will send `--wait` flag to Helm CLI.
	// Defaults to `false`.
	Wait bool `yaml:"wait,omitempty"`

	// RecreatePods if `true`, Devloop will send `--recreate-pods` flag to Helm CLI
	...
	// UseHelmSecrets instructs devloop to use secrets plugin on deployment.
	UseHelmSecrets bool `yaml:"useHelmSecrets,omitempty"`

	// Repo specifies the helm repository for remote charts.
	// If present, Devloop will send `--repo` Helm CLI flag or flags.
	Repo string `yaml:"repo,omitempty"`
	...
	// If present, Devloop will build a Helm `values` file that overrides
...
type Profile struct {
	...
	// Pipeline contains the definitions to replace the default devloop pipeline.
	Pipeline `yaml:",inline"`
}
...
type Activation struct {
	...
	// Command is a Devloop command for which the profile is auto-activated.
	// For example: `dev`.
	Command string `yaml:"command,omitempty"`
}
...
type BuildpackArtifact struct {
	...
	// Dependencies are the file dependencies that devloop should watch for both rebuilding and file syncing for this artifact.
...
type BuildpackDependencies struct {
	// Paths should be set to the file dependencies for this artifact, so that the devloop file watcher knows when to rebuild and perform file synchronization.
	Paths []string `yaml:"paths,omitempty" yamltags:"oneOf=dependency"`

	// Ignore specifies the paths that should be ignored by devloop's file watcher. If a file exists in both `paths` and in `ignore`, it will be ignored, and will be excluded from both rebuilds and file synchronization.
...
type BuildpackVolume struct {
	...
	Host string `yaml:"host" devloop:"filepath" yamltags:"required"`
...
// written by the user. It can be used to build images with builders that aren't directly integrated with devloop.
...
type CustomDependencies struct {
	// Dockerfile should be set if the artifact is built from a Dockerfile, from which devloop can determine dependencies.
	Dockerfile *DockerfileDependency `yaml:"dockerfile,omitempty" yamltags:"oneOf=dependency"`

	// Command represents a custom command that devloop executes to obtain dependencies. The output of this command *must* be a valid JSON array.
...
type CustomTest struct {
	...
	// TimeoutSeconds sets the wait time for devloop for the command to complete.
	// If unset or 0, Devloop will wait until the command completes.
...
type CustomTestDependencies struct {
	// Command represents a command that devloop executes to obtain dependencies. The output of this command *must* be a valid JSON array.
	Command string `yaml:"command,omitempty" yamltags:"oneOf=dependency"`

	// Paths locates the file dependencies for the command relative to workspace.
	// Paths should be set to the file dependencies for this command, so that the devloop file watcher knows when to retest and perform file synchronization.
	// For example: `["src/test/**"]`
	Paths []string `yaml:"paths,omitempty" yamltags:"oneOf=dependency"`

	// Ignore specifies the paths that should be ignored by devloop's file watcher. If a file exists in both `paths` and in `ignore`, it will be ignored, and will be excluded from both retest and file synchronization.
	// Will only work in conjunction with `paths`.
	Ignore []string `yaml:"ignore,omitempty"`
}

// DockerfileDependency *beta* is used to specify a custom build artifact that is built from a Dockerfile. This allows devloop to determine dependencies from the Dockerfile.
...
type KanikoArtifact struct {
	...
	ContextSubPath string `yaml:"contextSubPath,omitempty" devloop:"filepath"`
...
type DockerArtifact struct {
	...
	BuildArgs map[string]*string `yaml:"buildArgs,omitempty" devloop:"template"`
...
type KoArtifact struct {
	...
	Env []string `yaml:"env,omitempty" devloop:"template"`

	// Flags are additional build flags passed to `go build`.
	// For example: `["-trimpath", "-v"]`.
	Flags []string `yaml:"flags,omitempty" devloop:"template"`

	// Labels are key-value string pairs to add to the image config.
	// For example: `{"foo":"bar"}`.
	Labels map[string]string `yaml:"labels,omitempty" devloop:"template"`

	// Ldflags are linker flags passed to the builder.
	// For example: `["-buildid=", "-s", "-w"]`.
	Ldflags []string `yaml:"ldflags,omitempty" devloop:"template"`
...
type KoDependencies struct {
	...
	// Ignore specifies the paths that should be ignored by Devloop's file watcher.
...
type PostRenderHostHook struct {
	...
	Dir string `yaml:"dir,omitempty" devloop:"filepath"`
...
type RenderHooks struct {
	// PreHooks describes the list of lifecycle hooks to execute *before* each render step. Container hooks will only run if the container exists from a previous deployment step (for instance the successive iterations of a dev-loop during `devloop dev`).
...
type DeployHooks struct {
	// PreHooks describes the list of lifecycle hooks to execute *before* each deployer step. Container hooks will only run if the container exists from a previous deployment step (for instance the successive iterations of a dev-loop during `devloop dev`).
...
// https://github.com/lucky-tools/devloop/issues/4175
