...
import (
	...
	cfg "github.com/lucky-tools/devloop/pkg/devloop/config"
...
`
	...
	testutil.Run(t, "", func(t *testutil.T) {
		...
		tmpDir := t.NewTempDir().
			Write("devloop.yaml", addVersion(config))

		parsed, err := ParseConfig(tmpDir.Path("devloop.yaml"))
		t.CheckNoError(err)
		t.CheckTrue(len(parsed) > 0)

		devloopConfig := parsed[0].(*latest.DevloopConfig)
		activated, _, err := ApplyProfiles(devloopConfig, map[string]configlocations.YAMLOverrideInfo{}, cfg.DevloopOptions{}, []string{"patches"})
		t.CheckNoError(err)
		t.CheckDeepEqual([]string{"patches"}, activated)
		t.CheckDeepEqual("replacement", devloopConfig.Build.Artifacts[0].ImageName)
		t.CheckDeepEqual("Dockerfile.DEV", devloopConfig.Build.Artifacts[0].DockerArtifact.DockerfilePath)
		t.CheckDeepEqual("Dockerfile.second", devloopConfig.Build.Artifacts[1].DockerArtifact.DockerfilePath)
		t.CheckDeepEqual("third-replaced", devloopConfig.Build.Artifacts[2].ImageName)
		t.CheckDeepEqual(latest.DeployConfig{}, devloopConfig.Deploy)
	})
}
...
`
	...
	testutil.Run(t, "", func(t *testutil.T) {
		...
		parsed, err := ParseConfig(tmp.Path("devloop.yaml"))
		t.CheckNoError(err)
		t.CheckTrue(len(parsed) > 0)

		devloopConfig := parsed[0].(*latest.DevloopConfig)
		_, _, err = ApplyProfiles(devloopConfig, map[string]configlocations.YAMLOverrideInfo{}, cfg.DevloopOptions{}, []string{"patches"})
...
func TestApplyProfiles(t *testing.T) {
	...
	for _, test := range tests {
		testutil.Run(t, test.description, func(t *testutil.T) {
			setupFakeKubeConfig(t, api.Config{CurrentContext: "prod-context"})
			_, _, err := ApplyProfiles(test.config, map[string]configlocations.YAMLOverrideInfo{}, cfg.DevloopOptions{
				Command:               "dev",
...
`
	...
	testutil.Run(t, "", func(t *testutil.T) {
		...
		t.CheckDeepEqual(2, len(devloopConfig.Profiles))
		t.CheckDeepEqual("simple1", devloopConfig.Profiles[0].Name)
		t.CheckDeepEqual([]latest.Activation{{Env: "ABC=common"}, {Env: "ABC=1"}}, devloopConfig.Profiles[0].Activation)
		t.CheckDeepEqual("simple2", devloopConfig.Profiles[1].Name)
		t.CheckDeepEqual([]latest.Activation{{Env: "ABC=common"}, {Env: "ABC=2"}}, devloopConfig.Profiles[1].Activation)

		applied, _, err := ApplyProfiles(devloopConfig, map[string]configlocations.YAMLOverrideInfo{}, cfg.DevloopOptions{}, []string{"simple1"})
		t.CheckNoError(err)
		t.CheckDeepEqual([]string{"simple1"}, applied)
		t.CheckDeepEqual(1, len(devloopConfig.Build.Artifacts))
		t.CheckDeepEqual(latest.Artifact{ImageName: "simpleimage1"}, *devloopConfig.Build.Artifacts[0])
	})
}
