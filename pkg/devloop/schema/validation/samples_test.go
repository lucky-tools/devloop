...
func TestParseSamples(t *testing.T) {
	...
	for _, path := range paths {
		...
		testutil.Run(t, name, func(t *testutil.T) {
			...
			checkDevloopConfig(t, addHeader(buf))
		})
	}
}

func checkDevloopConfig(t *testutil.T, yaml []byte) {
	configFile := t.TempFile("devloop.yaml", yaml)
	parsed, err := schema.ParseConfigAndUpgrade(configFile)
	t.CheckNoError(err)
	var cfgs parser.DevloopConfigSet
	for _, p := range parsed {
		cfg := &parser.DevloopConfigEntry{DevloopConfig: p.(*latest.DevloopConfig)}
		err = defaults.Set(cfg.DevloopConfig)
		defaults.SetDefaultDeployer(cfg.DevloopConfig)
		t.CheckNoError(err)
...
func parseConfigFiles(t *testing.T, root string) {
	groupedPaths, err := walk.From(root).WhenHasName("devloop.yaml").CollectPathsGrouped(1)
	if err != nil {
		t.Fatalf("unable to list devloop configuration files in %q", root)
	}

	if len(groupedPaths) == 0 {
		t.Fatalf("did not find devloop configuration files in %q", root)
	}
