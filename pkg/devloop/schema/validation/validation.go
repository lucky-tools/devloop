...
import (
	...
	"github.com/lucky-tools/devloop/pkg/devloop/yamltags"
...
func GetValidationOpts(opts config.DevloopOptions) Options {
	switch opts.Mode() {
...
// ProcessToErrorWithLocation checks if the Devloop pipeline is valid and returns all encountered errors as ErrorWithLocation objects
func ProcessToErrorWithLocation(configs parser.DevloopConfigSet, validateConfig Options) []ErrorWithLocation {
	errs := validateImageNames(configs)
	for _, config := range configs {
		errs = append(errs, visitStructs(config, reflect.ValueOf(config.DevloopConfig), validateYamltags)...)
...
// Process checks if the Devloop pipeline is valid and returns all encountered errors as a concatenated string
func Process(configs parser.DevloopConfigSet, validateConfig Options) error {
...
// ProcessWithRunContext checks if the Devloop pipeline is valid when a RunContext is required.
...
func validateTaggingPolicy(cfg *parser.DevloopConfigEntry, bc latest.BuildConfig) (cfgErrs []ErrorWithLocation) {
...
func validateImageNames(configs parser.DevloopConfigSet) (errs []ErrorWithLocation) {
	...
	for _, c := range configs {
		for i, a := range c.Build.Artifacts {
			...
			if parsed.Tag != "" {
				errs = append(errs, wrapWithContext(c, ErrorWithLocation{
					Error:    fmt.Errorf("invalid image %q: no tag should be specified. Use taggers instead: https://devloop.dev/docs/how-tos/taggers/", a.ImageName),
					Location: curLines,
				})...)
			}
			...
			if parsed.Digest != "" {
				errs = append(errs, wrapWithContext(c, ErrorWithLocation{
					Error:    fmt.Errorf("invalid image %q: no digest should be specified. Use taggers instead: https://devloop.dev/docs/how-tos/taggers/", a.ImageName),
					Location: curLines,
				})...)
			}
		}
...
func validateArtifactDependencies(configs parser.DevloopConfigSet) (cfgErrs []ErrorWithLocation) {
...
func validateAcyclicDependencies(cfgs *parser.DevloopConfigSet, artifacts []*latest.Artifact) (cfgErrs []ErrorWithLocation) {
...
func validateValidDependencyAliases(cfgs *parser.DevloopConfigSet, artifacts []*latest.Artifact) (cfgErrs []ErrorWithLocation) {
...
func validateUniqueDependencyAliases(cfgs *parser.DevloopConfigSet, artifacts []*latest.Artifact) (cfgErrs []ErrorWithLocation) {
	type State int
	var (
		unseen   State = 0
		seen     State = 1
...
func validateDockerNetworkMode(cfg *parser.DevloopConfigEntry, artifacts []*latest.Artifact) (cfgErrs []ErrorWithLocation) {
	for i, a := range artifacts {
...
// Validate that test cases exist when `verify` is called, otherwise Devloop should error
...
func validateDockerNetworkContainerExists(ctx context.Context, artifacts []*latest.Artifact, runCtx docker.Config) []error {
	...
	for _, a := range artifacts {
		...
		if strings.HasPrefix(mode, prefix) {
			...
			containers, err := client.ContainerList(ctx, mobyclient.ContainerListOptions{})
			if err != nil {
...
func validateCustomDependencies(cfg *parser.DevloopConfigEntry, artifacts []*latest.Artifact) (cfgErrs []ErrorWithLocation) {
	for i, a := range artifacts {
...
func visitStructs(cfg *parser.DevloopConfigEntry, v reflect.Value, visitor func(interface{}) error) []ErrorWithLocation {
	switch v.Kind() {
	case reflect.Struct:
...
func validateSyncRules(cfg *parser.DevloopConfigEntry, artifacts []*latest.Artifact) []ErrorWithLocation {
	var cfgErrs []ErrorWithLocation
	for i, a := range artifacts {
		if a.Sync != nil {
...
func validatePortForwardResources(cfg *parser.DevloopConfigEntry, pfrs []*latest.PortForwardResource) []ErrorWithLocation {
...
func validateJibPluginTypes(cfg *parser.DevloopConfigEntry, artifacts []*latest.Artifact) (cfgErrs []ErrorWithLocation) {
	for i, a := range artifacts {
...
func validateKoSync(cfg *parser.DevloopConfigEntry, artifacts []*latest.Artifact) []ErrorWithLocation {
	var cfgErrs []ErrorWithLocation
	for i, a := range artifacts {
...
func validateArtifactTypes(cfg *parser.DevloopConfigEntry, bc latest.BuildConfig) []ErrorWithLocation {
	...
	case bc.LocalBuild != nil:
		for i, a := range bc.Artifacts {
			if misc.ArtifactType(a) == misc.Kaniko {
				cfgErrs = append(cfgErrs, ErrorWithLocation{
					Error:    fmt.Errorf("found a '%s' artifact, which is incompatible with the 'local' builder:\n\n%s\n\nTo use the '%s' builder, add the 'cluster' stanza to the 'build' section of your configuration. For information, see https://devloop.dev/docs/pipeline-stages/builders/", misc.ArtifactType(a), misc.FormatArtifact(a), misc.ArtifactType(a)),
	...
	case bc.Cluster != nil:
		for i, a := range bc.Artifacts {
			if misc.ArtifactType(a) != misc.Kaniko && misc.ArtifactType(a) != misc.Custom {
				cfgErrs = append(cfgErrs, ErrorWithLocation{
					Error:    fmt.Errorf("found a '%s' artifact, which is incompatible with the 'cluster' builder:\n\n%s\n\nTo use the '%s' builder, remove the 'cluster' stanza from the 'build' section of your configuration. For information, see https://devloop.dev/docs/pipeline-stages/builders/", misc.ArtifactType(a), misc.FormatArtifact(a), misc.ArtifactType(a)),
					Location: cfg.YAMLInfos.Locate(&cfg.Build.Artifacts[i].ArtifactType),
				})
			}
		}
...
func validateLogPrefix(cfg *parser.DevloopConfigEntry, lc latest.LogsConfig) []ErrorWithLocation {
...
func validateCustomTest(cfg *parser.DevloopConfigEntry, tcs []*latest.TestCase) (cfgErrs []ErrorWithLocation) {
	for i, tc := range tcs {
...
func wrapWithContext(config *parser.DevloopConfigEntry, errs ...ErrorWithLocation) []ErrorWithLocation {
	var id string
...
// - validates that kubectl manifest files specified in the devloop config exist
func validateKubectlManifests(configs parser.DevloopConfigSet) (errs []ErrorWithLocation) {
	for _, c := range configs {
		if c.IsRemote {
			continue
		}
		...
		for _, pattern := range c.Render.RawK8s {
			...
			if len(expanded) == 0 {
				...
				msg := fmt.Sprintf("Manifest file %q referenced in devloop config could not be found", pattern)
...
func validateLocationSetForCloudRun(rCtx *runcontext.RunContext) []error {
	...
	if runDeployer && !hasLocation {
		return []error{
			sErrors.NewError(errors.New("location must be specified with Cloud Run Deployer"),
				&proto.ActionableErr{
					...
					Suggestions: []*proto.Suggestion{
						{
							...
							Action: "Specify a Cloud Run location via the deploy.cloudrun.region field in devloop.yaml " +
