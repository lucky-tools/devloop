...
func (s *server) EventLog(stream proto.DevloopService_EventLogServer) error {
	return event.ForEachEvent(stream.Send)
}

func (s *server) Events(_ *empty.Empty, stream proto.DevloopService_EventsServer) error {
	return event.ForEachEvent(stream.Send)
}
