...
func TestTagTemplate_GenerateTag(t *testing.T) {
	aLocalTimeStamp := time.Date(2015, 0o3, 0o7, 11, 0o6, 39, 123456789, time.Local)
	...
	runCtx, _ := runcontext.GetRunContext(ctx, config.DevloopOptions{}, nil)
...
func TestCustomTemplate_NewCustomTemplateTagger(t *testing.T) {
	runCtx, _ := runcontext.GetRunContext(context.Background(), config.DevloopOptions{}, nil)
