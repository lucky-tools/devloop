...
var (
	version, gitCommit, buildDate, client string
...
// form `devloop/<version> (<os>/<arch>)`, and the version will be omitted if not available.
func UserAgent() string {
	if version == "" {
		// likely running under tests
		return fmt.Sprintf("devloop (%s)", platform)
	}
	return fmt.Sprintf("devloop/%s (%s)", version, platform)
}
...
// to `UserAgent()`.  Otherwise it is of the form `devloop/<version> (<os>/<arch>) <client>`, and the version
// will be omitted if not available.
// Use UserAgentWithClient method to record requests from devloop CLI users vs
// other clients.
