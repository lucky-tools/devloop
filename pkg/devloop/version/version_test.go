...
func TestUserAgent(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			platform:    "osx",
			version:     "1.0",
			expected:    "devloop/1.0 (osx)",
		},
		{
			description: "version not number specified",
			platform:    "osx",
			expected:    "devloop (osx)",
		},
	}
	for _, test := range tests {
...
func TestUserAgentWithClient(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			version:     "1.0",
			user:        "cloud-deploy",
			expected:    "devloop/1.0 (osx) cloud-deploy",
		},
		{
			...
			expected:    "devloop/1.0 (osx) cloud-deploy/dev",
		},
		{
